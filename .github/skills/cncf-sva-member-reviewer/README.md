...
7. Run `skcr validate` and `skcr asps coverage cncf-sva-member-reviewer`.
8. Run `skcr compile cncf-sva-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sva-member-reviewer`.
...
```bash
skpm validate cncf-sva-member-reviewer
skpm package cncf-sva-member-reviewer
skpm publish cncf-sva-member-reviewer
```
