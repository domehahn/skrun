...
7. Run `skcr validate` and `skcr asps coverage cncf-signoz-member-reviewer`.
8. Run `skcr compile cncf-signoz-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-signoz-member-reviewer`.
...
```bash
skpm validate cncf-signoz-member-reviewer
skpm package cncf-signoz-member-reviewer
skpm publish cncf-signoz-member-reviewer
```
