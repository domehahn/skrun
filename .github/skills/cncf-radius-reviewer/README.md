...
7. Run `skcr validate` and `skcr asps coverage cncf-radius-reviewer`.
8. Run `skcr compile cncf-radius-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-radius-reviewer`.
...
```bash
skpm validate cncf-radius-reviewer
skpm package cncf-radius-reviewer
skpm publish cncf-radius-reviewer
```
