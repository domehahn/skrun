...
7. Run `skcr validate` and `skcr asps coverage cncf-openscap-reviewer`.
8. Run `skcr compile cncf-openscap-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openscap-reviewer`.
...
```bash
skpm validate cncf-openscap-reviewer
skpm package cncf-openscap-reviewer
skpm publish cncf-openscap-reviewer
```
