...
7. Run `skcr validate` and `skcr asps coverage cncf-vmware-vsphere-reviewer`.
8. Run `skcr compile cncf-vmware-vsphere-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vmware-vsphere-reviewer`.
...
```bash
skpm validate cncf-vmware-vsphere-reviewer
skpm package cncf-vmware-vsphere-reviewer
skpm publish cncf-vmware-vsphere-reviewer
```
