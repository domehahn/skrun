...
7. Run `skcr validate` and `skcr asps coverage cncf-teller-reviewer`.
8. Run `skcr compile cncf-teller-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-teller-reviewer`.
...
```bash
skpm validate cncf-teller-reviewer
skpm package cncf-teller-reviewer
skpm publish cncf-teller-reviewer
```
