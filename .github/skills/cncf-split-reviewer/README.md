...
7. Run `skcr validate` and `skcr asps coverage cncf-split-reviewer`.
8. Run `skcr compile cncf-split-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-split-reviewer`.
...
```bash
skpm validate cncf-split-reviewer
skpm package cncf-split-reviewer
skpm publish cncf-split-reviewer
```
