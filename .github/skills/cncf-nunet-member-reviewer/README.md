...
7. Run `skcr validate` and `skcr asps coverage cncf-nunet-member-reviewer`.
8. Run `skcr compile cncf-nunet-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nunet-member-reviewer`.
...
```bash
skpm validate cncf-nunet-member-reviewer
skpm package cncf-nunet-member-reviewer
skpm publish cncf-nunet-member-reviewer
```
