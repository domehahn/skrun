...
7. Run `skcr validate` and `skcr asps coverage cncf-autogen-reviewer`.
8. Run `skcr compile cncf-autogen-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-autogen-reviewer`.
...
```bash
skpm validate cncf-autogen-reviewer
skpm package cncf-autogen-reviewer
skpm publish cncf-autogen-reviewer
```
