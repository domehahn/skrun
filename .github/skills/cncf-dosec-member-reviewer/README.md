...
7. Run `skcr validate` and `skcr asps coverage cncf-dosec-member-reviewer`.
8. Run `skcr compile cncf-dosec-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dosec-member-reviewer`.
...
```bash
skpm validate cncf-dosec-member-reviewer
skpm package cncf-dosec-member-reviewer
skpm publish cncf-dosec-member-reviewer
```
