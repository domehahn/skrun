...
7. Run `skcr validate` and `skcr asps coverage cncf-k0s-reviewer`.
8. Run `skcr compile cncf-k0s-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-k0s-reviewer`.
...
```bash
skpm validate cncf-k0s-reviewer
skpm package cncf-k0s-reviewer
skpm publish cncf-k0s-reviewer
```
