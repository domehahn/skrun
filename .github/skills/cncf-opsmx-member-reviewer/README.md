...
7. Run `skcr validate` and `skcr asps coverage cncf-opsmx-member-reviewer`.
8. Run `skcr compile cncf-opsmx-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opsmx-member-reviewer`.
...
```bash
skpm validate cncf-opsmx-member-reviewer
skpm package cncf-opsmx-member-reviewer
skpm publish cncf-opsmx-member-reviewer
```
