...
7. Run `skcr validate` and `skcr asps coverage cncf-moosefs-reviewer`.
8. Run `skcr compile cncf-moosefs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-moosefs-reviewer`.
...
```bash
skpm validate cncf-moosefs-reviewer
skpm package cncf-moosefs-reviewer
skpm publish cncf-moosefs-reviewer
```
