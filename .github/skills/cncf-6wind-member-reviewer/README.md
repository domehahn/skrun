...
7. Run `skcr validate` and `skcr asps coverage cncf-6wind-member-reviewer`.
8. Run `skcr compile cncf-6wind-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-6wind-member-reviewer`.
...
```bash
skpm validate cncf-6wind-member-reviewer
skpm package cncf-6wind-member-reviewer
skpm publish cncf-6wind-member-reviewer
```
