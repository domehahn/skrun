...
7. Run `skcr validate` and `skcr asps coverage cncf-spotify-member-reviewer`.
8. Run `skcr compile cncf-spotify-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spotify-member-reviewer`.
...
```bash
skpm validate cncf-spotify-member-reviewer
skpm package cncf-spotify-member-reviewer
skpm publish cncf-spotify-member-reviewer
```
