...
7. Run `skcr validate` and `skcr asps coverage cncf-seal-member-reviewer`.
8. Run `skcr compile cncf-seal-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-seal-member-reviewer`.
...
```bash
skpm validate cncf-seal-member-reviewer
skpm package cncf-seal-member-reviewer
skpm publish cncf-seal-member-reviewer
```
