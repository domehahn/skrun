...
7. Run `skcr validate` and `skcr asps coverage cncf-graphite-reviewer`.
8. Run `skcr compile cncf-graphite-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-graphite-reviewer`.
...
```bash
skpm validate cncf-graphite-reviewer
skpm package cncf-graphite-reviewer
skpm publish cncf-graphite-reviewer
```
