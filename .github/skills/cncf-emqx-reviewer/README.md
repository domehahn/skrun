...
7. Run `skcr validate` and `skcr asps coverage cncf-emqx-reviewer`.
8. Run `skcr compile cncf-emqx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-emqx-reviewer`.
...
```bash
skpm validate cncf-emqx-reviewer
skpm package cncf-emqx-reviewer
skpm publish cncf-emqx-reviewer
```
