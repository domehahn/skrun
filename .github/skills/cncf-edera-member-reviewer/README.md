...
7. Run `skcr validate` and `skcr asps coverage cncf-edera-member-reviewer`.
8. Run `skcr compile cncf-edera-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-edera-member-reviewer`.
...
```bash
skpm validate cncf-edera-member-reviewer
skpm package cncf-edera-member-reviewer
skpm publish cncf-edera-member-reviewer
```
