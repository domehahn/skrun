...
7. Run `skcr validate` and `skcr asps coverage cncf-cilium-reviewer`.
8. Run `skcr compile cncf-cilium-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cilium-reviewer`.
...
```bash
skpm validate cncf-cilium-reviewer
skpm package cncf-cilium-reviewer
skpm publish cncf-cilium-reviewer
```
