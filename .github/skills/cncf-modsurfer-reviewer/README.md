...
7. Run `skcr validate` and `skcr asps coverage cncf-modsurfer-reviewer`.
8. Run `skcr compile cncf-modsurfer-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-modsurfer-reviewer`.
...
```bash
skpm validate cncf-modsurfer-reviewer
skpm package cncf-modsurfer-reviewer
skpm publish cncf-modsurfer-reviewer
```
