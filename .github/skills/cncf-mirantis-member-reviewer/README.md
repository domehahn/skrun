...
7. Run `skcr validate` and `skcr asps coverage cncf-mirantis-member-reviewer`.
8. Run `skcr compile cncf-mirantis-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mirantis-member-reviewer`.
...
```bash
skpm validate cncf-mirantis-member-reviewer
skpm package cncf-mirantis-member-reviewer
skpm publish cncf-mirantis-member-reviewer
```
