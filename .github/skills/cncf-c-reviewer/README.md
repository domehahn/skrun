...
7. Run `skcr validate` and `skcr asps coverage cncf-c-reviewer`.
8. Run `skcr compile cncf-c-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-c-reviewer`.
...
```bash
skpm validate cncf-c-reviewer
skpm package cncf-c-reviewer
skpm publish cncf-c-reviewer
```
