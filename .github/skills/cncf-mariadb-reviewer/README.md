...
7. Run `skcr validate` and `skcr asps coverage cncf-mariadb-reviewer`.
8. Run `skcr compile cncf-mariadb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mariadb-reviewer`.
...
```bash
skpm validate cncf-mariadb-reviewer
skpm package cncf-mariadb-reviewer
skpm publish cncf-mariadb-reviewer
```
