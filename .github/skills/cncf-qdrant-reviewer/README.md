...
7. Run `skcr validate` and `skcr asps coverage cncf-qdrant-reviewer`.
8. Run `skcr compile cncf-qdrant-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-qdrant-reviewer`.
...
```bash
skpm validate cncf-qdrant-reviewer
skpm package cncf-qdrant-reviewer
skpm publish cncf-qdrant-reviewer
```
