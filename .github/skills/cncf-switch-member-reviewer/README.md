...
7. Run `skcr validate` and `skcr asps coverage cncf-switch-member-reviewer`.
8. Run `skcr compile cncf-switch-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-switch-member-reviewer`.
...
```bash
skpm validate cncf-switch-member-reviewer
skpm package cncf-switch-member-reviewer
skpm publish cncf-switch-member-reviewer
```
