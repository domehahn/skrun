...
7. Run `skcr validate` and `skcr asps coverage cncf-skipper-reviewer`.
8. Run `skcr compile cncf-skipper-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-skipper-reviewer`.
...
```bash
skpm validate cncf-skipper-reviewer
skpm package cncf-skipper-reviewer
skpm publish cncf-skipper-reviewer
```
