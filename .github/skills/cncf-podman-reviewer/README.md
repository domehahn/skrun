...
7. Run `skcr validate` and `skcr asps coverage cncf-podman-reviewer`.
8. Run `skcr compile cncf-podman-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-podman-reviewer`.
...
```bash
skpm validate cncf-podman-reviewer
skpm package cncf-podman-reviewer
skpm publish cncf-podman-reviewer
```
