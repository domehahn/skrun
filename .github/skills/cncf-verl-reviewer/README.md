...
7. Run `skcr validate` and `skcr asps coverage cncf-verl-reviewer`.
8. Run `skcr compile cncf-verl-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-verl-reviewer`.
...
```bash
skpm validate cncf-verl-reviewer
skpm package cncf-verl-reviewer
skpm publish cncf-verl-reviewer
```
