...
7. Run `skcr validate` and `skcr asps coverage cncf-headlamp-reviewer`.
8. Run `skcr compile cncf-headlamp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-headlamp-reviewer`.
...
```bash
skpm validate cncf-headlamp-reviewer
skpm package cncf-headlamp-reviewer
skpm publish cncf-headlamp-reviewer
```
