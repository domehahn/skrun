...
7. Run `skcr validate` and `skcr asps coverage cncf-develocity-reviewer`.
8. Run `skcr compile cncf-develocity-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-develocity-reviewer`.
...
```bash
skpm validate cncf-develocity-reviewer
skpm package cncf-develocity-reviewer
skpm publish cncf-develocity-reviewer
```
