...
7. Run `skcr validate` and `skcr asps coverage cncf-aviatrix-reviewer`.
8. Run `skcr compile cncf-aviatrix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-aviatrix-reviewer`.
...
```bash
skpm validate cncf-aviatrix-reviewer
skpm package cncf-aviatrix-reviewer
skpm publish cncf-aviatrix-reviewer
```
