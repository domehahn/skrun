...
7. Run `skcr validate` and `skcr asps coverage typescript-reviewer`.
8. Run `skcr compile typescript-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend typescript-reviewer`.
...
```bash
skpm validate typescript-reviewer
skpm package typescript-reviewer
skpm publish typescript-reviewer
```
