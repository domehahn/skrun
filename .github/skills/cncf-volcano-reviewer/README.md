...
7. Run `skcr validate` and `skcr asps coverage cncf-volcano-reviewer`.
8. Run `skcr compile cncf-volcano-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-volcano-reviewer`.
...
```bash
skpm validate cncf-volcano-reviewer
skpm package cncf-volcano-reviewer
skpm publish cncf-volcano-reviewer
```
