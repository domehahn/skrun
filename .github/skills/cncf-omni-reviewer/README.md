...
7. Run `skcr validate` and `skcr asps coverage cncf-omni-reviewer`.
8. Run `skcr compile cncf-omni-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-omni-reviewer`.
...
```bash
skpm validate cncf-omni-reviewer
skpm package cncf-omni-reviewer
skpm publish cncf-omni-reviewer
```
