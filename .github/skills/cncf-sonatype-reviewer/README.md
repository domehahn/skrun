...
7. Run `skcr validate` and `skcr asps coverage cncf-sonatype-reviewer`.
8. Run `skcr compile cncf-sonatype-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sonatype-reviewer`.
...
```bash
skpm validate cncf-sonatype-reviewer
skpm package cncf-sonatype-reviewer
skpm publish cncf-sonatype-reviewer
```
