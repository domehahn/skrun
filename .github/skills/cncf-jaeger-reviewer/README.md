...
7. Run `skcr validate` and `skcr asps coverage cncf-jaeger-reviewer`.
8. Run `skcr compile cncf-jaeger-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-jaeger-reviewer`.
...
```bash
skpm validate cncf-jaeger-reviewer
skpm package cncf-jaeger-reviewer
skpm publish cncf-jaeger-reviewer
```
