...
7. Run `skcr validate` and `skcr asps coverage cncf-mosn-reviewer`.
8. Run `skcr compile cncf-mosn-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mosn-reviewer`.
...
```bash
skpm validate cncf-mosn-reviewer
skpm package cncf-mosn-reviewer
skpm publish cncf-mosn-reviewer
```
