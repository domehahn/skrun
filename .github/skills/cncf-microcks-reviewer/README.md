...
7. Run `skcr validate` and `skcr asps coverage cncf-microcks-reviewer`.
8. Run `skcr compile cncf-microcks-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-microcks-reviewer`.
...
```bash
skpm validate cncf-microcks-reviewer
skpm package cncf-microcks-reviewer
skpm publish cncf-microcks-reviewer
```
