...
7. Run `skcr validate` and `skcr asps coverage cncf-logstash-reviewer`.
8. Run `skcr compile cncf-logstash-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-logstash-reviewer`.
...
```bash
skpm validate cncf-logstash-reviewer
skpm package cncf-logstash-reviewer
skpm publish cncf-logstash-reviewer
```
