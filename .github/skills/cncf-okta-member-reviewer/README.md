...
7. Run `skcr validate` and `skcr asps coverage cncf-okta-member-reviewer`.
8. Run `skcr compile cncf-okta-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-okta-member-reviewer`.
...
```bash
skpm validate cncf-okta-member-reviewer
skpm package cncf-okta-member-reviewer
skpm publish cncf-okta-member-reviewer
```
