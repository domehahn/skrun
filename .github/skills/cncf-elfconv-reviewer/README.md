...
7. Run `skcr validate` and `skcr asps coverage cncf-elfconv-reviewer`.
8. Run `skcr compile cncf-elfconv-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-elfconv-reviewer`.
...
```bash
skpm validate cncf-elfconv-reviewer
skpm package cncf-elfconv-reviewer
skpm publish cncf-elfconv-reviewer
```
