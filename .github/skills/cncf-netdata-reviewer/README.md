...
7. Run `skcr validate` and `skcr asps coverage cncf-netdata-reviewer`.
8. Run `skcr compile cncf-netdata-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-netdata-reviewer`.
...
```bash
skpm validate cncf-netdata-reviewer
skpm package cncf-netdata-reviewer
skpm publish cncf-netdata-reviewer
```
