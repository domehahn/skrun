...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeorbit-reviewer`.
8. Run `skcr compile cncf-kubeorbit-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeorbit-reviewer`.
...
```bash
skpm validate cncf-kubeorbit-reviewer
skpm package cncf-kubeorbit-reviewer
skpm publish cncf-kubeorbit-reviewer
```
