...
7. Run `skcr validate` and `skcr asps coverage cncf-razee-reviewer`.
8. Run `skcr compile cncf-razee-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-razee-reviewer`.
...
```bash
skpm validate cncf-razee-reviewer
skpm package cncf-razee-reviewer
skpm publish cncf-razee-reviewer
```
