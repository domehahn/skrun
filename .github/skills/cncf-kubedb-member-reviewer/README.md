...
7. Run `skcr validate` and `skcr asps coverage cncf-kubedb-member-reviewer`.
8. Run `skcr compile cncf-kubedb-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubedb-member-reviewer`.
...
```bash
skpm validate cncf-kubedb-member-reviewer
skpm package cncf-kubedb-member-reviewer
skpm publish cncf-kubedb-member-reviewer
```
