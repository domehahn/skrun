...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeasz-reviewer`.
8. Run `skcr compile cncf-kubeasz-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeasz-reviewer`.
...
```bash
skpm validate cncf-kubeasz-reviewer
skpm package cncf-kubeasz-reviewer
skpm publish cncf-kubeasz-reviewer
```
