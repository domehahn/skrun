...
7. Run `skcr validate` and `skcr asps coverage cncf-logiq-reviewer`.
8. Run `skcr compile cncf-logiq-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-logiq-reviewer`.
...
```bash
skpm validate cncf-logiq-reviewer
skpm package cncf-logiq-reviewer
skpm publish cncf-logiq-reviewer
```
