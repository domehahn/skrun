...
7. Run `skcr validate` and `skcr asps coverage cncf-valkey-reviewer`.
8. Run `skcr compile cncf-valkey-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-valkey-reviewer`.
...
```bash
skpm validate cncf-valkey-reviewer
skpm package cncf-valkey-reviewer
skpm publish cncf-valkey-reviewer
```
