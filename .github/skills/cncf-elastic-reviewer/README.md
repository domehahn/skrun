...
7. Run `skcr validate` and `skcr asps coverage cncf-elastic-reviewer`.
8. Run `skcr compile cncf-elastic-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-elastic-reviewer`.
...
```bash
skpm validate cncf-elastic-reviewer
skpm package cncf-elastic-reviewer
skpm publish cncf-elastic-reviewer
```
