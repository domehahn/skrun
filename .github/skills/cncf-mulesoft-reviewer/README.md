...
7. Run `skcr validate` and `skcr asps coverage cncf-mulesoft-reviewer`.
8. Run `skcr compile cncf-mulesoft-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mulesoft-reviewer`.
...
```bash
skpm validate cncf-mulesoft-reviewer
skpm package cncf-mulesoft-reviewer
skpm publish cncf-mulesoft-reviewer
```
