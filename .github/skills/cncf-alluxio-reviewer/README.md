...
7. Run `skcr validate` and `skcr asps coverage cncf-alluxio-reviewer`.
8. Run `skcr compile cncf-alluxio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-alluxio-reviewer`.
...
```bash
skpm validate cncf-alluxio-reviewer
skpm package cncf-alluxio-reviewer
skpm publish cncf-alluxio-reviewer
```
