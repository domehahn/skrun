...
7. Run `skcr validate` and `skcr asps coverage cncf-quarkus-reviewer`.
8. Run `skcr compile cncf-quarkus-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-quarkus-reviewer`.
...
```bash
skpm validate cncf-quarkus-reviewer
skpm package cncf-quarkus-reviewer
skpm publish cncf-quarkus-reviewer
```
