...
7. Run `skcr validate` and `skcr asps coverage cncf-kubetail-reviewer`.
8. Run `skcr compile cncf-kubetail-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubetail-reviewer`.
...
```bash
skpm validate cncf-kubetail-reviewer
skpm package cncf-kubetail-reviewer
skpm publish cncf-kubetail-reviewer
```
