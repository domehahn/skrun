...
7. Run `skcr validate` and `skcr asps coverage cncf-root-member-reviewer`.
8. Run `skcr compile cncf-root-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-root-member-reviewer`.
...
```bash
skpm validate cncf-root-member-reviewer
skpm package cncf-root-member-reviewer
skpm publish cncf-root-member-reviewer
```
