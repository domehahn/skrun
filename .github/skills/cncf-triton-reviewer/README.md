...
7. Run `skcr validate` and `skcr asps coverage cncf-triton-reviewer`.
8. Run `skcr compile cncf-triton-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-triton-reviewer`.
...
```bash
skpm validate cncf-triton-reviewer
skpm package cncf-triton-reviewer
skpm publish cncf-triton-reviewer
```
