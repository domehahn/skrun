...
7. Run `skcr validate` and `skcr asps coverage cncf-kubearmor-reviewer`.
8. Run `skcr compile cncf-kubearmor-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubearmor-reviewer`.
...
```bash
skpm validate cncf-kubearmor-reviewer
skpm package cncf-kubearmor-reviewer
skpm publish cncf-kubearmor-reviewer
```
