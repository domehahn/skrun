...
7. Run `skcr validate` and `skcr asps coverage cncf-spinkube-reviewer`.
8. Run `skcr compile cncf-spinkube-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spinkube-reviewer`.
...
```bash
skpm validate cncf-spinkube-reviewer
skpm package cncf-spinkube-reviewer
skpm publish cncf-spinkube-reviewer
```
