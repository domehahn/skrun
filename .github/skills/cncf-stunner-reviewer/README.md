...
7. Run `skcr validate` and `skcr asps coverage cncf-stunner-reviewer`.
8. Run `skcr compile cncf-stunner-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-stunner-reviewer`.
...
```bash
skpm validate cncf-stunner-reviewer
skpm package cncf-stunner-reviewer
skpm publish cncf-stunner-reviewer
```
