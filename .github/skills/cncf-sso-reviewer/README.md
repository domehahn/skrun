...
7. Run `skcr validate` and `skcr asps coverage cncf-sso-reviewer`.
8. Run `skcr compile cncf-sso-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sso-reviewer`.
...
```bash
skpm validate cncf-sso-reviewer
skpm package cncf-sso-reviewer
skpm publish cncf-sso-reviewer
```
