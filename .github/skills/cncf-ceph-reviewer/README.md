...
7. Run `skcr validate` and `skcr asps coverage cncf-ceph-reviewer`.
8. Run `skcr compile cncf-ceph-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ceph-reviewer`.
...
```bash
skpm validate cncf-ceph-reviewer
skpm package cncf-ceph-reviewer
skpm publish cncf-ceph-reviewer
```
