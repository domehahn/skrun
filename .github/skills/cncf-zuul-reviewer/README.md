...
7. Run `skcr validate` and `skcr asps coverage cncf-zuul-reviewer`.
8. Run `skcr compile cncf-zuul-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zuul-reviewer`.
...
```bash
skpm validate cncf-zuul-reviewer
skpm package cncf-zuul-reviewer
skpm publish cncf-zuul-reviewer
```
