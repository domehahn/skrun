...
7. Run `skcr validate` and `skcr asps coverage cncf-percona-member-reviewer`.
8. Run `skcr compile cncf-percona-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-percona-member-reviewer`.
...
```bash
skpm validate cncf-percona-member-reviewer
skpm package cncf-percona-member-reviewer
skpm publish cncf-percona-member-reviewer
```
