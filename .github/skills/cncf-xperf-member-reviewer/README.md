...
7. Run `skcr validate` and `skcr asps coverage cncf-xperf-member-reviewer`.
8. Run `skcr compile cncf-xperf-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-xperf-member-reviewer`.
...
```bash
skpm validate cncf-xperf-member-reviewer
skpm package cncf-xperf-member-reviewer
skpm publish cncf-xperf-member-reviewer
```
