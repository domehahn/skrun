...
7. Run `skcr validate` and `skcr asps coverage cncf-sysdig-member-reviewer`.
8. Run `skcr compile cncf-sysdig-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sysdig-member-reviewer`.
...
```bash
skpm validate cncf-sysdig-member-reviewer
skpm package cncf-sysdig-member-reviewer
skpm publish cncf-sysdig-member-reviewer
```
