...
7. Run `skcr validate` and `skcr asps coverage cncf-e-on-member-reviewer`.
8. Run `skcr compile cncf-e-on-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-e-on-member-reviewer`.
...
```bash
skpm validate cncf-e-on-member-reviewer
skpm package cncf-e-on-member-reviewer
skpm publish cncf-e-on-member-reviewer
```
