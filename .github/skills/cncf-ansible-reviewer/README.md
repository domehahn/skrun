...
7. Run `skcr validate` and `skcr asps coverage cncf-ansible-reviewer`.
8. Run `skcr compile cncf-ansible-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ansible-reviewer`.
...
```bash
skpm validate cncf-ansible-reviewer
skpm package cncf-ansible-reviewer
skpm publish cncf-ansible-reviewer
```
