...
7. Run `skcr validate` and `skcr asps coverage cncf-armo-member-reviewer`.
8. Run `skcr compile cncf-armo-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-armo-member-reviewer`.
...
```bash
skpm validate cncf-armo-member-reviewer
skpm package cncf-armo-member-reviewer
skpm publish cncf-armo-member-reviewer
```
