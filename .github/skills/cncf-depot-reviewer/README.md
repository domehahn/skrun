...
7. Run `skcr validate` and `skcr asps coverage cncf-depot-reviewer`.
8. Run `skcr compile cncf-depot-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-depot-reviewer`.
...
```bash
skpm validate cncf-depot-reviewer
skpm package cncf-depot-reviewer
skpm publish cncf-depot-reviewer
```
