...
7. Run `skcr validate` and `skcr asps coverage cncf-kubevela-reviewer`.
8. Run `skcr compile cncf-kubevela-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubevela-reviewer`.
...
```bash
skpm validate cncf-kubevela-reviewer
skpm package cncf-kubevela-reviewer
skpm publish cncf-kubevela-reviewer
```
