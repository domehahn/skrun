...
7. Run `skcr validate` and `skcr asps coverage cncf-influxdb-reviewer`.
8. Run `skcr compile cncf-influxdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-influxdb-reviewer`.
...
```bash
skpm validate cncf-influxdb-reviewer
skpm package cncf-influxdb-reviewer
skpm publish cncf-influxdb-reviewer
```
