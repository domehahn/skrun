...
7. Run `skcr validate` and `skcr asps coverage cncf-apache-tvm-reviewer`.
8. Run `skcr compile cncf-apache-tvm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-apache-tvm-reviewer`.
...
```bash
skpm validate cncf-apache-tvm-reviewer
skpm package cncf-apache-tvm-reviewer
skpm publish cncf-apache-tvm-reviewer
```
