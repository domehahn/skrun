...
7. Run `skcr validate` and `skcr asps coverage cncf-numaflow-reviewer`.
8. Run `skcr compile cncf-numaflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-numaflow-reviewer`.
...
```bash
skpm validate cncf-numaflow-reviewer
skpm package cncf-numaflow-reviewer
skpm publish cncf-numaflow-reviewer
```
