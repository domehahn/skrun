...
7. Run `skcr validate` and `skcr asps coverage cncf-kong-reviewer`.
8. Run `skcr compile cncf-kong-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kong-reviewer`.
...
```bash
skpm validate cncf-kong-reviewer
skpm package cncf-kong-reviewer
skpm publish cncf-kong-reviewer
```
