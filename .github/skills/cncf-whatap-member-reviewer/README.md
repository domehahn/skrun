...
7. Run `skcr validate` and `skcr asps coverage cncf-whatap-member-reviewer`.
8. Run `skcr compile cncf-whatap-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-whatap-member-reviewer`.
...
```bash
skpm validate cncf-whatap-member-reviewer
skpm package cncf-whatap-member-reviewer
skpm publish cncf-whatap-member-reviewer
```
