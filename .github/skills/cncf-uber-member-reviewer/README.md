...
7. Run `skcr validate` and `skcr asps coverage cncf-uber-member-reviewer`.
8. Run `skcr compile cncf-uber-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-uber-member-reviewer`.
...
```bash
skpm validate cncf-uber-member-reviewer
skpm package cncf-uber-member-reviewer
skpm publish cncf-uber-member-reviewer
```
