...
7. Run `skcr validate` and `skcr asps coverage cncf-falkordb-reviewer`.
8. Run `skcr compile cncf-falkordb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-falkordb-reviewer`.
...
```bash
skpm validate cncf-falkordb-reviewer
skpm package cncf-falkordb-reviewer
skpm publish cncf-falkordb-reviewer
```
