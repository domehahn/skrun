...
7. Run `skcr validate` and `skcr asps coverage cncf-grepr-member-reviewer`.
8. Run `skcr compile cncf-grepr-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grepr-member-reviewer`.
...
```bash
skpm validate cncf-grepr-member-reviewer
skpm package cncf-grepr-member-reviewer
skpm publish cncf-grepr-member-reviewer
```
