...
7. Run `skcr validate` and `skcr asps coverage ansible-reviewer`.
8. Run `skcr compile ansible-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend ansible-reviewer`.
...
```bash
skpm validate ansible-reviewer
skpm package ansible-reviewer
skpm publish ansible-reviewer
```
