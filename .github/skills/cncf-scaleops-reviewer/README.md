...
7. Run `skcr validate` and `skcr asps coverage cncf-scaleops-reviewer`.
8. Run `skcr compile cncf-scaleops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scaleops-reviewer`.
...
```bash
skpm validate cncf-scaleops-reviewer
skpm package cncf-scaleops-reviewer
skpm publish cncf-scaleops-reviewer
```
