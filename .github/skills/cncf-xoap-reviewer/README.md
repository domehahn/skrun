...
7. Run `skcr validate` and `skcr asps coverage cncf-xoap-reviewer`.
8. Run `skcr compile cncf-xoap-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-xoap-reviewer`.
...
```bash
skpm validate cncf-xoap-reviewer
skpm package cncf-xoap-reviewer
skpm publish cncf-xoap-reviewer
```
