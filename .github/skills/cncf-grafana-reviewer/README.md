...
7. Run `skcr validate` and `skcr asps coverage cncf-grafana-reviewer`.
8. Run `skcr compile cncf-grafana-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grafana-reviewer`.
...
```bash
skpm validate cncf-grafana-reviewer
skpm package cncf-grafana-reviewer
skpm publish cncf-grafana-reviewer
```
