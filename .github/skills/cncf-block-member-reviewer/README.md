...
7. Run `skcr validate` and `skcr asps coverage cncf-block-member-reviewer`.
8. Run `skcr compile cncf-block-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-block-member-reviewer`.
...
```bash
skpm validate cncf-block-member-reviewer
skpm package cncf-block-member-reviewer
skpm publish cncf-block-member-reviewer
```
