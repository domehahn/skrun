...
7. Run `skcr validate` and `skcr asps coverage cncf-kotlin-reviewer`.
8. Run `skcr compile cncf-kotlin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kotlin-reviewer`.
...
```bash
skpm validate cncf-kotlin-reviewer
skpm package cncf-kotlin-reviewer
skpm publish cncf-kotlin-reviewer
```
