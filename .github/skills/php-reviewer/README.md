...
7. Run `skcr validate` and `skcr asps coverage php-reviewer`.
8. Run `skcr compile php-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend php-reviewer`.
...
```bash
skpm validate php-reviewer
skpm package php-reviewer
skpm publish php-reviewer
```
