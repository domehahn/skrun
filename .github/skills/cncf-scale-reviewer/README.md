...
7. Run `skcr validate` and `skcr asps coverage cncf-scale-reviewer`.
8. Run `skcr compile cncf-scale-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scale-reviewer`.
...
```bash
skpm validate cncf-scale-reviewer
skpm package cncf-scale-reviewer
skpm publish cncf-scale-reviewer
```
