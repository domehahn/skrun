...
7. Run `skcr validate` and `skcr asps coverage cncf-genode-reviewer`.
8. Run `skcr compile cncf-genode-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-genode-reviewer`.
...
```bash
skpm validate cncf-genode-reviewer
skpm package cncf-genode-reviewer
skpm publish cncf-genode-reviewer
```
