...
7. Run `skcr validate` and `skcr asps coverage cncf-lancedb-reviewer`.
8. Run `skcr compile cncf-lancedb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lancedb-reviewer`.
...
```bash
skpm validate cncf-lancedb-reviewer
skpm package cncf-lancedb-reviewer
skpm publish cncf-lancedb-reviewer
```
