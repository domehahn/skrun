...
7. Run `skcr validate` and `skcr asps coverage cncf-atomist-reviewer`.
8. Run `skcr compile cncf-atomist-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-atomist-reviewer`.
...
```bash
skpm validate cncf-atomist-reviewer
skpm package cncf-atomist-reviewer
skpm publish cncf-atomist-reviewer
```
