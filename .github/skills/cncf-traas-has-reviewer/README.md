...
7. Run `skcr validate` and `skcr asps coverage cncf-traas-has-reviewer`.
8. Run `skcr compile cncf-traas-has-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-traas-has-reviewer`.
...
```bash
skpm validate cncf-traas-has-reviewer
skpm package cncf-traas-has-reviewer
skpm publish cncf-traas-has-reviewer
```
