...
7. Run `skcr validate` and `skcr asps coverage cncf-kepler-reviewer`.
8. Run `skcr compile cncf-kepler-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kepler-reviewer`.
...
```bash
skpm validate cncf-kepler-reviewer
skpm package cncf-kepler-reviewer
skpm publish cncf-kepler-reviewer
```
