...
7. Run `skcr validate` and `skcr asps coverage cncf-logz-io-reviewer`.
8. Run `skcr compile cncf-logz-io-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-logz-io-reviewer`.
...
```bash
skpm validate cncf-logz-io-reviewer
skpm package cncf-logz-io-reviewer
skpm publish cncf-logz-io-reviewer
```
