...
7. Run `skcr validate` and `skcr asps coverage cncf-buildkite-reviewer`.
8. Run `skcr compile cncf-buildkite-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-buildkite-reviewer`.
...
```bash
skpm validate cncf-buildkite-reviewer
skpm package cncf-buildkite-reviewer
skpm publish cncf-buildkite-reviewer
```
