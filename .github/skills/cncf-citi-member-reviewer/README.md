...
7. Run `skcr validate` and `skcr asps coverage cncf-citi-member-reviewer`.
8. Run `skcr compile cncf-citi-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-citi-member-reviewer`.
...
```bash
skpm validate cncf-citi-member-reviewer
skpm package cncf-citi-member-reviewer
skpm publish cncf-citi-member-reviewer
```
