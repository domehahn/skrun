...
7. Run `skcr validate` and `skcr asps coverage cncf-conoa-member-reviewer`.
8. Run `skcr compile cncf-conoa-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-conoa-member-reviewer`.
...
```bash
skpm validate cncf-conoa-member-reviewer
skpm package cncf-conoa-member-reviewer
skpm publish cncf-conoa-member-reviewer
```
