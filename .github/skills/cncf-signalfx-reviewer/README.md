...
7. Run `skcr validate` and `skcr asps coverage cncf-signalfx-reviewer`.
8. Run `skcr compile cncf-signalfx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-signalfx-reviewer`.
...
```bash
skpm validate cncf-signalfx-reviewer
skpm package cncf-signalfx-reviewer
skpm publish cncf-signalfx-reviewer
```
