...
7. Run `skcr validate` and `skcr asps coverage cncf-zstack-member-reviewer`.
8. Run `skcr compile cncf-zstack-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zstack-member-reviewer`.
...
```bash
skpm validate cncf-zstack-member-reviewer
skpm package cncf-zstack-member-reviewer
skpm publish cncf-zstack-member-reviewer
```
