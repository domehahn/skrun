...
7. Run `skcr validate` and `skcr asps coverage cncf-distr-reviewer`.
8. Run `skcr compile cncf-distr-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-distr-reviewer`.
...
```bash
skpm validate cncf-distr-reviewer
skpm package cncf-distr-reviewer
skpm publish cncf-distr-reviewer
```
