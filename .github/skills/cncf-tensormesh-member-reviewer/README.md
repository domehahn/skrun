...
7. Run `skcr validate` and `skcr asps coverage cncf-tensormesh-member-reviewer`.
8. Run `skcr compile cncf-tensormesh-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tensormesh-member-reviewer`.
...
```bash
skpm validate cncf-tensormesh-member-reviewer
skpm package cncf-tensormesh-member-reviewer
skpm publish cncf-tensormesh-member-reviewer
```
