...
7. Run `skcr validate` and `skcr asps coverage cncf-arpio-member-reviewer`.
8. Run `skcr compile cncf-arpio-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-arpio-member-reviewer`.
...
```bash
skpm validate cncf-arpio-member-reviewer
skpm package cncf-arpio-member-reviewer
skpm publish cncf-arpio-member-reviewer
```
