...
7. Run `skcr validate` and `skcr asps coverage cncf-synax-member-reviewer`.
8. Run `skcr compile cncf-synax-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-synax-member-reviewer`.
...
```bash
skpm validate cncf-synax-member-reviewer
skpm package cncf-synax-member-reviewer
skpm publish cncf-synax-member-reviewer
```
