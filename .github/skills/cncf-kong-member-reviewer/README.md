...
7. Run `skcr validate` and `skcr asps coverage cncf-kong-member-reviewer`.
8. Run `skcr compile cncf-kong-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kong-member-reviewer`.
...
```bash
skpm validate cncf-kong-member-reviewer
skpm package cncf-kong-member-reviewer
skpm publish cncf-kong-member-reviewer
```
