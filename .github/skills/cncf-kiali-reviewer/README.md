...
7. Run `skcr validate` and `skcr asps coverage cncf-kiali-reviewer`.
8. Run `skcr compile cncf-kiali-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kiali-reviewer`.
...
```bash
skpm validate cncf-kiali-reviewer
skpm package cncf-kiali-reviewer
skpm publish cncf-kiali-reviewer
```
