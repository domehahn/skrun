...
7. Run `skcr validate` and `skcr asps coverage cncf-torchx-reviewer`.
8. Run `skcr compile cncf-torchx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-torchx-reviewer`.
...
```bash
skpm validate cncf-torchx-reviewer
skpm package cncf-torchx-reviewer
skpm publish cncf-torchx-reviewer
```
