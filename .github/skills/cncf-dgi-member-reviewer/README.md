...
7. Run `skcr validate` and `skcr asps coverage cncf-dgi-member-reviewer`.
8. Run `skcr compile cncf-dgi-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dgi-member-reviewer`.
...
```bash
skpm validate cncf-dgi-member-reviewer
skpm package cncf-dgi-member-reviewer
skpm publish cncf-dgi-member-reviewer
```
