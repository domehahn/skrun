...
7. Run `skcr validate` and `skcr asps coverage vagrant-reviewer`.
8. Run `skcr compile vagrant-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend vagrant-reviewer`.
...
```bash
skpm validate vagrant-reviewer
skpm package vagrant-reviewer
skpm publish vagrant-reviewer
```
