...
7. Run `skcr validate` and `skcr asps coverage cncf-stackery-reviewer`.
8. Run `skcr compile cncf-stackery-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-stackery-reviewer`.
...
```bash
skpm validate cncf-stackery-reviewer
skpm package cncf-stackery-reviewer
skpm publish cncf-stackery-reviewer
```
