...
7. Run `skcr validate` and `skcr asps coverage cncf-helmwave-reviewer`.
8. Run `skcr compile cncf-helmwave-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-helmwave-reviewer`.
...
```bash
skpm validate cncf-helmwave-reviewer
skpm package cncf-helmwave-reviewer
skpm publish cncf-helmwave-reviewer
```
