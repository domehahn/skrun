...
7. Run `skcr validate` and `skcr asps coverage cncf-inlets-reviewer`.
8. Run `skcr compile cncf-inlets-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-inlets-reviewer`.
...
```bash
skpm validate cncf-inlets-reviewer
skpm package cncf-inlets-reviewer
skpm publish cncf-inlets-reviewer
```
