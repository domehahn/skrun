...
7. Run `skcr validate` and `skcr asps coverage cncf-nokv-reviewer`.
8. Run `skcr compile cncf-nokv-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nokv-reviewer`.
...
```bash
skpm validate cncf-nokv-reviewer
skpm package cncf-nokv-reviewer
skpm publish cncf-nokv-reviewer
```
