...
7. Run `skcr validate` and `skcr asps coverage cncf-tidb-reviewer`.
8. Run `skcr compile cncf-tidb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tidb-reviewer`.
...
```bash
skpm validate cncf-tidb-reviewer
skpm package cncf-tidb-reviewer
skpm publish cncf-tidb-reviewer
```
