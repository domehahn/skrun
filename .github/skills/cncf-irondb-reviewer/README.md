...
7. Run `skcr validate` and `skcr asps coverage cncf-irondb-reviewer`.
8. Run `skcr compile cncf-irondb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-irondb-reviewer`.
...
```bash
skpm validate cncf-irondb-reviewer
skpm package cncf-irondb-reviewer
skpm publish cncf-irondb-reviewer
```
