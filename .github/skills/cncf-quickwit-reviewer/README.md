...
7. Run `skcr validate` and `skcr asps coverage cncf-quickwit-reviewer`.
8. Run `skcr compile cncf-quickwit-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-quickwit-reviewer`.
...
```bash
skpm validate cncf-quickwit-reviewer
skpm package cncf-quickwit-reviewer
skpm publish cncf-quickwit-reviewer
```
