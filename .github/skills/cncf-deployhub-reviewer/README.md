...
7. Run `skcr validate` and `skcr asps coverage cncf-deployhub-reviewer`.
8. Run `skcr compile cncf-deployhub-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-deployhub-reviewer`.
...
```bash
skpm validate cncf-deployhub-reviewer
skpm package cncf-deployhub-reviewer
skpm publish cncf-deployhub-reviewer
```
