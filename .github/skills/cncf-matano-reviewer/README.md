...
7. Run `skcr validate` and `skcr asps coverage cncf-matano-reviewer`.
8. Run `skcr compile cncf-matano-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-matano-reviewer`.
...
```bash
skpm validate cncf-matano-reviewer
skpm package cncf-matano-reviewer
skpm publish cncf-matano-reviewer
```
