...
7. Run `skcr validate` and `skcr asps coverage cncf-lseg-member-reviewer`.
8. Run `skcr compile cncf-lseg-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lseg-member-reviewer`.
...
```bash
skpm validate cncf-lseg-member-reviewer
skpm package cncf-lseg-member-reviewer
skpm publish cncf-lseg-member-reviewer
```
