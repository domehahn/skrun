...
7. Run `skcr validate` and `skcr asps coverage cncf-meshcloud-member-reviewer`.
8. Run `skcr compile cncf-meshcloud-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-meshcloud-member-reviewer`.
...
```bash
skpm validate cncf-meshcloud-member-reviewer
skpm package cncf-meshcloud-member-reviewer
skpm publish cncf-meshcloud-member-reviewer
```
