...
7. Run `skcr validate` and `skcr asps coverage cncf-nipr-member-reviewer`.
8. Run `skcr compile cncf-nipr-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nipr-member-reviewer`.
...
```bash
skpm validate cncf-nipr-member-reviewer
skpm package cncf-nipr-member-reviewer
skpm publish cncf-nipr-member-reviewer
```
