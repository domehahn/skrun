...
7. Run `skcr validate` and `skcr asps coverage cncf-kics-reviewer`.
8. Run `skcr compile cncf-kics-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kics-reviewer`.
...
```bash
skpm validate cncf-kics-reviewer
skpm package cncf-kics-reviewer
skpm publish cncf-kics-reviewer
```
