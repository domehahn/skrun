...
7. Run `skcr validate` and `skcr asps coverage cncf-score-reviewer`.
8. Run `skcr compile cncf-score-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-score-reviewer`.
...
```bash
skpm validate cncf-score-reviewer
skpm package cncf-score-reviewer
skpm publish cncf-score-reviewer
```
