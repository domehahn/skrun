...
7. Run `skcr validate` and `skcr asps coverage ruby-reviewer`.
8. Run `skcr compile ruby-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend ruby-reviewer`.
...
```bash
skpm validate ruby-reviewer
skpm package ruby-reviewer
skpm publish ruby-reviewer
```
