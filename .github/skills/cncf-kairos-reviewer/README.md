...
7. Run `skcr validate` and `skcr asps coverage cncf-kairos-reviewer`.
8. Run `skcr compile cncf-kairos-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kairos-reviewer`.
...
```bash
skpm validate cncf-kairos-reviewer
skpm package cncf-kairos-reviewer
skpm publish cncf-kairos-reviewer
```
