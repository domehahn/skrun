...
7. Run `skcr validate` and `skcr asps coverage cncf-istio-reviewer`.
8. Run `skcr compile cncf-istio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-istio-reviewer`.
...
```bash
skpm validate cncf-istio-reviewer
skpm package cncf-istio-reviewer
skpm publish cncf-istio-reviewer
```
