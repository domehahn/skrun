...
7. Run `skcr validate` and `skcr asps coverage cncf-panweidb-reviewer`.
8. Run `skcr compile cncf-panweidb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-panweidb-reviewer`.
...
```bash
skpm validate cncf-panweidb-reviewer
skpm package cncf-panweidb-reviewer
skpm publish cncf-panweidb-reviewer
```
