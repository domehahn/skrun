...
7. Run `skcr validate` and `skcr asps coverage cncf-minio-reviewer`.
8. Run `skcr compile cncf-minio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-minio-reviewer`.
...
```bash
skpm validate cncf-minio-reviewer
skpm package cncf-minio-reviewer
skpm publish cncf-minio-reviewer
```
