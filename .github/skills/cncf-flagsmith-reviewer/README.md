...
7. Run `skcr validate` and `skcr asps coverage cncf-flagsmith-reviewer`.
8. Run `skcr compile cncf-flagsmith-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flagsmith-reviewer`.
...
```bash
skpm validate cncf-flagsmith-reviewer
skpm package cncf-flagsmith-reviewer
skpm publish cncf-flagsmith-reviewer
```
