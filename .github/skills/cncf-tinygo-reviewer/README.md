...
7. Run `skcr validate` and `skcr asps coverage cncf-tinygo-reviewer`.
8. Run `skcr compile cncf-tinygo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tinygo-reviewer`.
...
```bash
skpm validate cncf-tinygo-reviewer
skpm package cncf-tinygo-reviewer
skpm publish cncf-tinygo-reviewer
```
