...
7. Run `skcr validate` and `skcr asps coverage cncf-kraken-reviewer`.
8. Run `skcr compile cncf-kraken-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kraken-reviewer`.
...
```bash
skpm validate cncf-kraken-reviewer
skpm package cncf-kraken-reviewer
skpm publish cncf-kraken-reviewer
```
