...
7. Run `skcr validate` and `skcr asps coverage cncf-terraform-reviewer`.
8. Run `skcr compile cncf-terraform-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-terraform-reviewer`.
...
```bash
skpm validate cncf-terraform-reviewer
skpm package cncf-terraform-reviewer
skpm publish cncf-terraform-reviewer
```
