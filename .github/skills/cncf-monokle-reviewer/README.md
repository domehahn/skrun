...
7. Run `skcr validate` and `skcr asps coverage cncf-monokle-reviewer`.
8. Run `skcr compile cncf-monokle-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-monokle-reviewer`.
...
```bash
skpm validate cncf-monokle-reviewer
skpm package cncf-monokle-reviewer
skpm publish cncf-monokle-reviewer
```
