...
7. Run `skcr validate` and `skcr asps coverage cncf-membrane-reviewer`.
8. Run `skcr compile cncf-membrane-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-membrane-reviewer`.
...
```bash
skpm validate cncf-membrane-reviewer
skpm package cncf-membrane-reviewer
skpm publish cncf-membrane-reviewer
```
