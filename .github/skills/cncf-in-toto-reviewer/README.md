...
7. Run `skcr validate` and `skcr asps coverage cncf-in-toto-reviewer`.
8. Run `skcr compile cncf-in-toto-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-in-toto-reviewer`.
...
```bash
skpm validate cncf-in-toto-reviewer
skpm package cncf-in-toto-reviewer
skpm publish cncf-in-toto-reviewer
```
