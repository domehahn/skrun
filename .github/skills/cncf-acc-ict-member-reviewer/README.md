...
7. Run `skcr validate` and `skcr asps coverage cncf-acc-ict-member-reviewer`.
8. Run `skcr compile cncf-acc-ict-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-acc-ict-member-reviewer`.
...
```bash
skpm validate cncf-acc-ict-member-reviewer
skpm package cncf-acc-ict-member-reviewer
skpm publish cncf-acc-ict-member-reviewer
```
