...
7. Run `skcr validate` and `skcr asps coverage cncf-wavm-reviewer`.
8. Run `skcr compile cncf-wavm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wavm-reviewer`.
...
```bash
skpm validate cncf-wavm-reviewer
skpm package cncf-wavm-reviewer
skpm publish cncf-wavm-reviewer
```
