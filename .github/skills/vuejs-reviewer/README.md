...
7. Run `skcr validate` and `skcr asps coverage vuejs-reviewer`.
8. Run `skcr compile vuejs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend vuejs-reviewer`.
...
```bash
skpm validate vuejs-reviewer
skpm package vuejs-reviewer
skpm publish vuejs-reviewer
```
