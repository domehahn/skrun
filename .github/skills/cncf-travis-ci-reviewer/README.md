...
7. Run `skcr validate` and `skcr asps coverage cncf-travis-ci-reviewer`.
8. Run `skcr compile cncf-travis-ci-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-travis-ci-reviewer`.
...
```bash
skpm validate cncf-travis-ci-reviewer
skpm package cncf-travis-ci-reviewer
skpm publish cncf-travis-ci-reviewer
```
