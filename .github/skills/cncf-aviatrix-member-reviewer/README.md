...
7. Run `skcr validate` and `skcr asps coverage cncf-aviatrix-member-reviewer`.
8. Run `skcr compile cncf-aviatrix-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-aviatrix-member-reviewer`.
...
```bash
skpm validate cncf-aviatrix-member-reviewer
skpm package cncf-aviatrix-member-reviewer
skpm publish cncf-aviatrix-member-reviewer
```
