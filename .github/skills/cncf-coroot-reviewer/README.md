...
7. Run `skcr validate` and `skcr asps coverage cncf-coroot-reviewer`.
8. Run `skcr compile cncf-coroot-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-coroot-reviewer`.
...
```bash
skpm validate cncf-coroot-reviewer
skpm package cncf-coroot-reviewer
skpm publish cncf-coroot-reviewer
```
