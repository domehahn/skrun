...
7. Run `skcr validate` and `skcr asps coverage cncf-juicefs-reviewer`.
8. Run `skcr compile cncf-juicefs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-juicefs-reviewer`.
...
```bash
skpm validate cncf-juicefs-reviewer
skpm package cncf-juicefs-reviewer
skpm publish cncf-juicefs-reviewer
```
