...
7. Run `skcr validate` and `skcr asps coverage cncf-sensu-reviewer`.
8. Run `skcr compile cncf-sensu-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sensu-reviewer`.
...
```bash
skpm validate cncf-sensu-reviewer
skpm package cncf-sensu-reviewer
skpm publish cncf-sensu-reviewer
```
