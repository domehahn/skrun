...
7. Run `skcr validate` and `skcr asps coverage cncf-amnic-member-reviewer`.
8. Run `skcr compile cncf-amnic-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-amnic-member-reviewer`.
...
```bash
skpm validate cncf-amnic-member-reviewer
skpm package cncf-amnic-member-reviewer
skpm publish cncf-amnic-member-reviewer
```
