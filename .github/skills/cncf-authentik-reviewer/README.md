...
7. Run `skcr validate` and `skcr asps coverage cncf-authentik-reviewer`.
8. Run `skcr compile cncf-authentik-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-authentik-reviewer`.
...
```bash
skpm validate cncf-authentik-reviewer
skpm package cncf-authentik-reviewer
skpm publish cncf-authentik-reviewer
```
