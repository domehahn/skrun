...
7. Run `skcr validate` and `skcr asps coverage cncf-port-member-reviewer`.
8. Run `skcr compile cncf-port-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-port-member-reviewer`.
...
```bash
skpm validate cncf-port-member-reviewer
skpm package cncf-port-member-reviewer
skpm publish cncf-port-member-reviewer
```
