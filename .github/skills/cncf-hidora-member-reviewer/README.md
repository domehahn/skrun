...
7. Run `skcr validate` and `skcr asps coverage cncf-hidora-member-reviewer`.
8. Run `skcr compile cncf-hidora-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hidora-member-reviewer`.
...
```bash
skpm validate cncf-hidora-member-reviewer
skpm package cncf-hidora-member-reviewer
skpm publish cncf-hidora-member-reviewer
```
