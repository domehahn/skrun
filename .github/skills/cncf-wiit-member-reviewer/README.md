...
7. Run `skcr validate` and `skcr asps coverage cncf-wiit-member-reviewer`.
8. Run `skcr compile cncf-wiit-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wiit-member-reviewer`.
...
```bash
skpm validate cncf-wiit-member-reviewer
skpm package cncf-wiit-member-reviewer
skpm publish cncf-wiit-member-reviewer
```
