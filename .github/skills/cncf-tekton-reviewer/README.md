...
7. Run `skcr validate` and `skcr asps coverage cncf-tekton-reviewer`.
8. Run `skcr compile cncf-tekton-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tekton-reviewer`.
...
```bash
skpm validate cncf-tekton-reviewer
skpm package cncf-tekton-reviewer
skpm publish cncf-tekton-reviewer
```
