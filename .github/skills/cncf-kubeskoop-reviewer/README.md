...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeskoop-reviewer`.
8. Run `skcr compile cncf-kubeskoop-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeskoop-reviewer`.
...
```bash
skpm validate cncf-kubeskoop-reviewer
skpm package cncf-kubeskoop-reviewer
skpm publish cncf-kubeskoop-reviewer
```
