...
7. Run `skcr validate` and `skcr asps coverage cncf-kratos-reviewer`.
8. Run `skcr compile cncf-kratos-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kratos-reviewer`.
...
```bash
skpm validate cncf-kratos-reviewer
skpm package cncf-kratos-reviewer
skpm publish cncf-kratos-reviewer
```
