...
7. Run `skcr validate` and `skcr asps coverage cncf-incloud-member-reviewer`.
8. Run `skcr compile cncf-incloud-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-incloud-member-reviewer`.
...
```bash
skpm validate cncf-incloud-member-reviewer
skpm package cncf-incloud-member-reviewer
skpm publish cncf-incloud-member-reviewer
```
