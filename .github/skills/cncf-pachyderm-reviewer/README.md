...
7. Run `skcr validate` and `skcr asps coverage cncf-pachyderm-reviewer`.
8. Run `skcr compile cncf-pachyderm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pachyderm-reviewer`.
...
```bash
skpm validate cncf-pachyderm-reviewer
skpm package cncf-pachyderm-reviewer
skpm publish cncf-pachyderm-reviewer
```
