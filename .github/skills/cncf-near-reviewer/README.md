...
7. Run `skcr validate` and `skcr asps coverage cncf-near-reviewer`.
8. Run `skcr compile cncf-near-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-near-reviewer`.
...
```bash
skpm validate cncf-near-reviewer
skpm package cncf-near-reviewer
skpm publish cncf-near-reviewer
```
