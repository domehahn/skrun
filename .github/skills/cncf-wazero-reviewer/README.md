...
7. Run `skcr validate` and `skcr asps coverage cncf-wazero-reviewer`.
8. Run `skcr compile cncf-wazero-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wazero-reviewer`.
...
```bash
skpm validate cncf-wazero-reviewer
skpm package cncf-wazero-reviewer
skpm publish cncf-wazero-reviewer
```
