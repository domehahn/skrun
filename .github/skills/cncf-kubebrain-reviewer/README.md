...
7. Run `skcr validate` and `skcr asps coverage cncf-kubebrain-reviewer`.
8. Run `skcr compile cncf-kubebrain-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubebrain-reviewer`.
...
```bash
skpm validate cncf-kubebrain-reviewer
skpm package cncf-kubebrain-reviewer
skpm publish cncf-kubebrain-reviewer
```
