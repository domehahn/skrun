...
7. Run `skcr validate` and `skcr asps coverage cncf-spicedb-reviewer`.
8. Run `skcr compile cncf-spicedb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spicedb-reviewer`.
...
```bash
skpm validate cncf-spicedb-reviewer
skpm package cncf-spicedb-reviewer
skpm publish cncf-spicedb-reviewer
```
