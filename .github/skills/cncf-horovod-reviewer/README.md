...
7. Run `skcr validate` and `skcr asps coverage cncf-horovod-reviewer`.
8. Run `skcr compile cncf-horovod-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-horovod-reviewer`.
...
```bash
skpm validate cncf-horovod-reviewer
skpm package cncf-horovod-reviewer
skpm publish cncf-horovod-reviewer
```
