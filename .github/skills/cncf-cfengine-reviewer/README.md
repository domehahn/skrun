...
7. Run `skcr validate` and `skcr asps coverage cncf-cfengine-reviewer`.
8. Run `skcr compile cncf-cfengine-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cfengine-reviewer`.
...
```bash
skpm validate cncf-cfengine-reviewer
skpm package cncf-cfengine-reviewer
skpm publish cncf-cfengine-reviewer
```
