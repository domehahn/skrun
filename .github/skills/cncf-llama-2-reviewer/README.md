...
7. Run `skcr validate` and `skcr asps coverage cncf-llama-2-reviewer`.
8. Run `skcr compile cncf-llama-2-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llama-2-reviewer`.
...
```bash
skpm validate cncf-llama-2-reviewer
skpm package cncf-llama-2-reviewer
skpm publish cncf-llama-2-reviewer
```
