...
7. Run `skcr validate` and `skcr asps coverage cncf-nginx-wasm-reviewer`.
8. Run `skcr compile cncf-nginx-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nginx-wasm-reviewer`.
...
```bash
skpm validate cncf-nginx-wasm-reviewer
skpm package cncf-nginx-wasm-reviewer
skpm publish cncf-nginx-wasm-reviewer
```
