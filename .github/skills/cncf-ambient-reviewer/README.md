...
7. Run `skcr validate` and `skcr asps coverage cncf-ambient-reviewer`.
8. Run `skcr compile cncf-ambient-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ambient-reviewer`.
...
```bash
skpm validate cncf-ambient-reviewer
skpm package cncf-ambient-reviewer
skpm publish cncf-ambient-reviewer
```
