...
7. Run `skcr validate` and `skcr asps coverage cncf-knix-reviewer`.
8. Run `skcr compile cncf-knix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-knix-reviewer`.
...
```bash
skpm validate cncf-knix-reviewer
skpm package cncf-knix-reviewer
skpm publish cncf-knix-reviewer
```
