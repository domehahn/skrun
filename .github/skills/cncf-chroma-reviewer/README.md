...
7. Run `skcr validate` and `skcr asps coverage cncf-chroma-reviewer`.
8. Run `skcr compile cncf-chroma-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-chroma-reviewer`.
...
```bash
skpm validate cncf-chroma-reviewer
skpm package cncf-chroma-reviewer
skpm publish cncf-chroma-reviewer
```
