...
7. Run `skcr validate` and `skcr asps coverage cncf-bamboo-reviewer`.
8. Run `skcr compile cncf-bamboo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bamboo-reviewer`.
...
```bash
skpm validate cncf-bamboo-reviewer
skpm package cncf-bamboo-reviewer
skpm publish cncf-bamboo-reviewer
```
