...
7. Run `skcr validate` and `skcr asps coverage quarkus-reviewer`.
8. Run `skcr compile quarkus-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend quarkus-reviewer`.
...
```bash
skpm validate quarkus-reviewer
skpm package quarkus-reviewer
skpm publish quarkus-reviewer
```
