...
7. Run `skcr validate` and `skcr asps coverage cncf-gcore-member-reviewer`.
8. Run `skcr compile cncf-gcore-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gcore-member-reviewer`.
...
```bash
skpm validate cncf-gcore-member-reviewer
skpm package cncf-gcore-member-reviewer
skpm publish cncf-gcore-member-reviewer
```
