...
7. Run `skcr validate` and `skcr asps coverage cncf-eidp-member-reviewer`.
8. Run `skcr compile cncf-eidp-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-eidp-member-reviewer`.
...
```bash
skpm validate cncf-eidp-member-reviewer
skpm package cncf-eidp-member-reviewer
skpm publish cncf-eidp-member-reviewer
```
