...
7. Run `skcr validate` and `skcr asps coverage cncf-sentinel-reviewer`.
8. Run `skcr compile cncf-sentinel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sentinel-reviewer`.
...
```bash
skpm validate cncf-sentinel-reviewer
skpm package cncf-sentinel-reviewer
skpm publish cncf-sentinel-reviewer
```
