...
7. Run `skcr validate` and `skcr asps coverage cncf-clair-reviewer`.
8. Run `skcr compile cncf-clair-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-clair-reviewer`.
...
```bash
skpm validate cncf-clair-reviewer
skpm package cncf-clair-reviewer
skpm publish cncf-clair-reviewer
```
