...
7. Run `skcr validate` and `skcr asps coverage cncf-zipkin-reviewer`.
8. Run `skcr compile cncf-zipkin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zipkin-reviewer`.
...
```bash
skpm validate cncf-zipkin-reviewer
skpm package cncf-zipkin-reviewer
skpm publish cncf-zipkin-reviewer
```
