...
7. Run `skcr validate` and `skcr asps coverage cncf-kserve-reviewer`.
8. Run `skcr compile cncf-kserve-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kserve-reviewer`.
...
```bash
skpm validate cncf-kserve-reviewer
skpm package cncf-kserve-reviewer
skpm publish cncf-kserve-reviewer
```
