...
7. Run `skcr validate` and `skcr asps coverage cncf-kubero-reviewer`.
8. Run `skcr compile cncf-kubero-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubero-reviewer`.
...
```bash
skpm validate cncf-kubero-reviewer
skpm package cncf-kubero-reviewer
skpm publish cncf-kubero-reviewer
```
