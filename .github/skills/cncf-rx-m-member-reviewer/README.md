...
7. Run `skcr validate` and `skcr asps coverage cncf-rx-m-member-reviewer`.
8. Run `skcr compile cncf-rx-m-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rx-m-member-reviewer`.
...
```bash
skpm validate cncf-rx-m-member-reviewer
skpm package cncf-rx-m-member-reviewer
skpm publish cncf-rx-m-member-reviewer
```
