...
7. Run `skcr validate` and `skcr asps coverage cncf-kubefirst-reviewer`.
8. Run `skcr compile cncf-kubefirst-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubefirst-reviewer`.
...
```bash
skpm validate cncf-kubefirst-reviewer
skpm package cncf-kubefirst-reviewer
skpm publish cncf-kubefirst-reviewer
```
