...
7. Run `skcr validate` and `skcr asps coverage cncf-micrometer-reviewer`.
8. Run `skcr compile cncf-micrometer-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-micrometer-reviewer`.
...
```bash
skpm validate cncf-micrometer-reviewer
skpm package cncf-micrometer-reviewer
skpm publish cncf-micrometer-reviewer
```
