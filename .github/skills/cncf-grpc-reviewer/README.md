...
7. Run `skcr validate` and `skcr asps coverage cncf-grpc-reviewer`.
8. Run `skcr compile cncf-grpc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grpc-reviewer`.
...
```bash
skpm validate cncf-grpc-reviewer
skpm package cncf-grpc-reviewer
skpm publish cncf-grpc-reviewer
```
