...
7. Run `skcr validate` and `skcr asps coverage cncf-dubbo-reviewer`.
8. Run `skcr compile cncf-dubbo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dubbo-reviewer`.
...
```bash
skpm validate cncf-dubbo-reviewer
skpm package cncf-dubbo-reviewer
skpm publish cncf-dubbo-reviewer
```
