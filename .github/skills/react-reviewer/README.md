...
7. Run `skcr validate` and `skcr asps coverage react-reviewer`.
8. Run `skcr compile react-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend react-reviewer`.
...
```bash
skpm validate react-reviewer
skpm package react-reviewer
skpm publish react-reviewer
```
