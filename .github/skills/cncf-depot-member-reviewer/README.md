...
7. Run `skcr validate` and `skcr asps coverage cncf-depot-member-reviewer`.
8. Run `skcr compile cncf-depot-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-depot-member-reviewer`.
...
```bash
skpm validate cncf-depot-member-reviewer
skpm package cncf-depot-member-reviewer
skpm publish cncf-depot-member-reviewer
```
