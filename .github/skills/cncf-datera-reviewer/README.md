...
7. Run `skcr validate` and `skcr asps coverage cncf-datera-reviewer`.
8. Run `skcr compile cncf-datera-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-datera-reviewer`.
...
```bash
skpm validate cncf-datera-reviewer
skpm package cncf-datera-reviewer
skpm publish cncf-datera-reviewer
```
