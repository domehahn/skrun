...
7. Run `skcr validate` and `skcr asps coverage cncf-replex-reviewer`.
8. Run `skcr compile cncf-replex-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-replex-reviewer`.
...
```bash
skpm validate cncf-replex-reviewer
skpm package cncf-replex-reviewer
skpm publish cncf-replex-reviewer
```
