...
7. Run `skcr validate` and `skcr asps coverage cncf-flannel-reviewer`.
8. Run `skcr compile cncf-flannel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flannel-reviewer`.
...
```bash
skpm validate cncf-flannel-reviewer
skpm package cncf-flannel-reviewer
skpm publish cncf-flannel-reviewer
```
