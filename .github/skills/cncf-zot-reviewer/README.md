...
7. Run `skcr validate` and `skcr asps coverage cncf-zot-reviewer`.
8. Run `skcr compile cncf-zot-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zot-reviewer`.
...
```bash
skpm validate cncf-zot-reviewer
skpm package cncf-zot-reviewer
skpm publish cncf-zot-reviewer
```
