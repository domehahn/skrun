...
7. Run `skcr validate` and `skcr asps coverage cncf-kbind-reviewer`.
8. Run `skcr compile cncf-kbind-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kbind-reviewer`.
...
```bash
skpm validate cncf-kbind-reviewer
skpm package cncf-kbind-reviewer
skpm publish cncf-kbind-reviewer
```
