...
7. Run `skcr validate` and `skcr asps coverage cncf-tikv-reviewer`.
8. Run `skcr compile cncf-tikv-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tikv-reviewer`.
...
```bash
skpm validate cncf-tikv-reviewer
skpm package cncf-tikv-reviewer
skpm publish cncf-tikv-reviewer
```
