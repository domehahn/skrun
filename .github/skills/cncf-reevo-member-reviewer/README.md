...
7. Run `skcr validate` and `skcr asps coverage cncf-reevo-member-reviewer`.
8. Run `skcr compile cncf-reevo-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-reevo-member-reviewer`.
...
```bash
skpm validate cncf-reevo-member-reviewer
skpm package cncf-reevo-member-reviewer
skpm publish cncf-reevo-member-reviewer
```
