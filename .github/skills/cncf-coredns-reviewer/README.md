...
7. Run `skcr validate` and `skcr asps coverage cncf-coredns-reviewer`.
8. Run `skcr compile cncf-coredns-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-coredns-reviewer`.
...
```bash
skpm validate cncf-coredns-reviewer
skpm package cncf-coredns-reviewer
skpm publish cncf-coredns-reviewer
```
