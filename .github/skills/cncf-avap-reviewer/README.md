...
7. Run `skcr validate` and `skcr asps coverage cncf-avap-reviewer`.
8. Run `skcr compile cncf-avap-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-avap-reviewer`.
...
```bash
skpm validate cncf-avap-reviewer
skpm package cncf-avap-reviewer
skpm publish cncf-avap-reviewer
```
