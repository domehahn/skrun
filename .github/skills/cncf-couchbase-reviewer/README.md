...
7. Run `skcr validate` and `skcr asps coverage cncf-couchbase-reviewer`.
8. Run `skcr compile cncf-couchbase-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-couchbase-reviewer`.
...
```bash
skpm validate cncf-couchbase-reviewer
skpm package cncf-couchbase-reviewer
skpm publish cncf-couchbase-reviewer
```
