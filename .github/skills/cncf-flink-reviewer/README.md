...
7. Run `skcr validate` and `skcr asps coverage cncf-flink-reviewer`.
8. Run `skcr compile cncf-flink-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flink-reviewer`.
...
```bash
skpm validate cncf-flink-reviewer
skpm package cncf-flink-reviewer
skpm publish cncf-flink-reviewer
```
