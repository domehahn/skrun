...
7. Run `skcr validate` and `skcr asps coverage cncf-dachs-it-member-reviewer`.
8. Run `skcr compile cncf-dachs-it-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dachs-it-member-reviewer`.
...
```bash
skpm validate cncf-dachs-it-member-reviewer
skpm package cncf-dachs-it-member-reviewer
skpm publish cncf-dachs-it-member-reviewer
```
