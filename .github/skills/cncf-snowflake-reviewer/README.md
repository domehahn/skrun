...
7. Run `skcr validate` and `skcr asps coverage cncf-snowflake-reviewer`.
8. Run `skcr compile cncf-snowflake-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-snowflake-reviewer`.
...
```bash
skpm validate cncf-snowflake-reviewer
skpm package cncf-snowflake-reviewer
skpm publish cncf-snowflake-reviewer
```
