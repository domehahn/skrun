...
7. Run `skcr validate` and `skcr asps coverage cncf-chaterm-reviewer`.
8. Run `skcr compile cncf-chaterm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-chaterm-reviewer`.
...
```bash
skpm validate cncf-chaterm-reviewer
skpm package cncf-chaterm-reviewer
skpm publish cncf-chaterm-reviewer
```
