...
7. Run `skcr validate` and `skcr asps coverage cncf-libsql-reviewer`.
8. Run `skcr compile cncf-libsql-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-libsql-reviewer`.
...
```bash
skpm validate cncf-libsql-reviewer
skpm package cncf-libsql-reviewer
skpm publish cncf-libsql-reviewer
```
