...
7. Run `skcr validate` and `skcr asps coverage cncf-slime-reviewer`.
8. Run `skcr compile cncf-slime-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-slime-reviewer`.
...
```bash
skpm validate cncf-slime-reviewer
skpm package cncf-slime-reviewer
skpm publish cncf-slime-reviewer
```
