...
7. Run `skcr validate` and `skcr asps coverage cncf-scylla-reviewer`.
8. Run `skcr compile cncf-scylla-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scylla-reviewer`.
...
```bash
skpm validate cncf-scylla-reviewer
skpm package cncf-scylla-reviewer
skpm publish cncf-scylla-reviewer
```
