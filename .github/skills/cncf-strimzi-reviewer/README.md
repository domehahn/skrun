...
7. Run `skcr validate` and `skcr asps coverage cncf-strimzi-reviewer`.
8. Run `skcr compile cncf-strimzi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-strimzi-reviewer`.
...
```bash
skpm validate cncf-strimzi-reviewer
skpm package cncf-strimzi-reviewer
skpm publish cncf-strimzi-reviewer
```
