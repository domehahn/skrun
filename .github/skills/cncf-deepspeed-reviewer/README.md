...
7. Run `skcr validate` and `skcr asps coverage cncf-deepspeed-reviewer`.
8. Run `skcr compile cncf-deepspeed-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-deepspeed-reviewer`.
...
```bash
skpm validate cncf-deepspeed-reviewer
skpm package cncf-deepspeed-reviewer
skpm publish cncf-deepspeed-reviewer
```
