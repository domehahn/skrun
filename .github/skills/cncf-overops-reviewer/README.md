...
7. Run `skcr validate` and `skcr asps coverage cncf-overops-reviewer`.
8. Run `skcr compile cncf-overops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-overops-reviewer`.
...
```bash
skpm validate cncf-overops-reviewer
skpm package cncf-overops-reviewer
skpm publish cncf-overops-reviewer
```
