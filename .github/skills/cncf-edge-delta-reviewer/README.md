...
7. Run `skcr validate` and `skcr asps coverage cncf-edge-delta-reviewer`.
8. Run `skcr compile cncf-edge-delta-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-edge-delta-reviewer`.
...
```bash
skpm validate cncf-edge-delta-reviewer
skpm package cncf-edge-delta-reviewer
skpm publish cncf-edge-delta-reviewer
```
