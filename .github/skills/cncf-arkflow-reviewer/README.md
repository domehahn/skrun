...
7. Run `skcr validate` and `skcr asps coverage cncf-arkflow-reviewer`.
8. Run `skcr compile cncf-arkflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-arkflow-reviewer`.
...
```bash
skpm validate cncf-arkflow-reviewer
skpm package cncf-arkflow-reviewer
skpm publish cncf-arkflow-reviewer
```
