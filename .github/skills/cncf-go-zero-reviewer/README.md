...
7. Run `skcr validate` and `skcr asps coverage cncf-go-zero-reviewer`.
8. Run `skcr compile cncf-go-zero-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-go-zero-reviewer`.
...
```bash
skpm validate cncf-go-zero-reviewer
skpm package cncf-go-zero-reviewer
skpm publish cncf-go-zero-reviewer
```
