...
7. Run `skcr validate` and `skcr asps coverage cncf-infracost-reviewer`.
8. Run `skcr compile cncf-infracost-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-infracost-reviewer`.
...
```bash
skpm validate cncf-infracost-reviewer
skpm package cncf-infracost-reviewer
skpm publish cncf-infracost-reviewer
```
