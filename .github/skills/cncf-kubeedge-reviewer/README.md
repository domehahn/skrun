...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeedge-reviewer`.
8. Run `skcr compile cncf-kubeedge-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeedge-reviewer`.
...
```bash
skpm validate cncf-kubeedge-reviewer
skpm package cncf-kubeedge-reviewer
skpm publish cncf-kubeedge-reviewer
```
