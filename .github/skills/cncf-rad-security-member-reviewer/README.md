...
7. Run `skcr validate` and `skcr asps coverage cncf-rad-security-member-reviewer`.
8. Run `skcr compile cncf-rad-security-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rad-security-member-reviewer`.
...
```bash
skpm validate cncf-rad-security-member-reviewer
skpm package cncf-rad-security-member-reviewer
skpm publish cncf-rad-security-member-reviewer
```
