...
7. Run `skcr validate` and `skcr asps coverage cncf-kubecost-reviewer`.
8. Run `skcr compile cncf-kubecost-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubecost-reviewer`.
...
```bash
skpm validate cncf-kubecost-reviewer
skpm package cncf-kubecost-reviewer
skpm publish cncf-kubecost-reviewer
```
