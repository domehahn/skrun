...
7. Run `skcr validate` and `skcr asps coverage cncf-cosmwasm-reviewer`.
8. Run `skcr compile cncf-cosmwasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cosmwasm-reviewer`.
...
```bash
skpm validate cncf-cosmwasm-reviewer
skpm package cncf-cosmwasm-reviewer
skpm publish cncf-cosmwasm-reviewer
```
