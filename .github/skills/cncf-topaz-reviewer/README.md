...
7. Run `skcr validate` and `skcr asps coverage cncf-topaz-reviewer`.
8. Run `skcr compile cncf-topaz-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-topaz-reviewer`.
...
```bash
skpm validate cncf-topaz-reviewer
skpm package cncf-topaz-reviewer
skpm publish cncf-topaz-reviewer
```
