...
7. Run `skcr validate` and `skcr asps coverage cncf-coredge-member-reviewer`.
8. Run `skcr compile cncf-coredge-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-coredge-member-reviewer`.
...
```bash
skpm validate cncf-coredge-member-reviewer
skpm package cncf-coredge-member-reviewer
skpm publish cncf-coredge-member-reviewer
```
