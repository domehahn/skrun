...
7. Run `skcr validate` and `skcr asps coverage cncf-hedgehog-member-reviewer`.
8. Run `skcr compile cncf-hedgehog-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hedgehog-member-reviewer`.
...
```bash
skpm validate cncf-hedgehog-member-reviewer
skpm package cncf-hedgehog-member-reviewer
skpm publish cncf-hedgehog-member-reviewer
```
