...
7. Run `skcr validate` and `skcr asps coverage cncf-curve-reviewer`.
8. Run `skcr compile cncf-curve-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-curve-reviewer`.
...
```bash
skpm validate cncf-curve-reviewer
skpm package cncf-curve-reviewer
skpm publish cncf-curve-reviewer
```
