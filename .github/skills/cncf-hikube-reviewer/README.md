...
7. Run `skcr validate` and `skcr asps coverage cncf-hikube-reviewer`.
8. Run `skcr compile cncf-hikube-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hikube-reviewer`.
...
```bash
skpm validate cncf-hikube-reviewer
skpm package cncf-hikube-reviewer
skpm publish cncf-hikube-reviewer
```
