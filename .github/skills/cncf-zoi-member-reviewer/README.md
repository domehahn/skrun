...
7. Run `skcr validate` and `skcr asps coverage cncf-zoi-member-reviewer`.
8. Run `skcr compile cncf-zoi-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zoi-member-reviewer`.
...
```bash
skpm validate cncf-zoi-member-reviewer
skpm package cncf-zoi-member-reviewer
skpm publish cncf-zoi-member-reviewer
```
