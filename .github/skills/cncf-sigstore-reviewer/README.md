...
7. Run `skcr validate` and `skcr asps coverage cncf-sigstore-reviewer`.
8. Run `skcr compile cncf-sigstore-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sigstore-reviewer`.
...
```bash
skpm validate cncf-sigstore-reviewer
skpm package cncf-sigstore-reviewer
skpm publish cncf-sigstore-reviewer
```
