...
7. Run `skcr validate` and `skcr asps coverage cncf-tyk-reviewer`.
8. Run `skcr compile cncf-tyk-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tyk-reviewer`.
...
```bash
skpm validate cncf-tyk-reviewer
skpm package cncf-tyk-reviewer
skpm publish cncf-tyk-reviewer
```
