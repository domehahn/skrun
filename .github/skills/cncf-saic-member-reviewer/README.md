...
7. Run `skcr validate` and `skcr asps coverage cncf-saic-member-reviewer`.
8. Run `skcr compile cncf-saic-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-saic-member-reviewer`.
...
```bash
skpm validate cncf-saic-member-reviewer
skpm package cncf-saic-member-reviewer
skpm publish cncf-saic-member-reviewer
```
