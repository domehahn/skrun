...
7. Run `skcr validate` and `skcr asps coverage cncf-harbor-wasm-reviewer`.
8. Run `skcr compile cncf-harbor-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-harbor-wasm-reviewer`.
...
```bash
skpm validate cncf-harbor-wasm-reviewer
skpm package cncf-harbor-wasm-reviewer
skpm publish cncf-harbor-wasm-reviewer
```
