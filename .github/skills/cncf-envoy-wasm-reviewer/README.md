...
7. Run `skcr validate` and `skcr asps coverage cncf-envoy-wasm-reviewer`.
8. Run `skcr compile cncf-envoy-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-envoy-wasm-reviewer`.
...
```bash
skpm validate cncf-envoy-wasm-reviewer
skpm package cncf-envoy-wasm-reviewer
skpm publish cncf-envoy-wasm-reviewer
```
