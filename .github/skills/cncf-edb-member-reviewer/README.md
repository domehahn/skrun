...
7. Run `skcr validate` and `skcr asps coverage cncf-edb-member-reviewer`.
8. Run `skcr compile cncf-edb-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-edb-member-reviewer`.
...
```bash
skpm validate cncf-edb-member-reviewer
skpm package cncf-edb-member-reviewer
skpm publish cncf-edb-member-reviewer
```
