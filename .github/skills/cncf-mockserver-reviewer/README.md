...
7. Run `skcr validate` and `skcr asps coverage cncf-mockserver-reviewer`.
8. Run `skcr compile cncf-mockserver-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mockserver-reviewer`.
...
```bash
skpm validate cncf-mockserver-reviewer
skpm package cncf-mockserver-reviewer
skpm publish cncf-mockserver-reviewer
```
