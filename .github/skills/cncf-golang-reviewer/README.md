...
7. Run `skcr validate` and `skcr asps coverage cncf-golang-reviewer`.
8. Run `skcr compile cncf-golang-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-golang-reviewer`.
...
```bash
skpm validate cncf-golang-reviewer
skpm package cncf-golang-reviewer
skpm publish cncf-golang-reviewer
```
