...
7. Run `skcr validate` and `skcr asps coverage cncf-cleanstart-member-reviewer`.
8. Run `skcr compile cncf-cleanstart-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cleanstart-member-reviewer`.
...
```bash
skpm validate cncf-cleanstart-member-reviewer
skpm package cncf-cleanstart-member-reviewer
skpm publish cncf-cleanstart-member-reviewer
```
