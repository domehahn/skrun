...
7. Run `skcr validate` and `skcr asps coverage cncf-cassandra-reviewer`.
8. Run `skcr compile cncf-cassandra-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cassandra-reviewer`.
...
```bash
skpm validate cncf-cassandra-reviewer
skpm package cncf-cassandra-reviewer
skpm publish cncf-cassandra-reviewer
```
