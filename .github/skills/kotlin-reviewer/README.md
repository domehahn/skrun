...
7. Run `skcr validate` and `skcr asps coverage kotlin-reviewer`.
8. Run `skcr compile kotlin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend kotlin-reviewer`.
...
```bash
skpm validate kotlin-reviewer
skpm package kotlin-reviewer
skpm publish kotlin-reviewer
```
