...
7. Run `skcr validate` and `skcr asps coverage cncf-nginx-member-reviewer`.
8. Run `skcr compile cncf-nginx-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nginx-member-reviewer`.
...
```bash
skpm validate cncf-nginx-member-reviewer
skpm package cncf-nginx-member-reviewer
skpm publish cncf-nginx-member-reviewer
```
