...
7. Run `skcr validate` and `skcr asps coverage cncf-haproxy-reviewer`.
8. Run `skcr compile cncf-haproxy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-haproxy-reviewer`.
...
```bash
skpm validate cncf-haproxy-reviewer
skpm package cncf-haproxy-reviewer
skpm publish cncf-haproxy-reviewer
```
