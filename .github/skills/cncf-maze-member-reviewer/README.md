...
7. Run `skcr validate` and `skcr asps coverage cncf-maze-member-reviewer`.
8. Run `skcr compile cncf-maze-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-maze-member-reviewer`.
...
```bash
skpm validate cncf-maze-member-reviewer
skpm package cncf-maze-member-reviewer
skpm publish cncf-maze-member-reviewer
```
