...
7. Run `skcr validate` and `skcr asps coverage cncf-bosh-reviewer`.
8. Run `skcr compile cncf-bosh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bosh-reviewer`.
...
```bash
skpm validate cncf-bosh-reviewer
skpm package cncf-bosh-reviewer
skpm publish cncf-bosh-reviewer
```
