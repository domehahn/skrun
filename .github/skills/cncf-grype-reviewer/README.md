...
7. Run `skcr validate` and `skcr asps coverage cncf-grype-reviewer`.
8. Run `skcr compile cncf-grype-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grype-reviewer`.
...
```bash
skpm validate cncf-grype-reviewer
skpm package cncf-grype-reviewer
skpm publish cncf-grype-reviewer
```
