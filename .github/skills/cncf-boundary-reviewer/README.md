...
7. Run `skcr validate` and `skcr asps coverage cncf-boundary-reviewer`.
8. Run `skcr compile cncf-boundary-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-boundary-reviewer`.
...
```bash
skpm validate cncf-boundary-reviewer
skpm package cncf-boundary-reviewer
skpm publish cncf-boundary-reviewer
```
