...
7. Run `skcr validate` and `skcr asps coverage cncf-shturval-reviewer`.
8. Run `skcr compile cncf-shturval-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-shturval-reviewer`.
...
```bash
skpm validate cncf-shturval-reviewer
skpm package cncf-shturval-reviewer
skpm publish cncf-shturval-reviewer
```
