...
7. Run `skcr validate` and `skcr asps coverage cncf-guida-member-reviewer`.
8. Run `skcr compile cncf-guida-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-guida-member-reviewer`.
...
```bash
skpm validate cncf-guida-member-reviewer
skpm package cncf-guida-member-reviewer
skpm publish cncf-guida-member-reviewer
```
