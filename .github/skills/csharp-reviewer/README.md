...
7. Run `skcr validate` and `skcr asps coverage csharp-reviewer`.
8. Run `skcr compile csharp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend csharp-reviewer`.
...
```bash
skpm validate csharp-reviewer
skpm package csharp-reviewer
skpm publish csharp-reviewer
```
