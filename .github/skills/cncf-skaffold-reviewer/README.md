...
7. Run `skcr validate` and `skcr asps coverage cncf-skaffold-reviewer`.
8. Run `skcr compile cncf-skaffold-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-skaffold-reviewer`.
...
```bash
skpm validate cncf-skaffold-reviewer
skpm package cncf-skaffold-reviewer
skpm publish cncf-skaffold-reviewer
```
