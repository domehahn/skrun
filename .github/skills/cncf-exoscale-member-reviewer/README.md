...
7. Run `skcr validate` and `skcr asps coverage cncf-exoscale-member-reviewer`.
8. Run `skcr compile cncf-exoscale-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-exoscale-member-reviewer`.
...
```bash
skpm validate cncf-exoscale-member-reviewer
skpm package cncf-exoscale-member-reviewer
skpm publish cncf-exoscale-member-reviewer
```
