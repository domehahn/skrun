...
7. Run `skcr validate` and `skcr asps coverage cncf-3-shake-member-reviewer`.
8. Run `skcr compile cncf-3-shake-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-3-shake-member-reviewer`.
...
```bash
skpm validate cncf-3-shake-member-reviewer
skpm package cncf-3-shake-member-reviewer
skpm publish cncf-3-shake-member-reviewer
```
