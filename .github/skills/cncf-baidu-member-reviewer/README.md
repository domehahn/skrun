...
7. Run `skcr validate` and `skcr asps coverage cncf-baidu-member-reviewer`.
8. Run `skcr compile cncf-baidu-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-baidu-member-reviewer`.
...
```bash
skpm validate cncf-baidu-member-reviewer
skpm package cncf-baidu-member-reviewer
skpm publish cncf-baidu-member-reviewer
```
