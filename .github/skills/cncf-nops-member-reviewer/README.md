...
7. Run `skcr validate` and `skcr asps coverage cncf-nops-member-reviewer`.
8. Run `skcr compile cncf-nops-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nops-member-reviewer`.
...
```bash
skpm validate cncf-nops-member-reviewer
skpm package cncf-nops-member-reviewer
skpm publish cncf-nops-member-reviewer
```
