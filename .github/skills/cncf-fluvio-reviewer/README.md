...
7. Run `skcr validate` and `skcr asps coverage cncf-fluvio-reviewer`.
8. Run `skcr compile cncf-fluvio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fluvio-reviewer`.
...
```bash
skpm validate cncf-fluvio-reviewer
skpm package cncf-fluvio-reviewer
skpm publish cncf-fluvio-reviewer
```
