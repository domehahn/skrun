...
7. Run `skcr validate` and `skcr asps coverage cncf-hyperopt-reviewer`.
8. Run `skcr compile cncf-hyperopt-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hyperopt-reviewer`.
...
```bash
skpm validate cncf-hyperopt-reviewer
skpm package cncf-hyperopt-reviewer
skpm publish cncf-hyperopt-reviewer
```
