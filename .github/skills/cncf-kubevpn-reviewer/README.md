...
7. Run `skcr validate` and `skcr asps coverage cncf-kubevpn-reviewer`.
8. Run `skcr compile cncf-kubevpn-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubevpn-reviewer`.
...
```bash
skpm validate cncf-kubevpn-reviewer
skpm package cncf-kubevpn-reviewer
skpm publish cncf-kubevpn-reviewer
```
