...
7. Run `skcr validate` and `skcr asps coverage cncf-loggly-reviewer`.
8. Run `skcr compile cncf-loggly-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-loggly-reviewer`.
...
```bash
skpm validate cncf-loggly-reviewer
skpm package cncf-loggly-reviewer
skpm publish cncf-loggly-reviewer
```
