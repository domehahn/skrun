...
7. Run `skcr validate` and `skcr asps coverage cncf-mogdb-reviewer`.
8. Run `skcr compile cncf-mogdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mogdb-reviewer`.
...
```bash
skpm validate cncf-mogdb-reviewer
skpm package cncf-mogdb-reviewer
skpm publish cncf-mogdb-reviewer
```
