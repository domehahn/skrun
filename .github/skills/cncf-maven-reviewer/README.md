...
7. Run `skcr validate` and `skcr asps coverage cncf-maven-reviewer`.
8. Run `skcr compile cncf-maven-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-maven-reviewer`.
...
```bash
skpm validate cncf-maven-reviewer
skpm package cncf-maven-reviewer
skpm publish cncf-maven-reviewer
```
