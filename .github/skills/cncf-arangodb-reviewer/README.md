...
7. Run `skcr validate` and `skcr asps coverage cncf-arangodb-reviewer`.
8. Run `skcr compile cncf-arangodb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-arangodb-reviewer`.
...
```bash
skpm validate cncf-arangodb-reviewer
skpm package cncf-arangodb-reviewer
skpm publish cncf-arangodb-reviewer
```
