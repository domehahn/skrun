...
7. Run `skcr validate` and `skcr asps coverage cncf-ionos-member-reviewer`.
8. Run `skcr compile cncf-ionos-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ionos-member-reviewer`.
...
```bash
skpm validate cncf-ionos-member-reviewer
skpm package cncf-ionos-member-reviewer
skpm publish cncf-ionos-member-reviewer
```
