...
7. Run `skcr validate` and `skcr asps coverage cncf-kubara-reviewer`.
8. Run `skcr compile cncf-kubara-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubara-reviewer`.
...
```bash
skpm validate cncf-kubara-reviewer
skpm package cncf-kubara-reviewer
skpm publish cncf-kubara-reviewer
```
