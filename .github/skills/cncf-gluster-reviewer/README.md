...
7. Run `skcr validate` and `skcr asps coverage cncf-gluster-reviewer`.
8. Run `skcr compile cncf-gluster-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gluster-reviewer`.
...
```bash
skpm validate cncf-gluster-reviewer
skpm package cncf-gluster-reviewer
skpm publish cncf-gluster-reviewer
```
