...
7. Run `skcr validate` and `skcr asps coverage cncf-kurl-reviewer`.
8. Run `skcr compile cncf-kurl-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kurl-reviewer`.
...
```bash
skpm validate cncf-kurl-reviewer
skpm package cncf-kurl-reviewer
skpm publish cncf-kurl-reviewer
```
