...
7. Run `skcr validate` and `skcr asps coverage cncf-hpe-storage-reviewer`.
8. Run `skcr compile cncf-hpe-storage-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hpe-storage-reviewer`.
...
```bash
skpm validate cncf-hpe-storage-reviewer
skpm package cncf-hpe-storage-reviewer
skpm publish cncf-hpe-storage-reviewer
```
