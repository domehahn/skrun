...
7. Run `skcr validate` and `skcr asps coverage cncf-vault-reviewer`.
8. Run `skcr compile cncf-vault-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vault-reviewer`.
...
```bash
skpm validate cncf-vault-reviewer
skpm package cncf-vault-reviewer
skpm publish cncf-vault-reviewer
```
