...
7. Run `skcr validate` and `skcr asps coverage cncf-keycloak-reviewer`.
8. Run `skcr compile cncf-keycloak-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-keycloak-reviewer`.
...
```bash
skpm validate cncf-keycloak-reviewer
skpm package cncf-keycloak-reviewer
skpm publish cncf-keycloak-reviewer
```
