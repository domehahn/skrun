...
7. Run `skcr validate` and `skcr asps coverage cncf-wasmer-reviewer`.
8. Run `skcr compile cncf-wasmer-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasmer-reviewer`.
...
```bash
skpm validate cncf-wasmer-reviewer
skpm package cncf-wasmer-reviewer
skpm publish cncf-wasmer-reviewer
```
