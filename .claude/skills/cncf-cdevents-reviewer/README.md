...
7. Run `skcr validate` and `skcr asps coverage cncf-cdevents-reviewer`.
8. Run `skcr compile cncf-cdevents-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cdevents-reviewer`.
...
```bash
skpm validate cncf-cdevents-reviewer
skpm package cncf-cdevents-reviewer
skpm publish cncf-cdevents-reviewer
```
