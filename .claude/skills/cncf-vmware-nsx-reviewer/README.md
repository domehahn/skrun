...
7. Run `skcr validate` and `skcr asps coverage cncf-vmware-nsx-reviewer`.
8. Run `skcr compile cncf-vmware-nsx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vmware-nsx-reviewer`.
...
```bash
skpm validate cncf-vmware-nsx-reviewer
skpm package cncf-vmware-nsx-reviewer
skpm publish cncf-vmware-nsx-reviewer
```
