...
7. Run `skcr validate` and `skcr asps coverage cncf-iomesh-reviewer`.
8. Run `skcr compile cncf-iomesh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-iomesh-reviewer`.
...
```bash
skpm validate cncf-iomesh-reviewer
skpm package cncf-iomesh-reviewer
skpm publish cncf-iomesh-reviewer
```
