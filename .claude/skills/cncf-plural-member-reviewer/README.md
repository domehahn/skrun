...
7. Run `skcr validate` and `skcr asps coverage cncf-plural-member-reviewer`.
8. Run `skcr compile cncf-plural-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-plural-member-reviewer`.
...
```bash
skpm validate cncf-plural-member-reviewer
skpm package cncf-plural-member-reviewer
skpm publish cncf-plural-member-reviewer
```
