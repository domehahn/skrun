...
7. Run `skcr validate` and `skcr asps coverage opentofu-reviewer`.
8. Run `skcr compile opentofu-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend opentofu-reviewer`.
...
```bash
skpm validate opentofu-reviewer
skpm package opentofu-reviewer
skpm publish opentofu-reviewer
```
