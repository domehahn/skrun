...
7. Run `skcr validate` and `skcr asps coverage cncf-carvel-reviewer`.
8. Run `skcr compile cncf-carvel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-carvel-reviewer`.
...
```bash
skpm validate cncf-carvel-reviewer
skpm package cncf-carvel-reviewer
skpm publish cncf-carvel-reviewer
```
