...
7. Run `skcr validate` and `skcr asps coverage cncf-appneta-reviewer`.
8. Run `skcr compile cncf-appneta-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-appneta-reviewer`.
...
```bash
skpm validate cncf-appneta-reviewer
skpm package cncf-appneta-reviewer
skpm publish cncf-appneta-reviewer
```
