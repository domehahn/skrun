...
7. Run `skcr validate` and `skcr asps coverage cncf-pure-storage-reviewer`.
8. Run `skcr compile cncf-pure-storage-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pure-storage-reviewer`.
...
```bash
skpm validate cncf-pure-storage-reviewer
skpm package cncf-pure-storage-reviewer
skpm publish cncf-pure-storage-reviewer
```
