...
7. Run `skcr validate` and `skcr asps coverage cncf-vcluster-member-reviewer`.
8. Run `skcr compile cncf-vcluster-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vcluster-member-reviewer`.
...
```bash
skpm validate cncf-vcluster-member-reviewer
skpm package cncf-vcluster-member-reviewer
skpm publish cncf-vcluster-member-reviewer
```
