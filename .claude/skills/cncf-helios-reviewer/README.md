...
7. Run `skcr validate` and `skcr asps coverage cncf-helios-reviewer`.
8. Run `skcr compile cncf-helios-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-helios-reviewer`.
...
```bash
skpm validate cncf-helios-reviewer
skpm package cncf-helios-reviewer
skpm publish cncf-helios-reviewer
```
