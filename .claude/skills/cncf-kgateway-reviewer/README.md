...
7. Run `skcr validate` and `skcr asps coverage cncf-kgateway-reviewer`.
8. Run `skcr compile cncf-kgateway-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kgateway-reviewer`.
...
```bash
skpm validate cncf-kgateway-reviewer
skpm package cncf-kgateway-reviewer
skpm publish cncf-kgateway-reviewer
```
