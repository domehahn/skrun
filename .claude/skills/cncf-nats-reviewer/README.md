...
7. Run `skcr validate` and `skcr asps coverage cncf-nats-reviewer`.
8. Run `skcr compile cncf-nats-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nats-reviewer`.
...
```bash
skpm validate cncf-nats-reviewer
skpm package cncf-nats-reviewer
skpm publish cncf-nats-reviewer
```
