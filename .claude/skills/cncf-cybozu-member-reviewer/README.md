...
7. Run `skcr validate` and `skcr asps coverage cncf-cybozu-member-reviewer`.
8. Run `skcr compile cncf-cybozu-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cybozu-member-reviewer`.
...
```bash
skpm validate cncf-cybozu-member-reviewer
skpm package cncf-cybozu-member-reviewer
skpm publish cncf-cybozu-member-reviewer
```
