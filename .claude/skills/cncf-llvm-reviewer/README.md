...
7. Run `skcr validate` and `skcr asps coverage cncf-llvm-reviewer`.
8. Run `skcr compile cncf-llvm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llvm-reviewer`.
...
```bash
skpm validate cncf-llvm-reviewer
skpm package cncf-llvm-reviewer
skpm publish cncf-llvm-reviewer
```
