...
7. Run `skcr validate` and `skcr asps coverage cncf-radar-reviewer`.
8. Run `skcr compile cncf-radar-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-radar-reviewer`.
...
```bash
skpm validate cncf-radar-reviewer
skpm package cncf-radar-reviewer
skpm publish cncf-radar-reviewer
```
