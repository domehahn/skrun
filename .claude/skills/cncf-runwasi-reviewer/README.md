...
7. Run `skcr validate` and `skcr asps coverage cncf-runwasi-reviewer`.
8. Run `skcr compile cncf-runwasi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-runwasi-reviewer`.
...
```bash
skpm validate cncf-runwasi-reviewer
skpm package cncf-runwasi-reviewer
skpm publish cncf-runwasi-reviewer
```
