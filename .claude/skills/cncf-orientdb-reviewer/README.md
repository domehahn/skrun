...
7. Run `skcr validate` and `skcr asps coverage cncf-orientdb-reviewer`.
8. Run `skcr compile cncf-orientdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-orientdb-reviewer`.
...
```bash
skpm validate cncf-orientdb-reviewer
skpm package cncf-orientdb-reviewer
skpm publish cncf-orientdb-reviewer
```
