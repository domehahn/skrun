...
7. Run `skcr validate` and `skcr asps coverage cncf-spegel-reviewer`.
8. Run `skcr compile cncf-spegel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spegel-reviewer`.
...
```bash
skpm validate cncf-spegel-reviewer
skpm package cncf-spegel-reviewer
skpm publish cncf-spegel-reviewer
```
