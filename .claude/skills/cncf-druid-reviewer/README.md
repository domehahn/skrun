...
7. Run `skcr validate` and `skcr asps coverage cncf-druid-reviewer`.
8. Run `skcr compile cncf-druid-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-druid-reviewer`.
...
```bash
skpm validate cncf-druid-reviewer
skpm package cncf-druid-reviewer
skpm publish cncf-druid-reviewer
```
