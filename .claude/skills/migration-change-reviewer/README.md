...
7. Run `skcr validate` and `skcr asps coverage migration-change-reviewer`.
8. Run `skcr compile migration-change-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend migration-change-reviewer`.
...
```bash
skpm validate migration-change-reviewer
skpm package migration-change-reviewer
skpm publish migration-change-reviewer
```
