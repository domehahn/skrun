...
7. Run `skcr validate` and `skcr asps coverage cncf-rx-m-kcsp-reviewer`.
8. Run `skcr compile cncf-rx-m-kcsp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rx-m-kcsp-reviewer`.
...
```bash
skpm validate cncf-rx-m-kcsp-reviewer
skpm package cncf-rx-m-kcsp-reviewer
skpm publish cncf-rx-m-kcsp-reviewer
```
