...
7. Run `skcr validate` and `skcr asps coverage cncf-crate-io-reviewer`.
8. Run `skcr compile cncf-crate-io-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-crate-io-reviewer`.
...
```bash
skpm validate cncf-crate-io-reviewer
skpm package cncf-crate-io-reviewer
skpm publish cncf-crate-io-reviewer
```
