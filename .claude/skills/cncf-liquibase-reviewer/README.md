...
7. Run `skcr validate` and `skcr asps coverage cncf-liquibase-reviewer`.
8. Run `skcr compile cncf-liquibase-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-liquibase-reviewer`.
...
```bash
skpm validate cncf-liquibase-reviewer
skpm package cncf-liquibase-reviewer
skpm publish cncf-liquibase-reviewer
```
