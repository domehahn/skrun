...
7. Run `skcr validate` and `skcr asps coverage cncf-rtx-member-reviewer`.
8. Run `skcr compile cncf-rtx-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rtx-member-reviewer`.
...
```bash
skpm validate cncf-rtx-member-reviewer
skpm package cncf-rtx-member-reviewer
skpm publish cncf-rtx-member-reviewer
```
