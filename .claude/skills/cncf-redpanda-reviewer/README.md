...
7. Run `skcr validate` and `skcr asps coverage cncf-redpanda-reviewer`.
8. Run `skcr compile cncf-redpanda-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-redpanda-reviewer`.
...
```bash
skpm validate cncf-redpanda-reviewer
skpm package cncf-redpanda-reviewer
skpm publish cncf-redpanda-reviewer
```
