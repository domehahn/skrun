...
7. Run `skcr validate` and `skcr asps coverage cncf-exein-member-reviewer`.
8. Run `skcr compile cncf-exein-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-exein-member-reviewer`.
...
```bash
skpm validate cncf-exein-member-reviewer
skpm package cncf-exein-member-reviewer
skpm publish cncf-exein-member-reviewer
```
