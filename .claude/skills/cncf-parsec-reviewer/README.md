...
7. Run `skcr validate` and `skcr asps coverage cncf-parsec-reviewer`.
8. Run `skcr compile cncf-parsec-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-parsec-reviewer`.
...
```bash
skpm validate cncf-parsec-reviewer
skpm package cncf-parsec-reviewer
skpm publish cncf-parsec-reviewer
```
