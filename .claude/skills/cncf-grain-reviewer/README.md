...
7. Run `skcr validate` and `skcr asps coverage cncf-grain-reviewer`.
8. Run `skcr compile cncf-grain-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grain-reviewer`.
...
```bash
skpm validate cncf-grain-reviewer
skpm package cncf-grain-reviewer
skpm publish cncf-grain-reviewer
```
