...
7. Run `skcr validate` and `skcr asps coverage cncf-scalingo-reviewer`.
8. Run `skcr compile cncf-scalingo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scalingo-reviewer`.
...
```bash
skpm validate cncf-scalingo-reviewer
skpm package cncf-scalingo-reviewer
skpm publish cncf-scalingo-reviewer
```
