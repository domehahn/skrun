...
7. Run `skcr validate` and `skcr asps coverage cncf-deepflow-reviewer`.
8. Run `skcr compile cncf-deepflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-deepflow-reviewer`.
...
```bash
skpm validate cncf-deepflow-reviewer
skpm package cncf-deepflow-reviewer
skpm publish cncf-deepflow-reviewer
```
