...
7. Run `skcr validate` and `skcr asps coverage cncf-azure-registry-reviewer`.
8. Run `skcr compile cncf-azure-registry-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-azure-registry-reviewer`.
...
```bash
skpm validate cncf-azure-registry-reviewer
skpm package cncf-azure-registry-reviewer
skpm publish cncf-azure-registry-reviewer
```
