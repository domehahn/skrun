...
7. Run `skcr validate` and `skcr asps coverage cncf-tsuru-reviewer`.
8. Run `skcr compile cncf-tsuru-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tsuru-reviewer`.
...
```bash
skpm validate cncf-tsuru-reviewer
skpm package cncf-tsuru-reviewer
skpm publish cncf-tsuru-reviewer
```
