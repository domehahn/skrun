...
7. Run `skcr validate` and `skcr asps coverage cncf-agones-reviewer`.
8. Run `skcr compile cncf-agones-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-agones-reviewer`.
...
```bash
skpm validate cncf-agones-reviewer
skpm package cncf-agones-reviewer
skpm publish cncf-agones-reviewer
```
