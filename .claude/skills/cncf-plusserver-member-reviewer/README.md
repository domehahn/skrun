...
7. Run `skcr validate` and `skcr asps coverage cncf-plusserver-member-reviewer`.
8. Run `skcr compile cncf-plusserver-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-plusserver-member-reviewer`.
...
```bash
skpm validate cncf-plusserver-member-reviewer
skpm package cncf-plusserver-member-reviewer
skpm publish cncf-plusserver-member-reviewer
```
