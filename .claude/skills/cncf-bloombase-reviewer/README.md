...
7. Run `skcr validate` and `skcr asps coverage cncf-bloombase-reviewer`.
8. Run `skcr compile cncf-bloombase-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bloombase-reviewer`.
...
```bash
skpm validate cncf-bloombase-reviewer
skpm package cncf-bloombase-reviewer
skpm publish cncf-bloombase-reviewer
```
