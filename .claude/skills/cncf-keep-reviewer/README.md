...
7. Run `skcr validate` and `skcr asps coverage cncf-keep-reviewer`.
8. Run `skcr compile cncf-keep-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-keep-reviewer`.
...
```bash
skpm validate cncf-keep-reviewer
skpm package cncf-keep-reviewer
skpm publish cncf-keep-reviewer
```
