...
7. Run `skcr validate` and `skcr asps coverage cncf-txtai-reviewer`.
8. Run `skcr compile cncf-txtai-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-txtai-reviewer`.
...
```bash
skpm validate cncf-txtai-reviewer
skpm package cncf-txtai-reviewer
skpm publish cncf-txtai-reviewer
```
