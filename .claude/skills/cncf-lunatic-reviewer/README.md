...
7. Run `skcr validate` and `skcr asps coverage cncf-lunatic-reviewer`.
8. Run `skcr compile cncf-lunatic-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lunatic-reviewer`.
...
```bash
skpm validate cncf-lunatic-reviewer
skpm package cncf-lunatic-reviewer
skpm publish cncf-lunatic-reviewer
```
