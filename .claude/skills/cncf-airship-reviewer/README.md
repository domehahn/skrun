...
7. Run `skcr validate` and `skcr asps coverage cncf-airship-reviewer`.
8. Run `skcr compile cncf-airship-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-airship-reviewer`.
...
```bash
skpm validate cncf-airship-reviewer
skpm package cncf-airship-reviewer
skpm publish cncf-airship-reviewer
```
