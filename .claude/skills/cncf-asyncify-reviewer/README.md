...
7. Run `skcr validate` and `skcr asps coverage cncf-asyncify-reviewer`.
8. Run `skcr compile cncf-asyncify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-asyncify-reviewer`.
...
```bash
skpm validate cncf-asyncify-reviewer
skpm package cncf-asyncify-reviewer
skpm publish cncf-asyncify-reviewer
```
