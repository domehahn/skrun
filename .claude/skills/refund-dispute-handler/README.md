...
7. Run `skcr validate` and `skcr asps coverage refund-dispute-handler`.
8. Run `skcr compile refund-dispute-handler --target skil`.
9. Run `skcr sync` and `skcr version recommend refund-dispute-handler`.
...
```bash
skpm validate refund-dispute-handler
skpm package refund-dispute-handler
skpm publish refund-dispute-handler
```
