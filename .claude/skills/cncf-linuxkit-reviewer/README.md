...
7. Run `skcr validate` and `skcr asps coverage cncf-linuxkit-reviewer`.
8. Run `skcr compile cncf-linuxkit-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-linuxkit-reviewer`.
...
```bash
skpm validate cncf-linuxkit-reviewer
skpm package cncf-linuxkit-reviewer
skpm publish cncf-linuxkit-reviewer
```
