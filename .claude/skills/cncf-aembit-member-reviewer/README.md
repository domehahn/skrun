...
7. Run `skcr validate` and `skcr asps coverage cncf-aembit-member-reviewer`.
8. Run `skcr compile cncf-aembit-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-aembit-member-reviewer`.
...
```bash
skpm validate cncf-aembit-member-reviewer
skpm package cncf-aembit-member-reviewer
skpm publish cncf-aembit-member-reviewer
```
