...
7. Run `skcr validate` and `skcr asps coverage cncf-kubevirt-reviewer`.
8. Run `skcr compile cncf-kubevirt-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubevirt-reviewer`.
...
```bash
skpm validate cncf-kubevirt-reviewer
skpm package cncf-kubevirt-reviewer
skpm publish cncf-kubevirt-reviewer
```
