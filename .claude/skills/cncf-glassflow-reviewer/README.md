...
7. Run `skcr validate` and `skcr asps coverage cncf-glassflow-reviewer`.
8. Run `skcr compile cncf-glassflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-glassflow-reviewer`.
...
```bash
skpm validate cncf-glassflow-reviewer
skpm package cncf-glassflow-reviewer
skpm publish cncf-glassflow-reviewer
```
