...
7. Run `skcr validate` and `skcr asps coverage cncf-unikraft-reviewer`.
8. Run `skcr compile cncf-unikraft-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-unikraft-reviewer`.
...
```bash
skpm validate cncf-unikraft-reviewer
skpm package cncf-unikraft-reviewer
skpm publish cncf-unikraft-reviewer
```
