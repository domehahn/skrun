...
7. Run `skcr validate` and `skcr asps coverage cncf-antrea-reviewer`.
8. Run `skcr compile cncf-antrea-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-antrea-reviewer`.
...
```bash
skpm validate cncf-antrea-reviewer
skpm package cncf-antrea-reviewer
skpm publish cncf-antrea-reviewer
```
