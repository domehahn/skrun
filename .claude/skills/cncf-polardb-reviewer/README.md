...
7. Run `skcr validate` and `skcr asps coverage cncf-polardb-reviewer`.
8. Run `skcr compile cncf-polardb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-polardb-reviewer`.
...
```bash
skpm validate cncf-polardb-reviewer
skpm package cncf-polardb-reviewer
skpm publish cncf-polardb-reviewer
```
