...
7. Run `skcr validate` and `skcr asps coverage cncf-microk8s-reviewer`.
8. Run `skcr compile cncf-microk8s-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-microk8s-reviewer`.
...
```bash
skpm validate cncf-microk8s-reviewer
skpm package cncf-microk8s-reviewer
skpm publish cncf-microk8s-reviewer
```
