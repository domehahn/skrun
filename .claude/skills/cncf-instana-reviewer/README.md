...
7. Run `skcr validate` and `skcr asps coverage cncf-instana-reviewer`.
8. Run `skcr compile cncf-instana-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-instana-reviewer`.
...
```bash
skpm validate cncf-instana-reviewer
skpm package cncf-instana-reviewer
skpm publish cncf-instana-reviewer
```
