...
7. Run `skcr validate` and `skcr asps coverage cncf-harbor-reviewer`.
8. Run `skcr compile cncf-harbor-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-harbor-reviewer`.
...
```bash
skpm validate cncf-harbor-reviewer
skpm package cncf-harbor-reviewer
skpm publish cncf-harbor-reviewer
```
