...
7. Run `skcr validate` and `skcr asps coverage cncf-langchain-reviewer`.
8. Run `skcr compile cncf-langchain-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-langchain-reviewer`.
...
```bash
skpm validate cncf-langchain-reviewer
skpm package cncf-langchain-reviewer
skpm publish cncf-langchain-reviewer
```
