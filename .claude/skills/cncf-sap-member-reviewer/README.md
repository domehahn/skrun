...
7. Run `skcr validate` and `skcr asps coverage cncf-sap-member-reviewer`.
8. Run `skcr compile cncf-sap-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sap-member-reviewer`.
...
```bash
skpm validate cncf-sap-member-reviewer
skpm package cncf-sap-member-reviewer
skpm publish cncf-sap-member-reviewer
```
