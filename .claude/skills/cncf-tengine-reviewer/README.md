...
7. Run `skcr validate` and `skcr asps coverage cncf-tengine-reviewer`.
8. Run `skcr compile cncf-tengine-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tengine-reviewer`.
...
```bash
skpm validate cncf-tengine-reviewer
skpm package cncf-tengine-reviewer
skpm publish cncf-tengine-reviewer
```
