...
7. Run `skcr validate` and `skcr asps coverage cncf-hpe-member-reviewer`.
8. Run `skcr compile cncf-hpe-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hpe-member-reviewer`.
...
```bash
skpm validate cncf-hpe-member-reviewer
skpm package cncf-hpe-member-reviewer
skpm publish cncf-hpe-member-reviewer
```
