...
7. Run `skcr validate` and `skcr asps coverage cncf-ligato-reviewer`.
8. Run `skcr compile cncf-ligato-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ligato-reviewer`.
...
```bash
skpm validate cncf-ligato-reviewer
skpm package cncf-ligato-reviewer
skpm publish cncf-ligato-reviewer
```
