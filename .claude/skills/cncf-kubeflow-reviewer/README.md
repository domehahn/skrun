...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeflow-reviewer`.
8. Run `skcr compile cncf-kubeflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeflow-reviewer`.
...
```bash
skpm validate cncf-kubeflow-reviewer
skpm package cncf-kubeflow-reviewer
skpm publish cncf-kubeflow-reviewer
```
