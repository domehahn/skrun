...
7. Run `skcr validate` and `skcr asps coverage cncf-kubesphere-member-reviewer`.
8. Run `skcr compile cncf-kubesphere-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubesphere-member-reviewer`.
...
```bash
skpm validate cncf-kubesphere-member-reviewer
skpm package cncf-kubesphere-member-reviewer
skpm publish cncf-kubesphere-member-reviewer
```
