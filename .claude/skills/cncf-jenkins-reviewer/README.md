...
7. Run `skcr validate` and `skcr asps coverage cncf-jenkins-reviewer`.
8. Run `skcr compile cncf-jenkins-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-jenkins-reviewer`.
...
```bash
skpm validate cncf-jenkins-reviewer
skpm package cncf-jenkins-reviewer
skpm publish cncf-jenkins-reviewer
```
