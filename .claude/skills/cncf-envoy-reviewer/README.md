...
7. Run `skcr validate` and `skcr asps coverage cncf-envoy-reviewer`.
8. Run `skcr compile cncf-envoy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-envoy-reviewer`.
...
```bash
skpm validate cncf-envoy-reviewer
skpm package cncf-envoy-reviewer
skpm publish cncf-envoy-reviewer
```
