...
7. Run `skcr validate` and `skcr asps coverage cncf-kind-reviewer`.
8. Run `skcr compile cncf-kind-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kind-reviewer`.
...
```bash
skpm validate cncf-kind-reviewer
skpm package cncf-kind-reviewer
skpm publish cncf-kind-reviewer
```
