...
7. Run `skcr validate` and `skcr asps coverage payment-security-reviewer`.
8. Run `skcr compile payment-security-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend payment-security-reviewer`.
...
```bash
skpm validate payment-security-reviewer
skpm package payment-security-reviewer
skpm publish payment-security-reviewer
```
