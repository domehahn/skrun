...
7. Run `skcr validate` and `skcr asps coverage cncf-sglang-reviewer`.
8. Run `skcr compile cncf-sglang-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sglang-reviewer`.
...
```bash
skpm validate cncf-sglang-reviewer
skpm package cncf-sglang-reviewer
skpm publish cncf-sglang-reviewer
```
