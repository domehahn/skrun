...
7. Run `skcr validate` and `skcr asps coverage cncf-fission-reviewer`.
8. Run `skcr compile cncf-fission-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fission-reviewer`.
...
```bash
skpm validate cncf-fission-reviewer
skpm package cncf-fission-reviewer
skpm publish cncf-fission-reviewer
```
