...
7. Run `skcr validate` and `skcr asps coverage cncf-openio-reviewer`.
8. Run `skcr compile cncf-openio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openio-reviewer`.
...
```bash
skpm validate cncf-openio-reviewer
skpm package cncf-openio-reviewer
skpm publish cncf-openio-reviewer
```
