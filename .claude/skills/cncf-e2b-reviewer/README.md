...
7. Run `skcr validate` and `skcr asps coverage cncf-e2b-reviewer`.
8. Run `skcr compile cncf-e2b-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-e2b-reviewer`.
...
```bash
skpm validate cncf-e2b-reviewer
skpm package cncf-e2b-reviewer
skpm publish cncf-e2b-reviewer
```
