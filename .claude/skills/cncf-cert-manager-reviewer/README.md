...
7. Run `skcr validate` and `skcr asps coverage cncf-cert-manager-reviewer`.
8. Run `skcr compile cncf-cert-manager-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cert-manager-reviewer`.
...
```bash
skpm validate cncf-cert-manager-reviewer
skpm package cncf-cert-manager-reviewer
skpm publish cncf-cert-manager-reviewer
```
