...
7. Run `skcr validate` and `skcr asps coverage cncf-ziax-member-reviewer`.
8. Run `skcr compile cncf-ziax-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ziax-member-reviewer`.
...
```bash
skpm validate cncf-ziax-member-reviewer
skpm package cncf-ziax-member-reviewer
skpm publish cncf-ziax-member-reviewer
```
