...
7. Run `skcr validate` and `skcr asps coverage cncf-pulumi-reviewer`.
8. Run `skcr compile cncf-pulumi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pulumi-reviewer`.
...
```bash
skpm validate cncf-pulumi-reviewer
skpm package cncf-pulumi-reviewer
skpm publish cncf-pulumi-reviewer
```
