...
7. Run `skcr validate` and `skcr asps coverage cncf-kube-bench-reviewer`.
8. Run `skcr compile cncf-kube-bench-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kube-bench-reviewer`.
...
```bash
skpm validate cncf-kube-bench-reviewer
skpm package cncf-kube-bench-reviewer
skpm publish cncf-kube-bench-reviewer
```
