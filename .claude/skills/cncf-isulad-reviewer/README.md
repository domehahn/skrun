...
7. Run `skcr validate` and `skcr asps coverage cncf-isulad-reviewer`.
8. Run `skcr compile cncf-isulad-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-isulad-reviewer`.
...
```bash
skpm validate cncf-isulad-reviewer
skpm package cncf-isulad-reviewer
skpm publish cncf-isulad-reviewer
```
