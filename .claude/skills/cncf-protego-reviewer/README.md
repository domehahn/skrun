...
7. Run `skcr validate` and `skcr asps coverage cncf-protego-reviewer`.
8. Run `skcr compile cncf-protego-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-protego-reviewer`.
...
```bash
skpm validate cncf-protego-reviewer
skpm package cncf-protego-reviewer
skpm publish cncf-protego-reviewer
```
