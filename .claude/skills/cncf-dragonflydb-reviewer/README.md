...
7. Run `skcr validate` and `skcr asps coverage cncf-dragonflydb-reviewer`.
8. Run `skcr compile cncf-dragonflydb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dragonflydb-reviewer`.
...
```bash
skpm validate cncf-dragonflydb-reviewer
skpm package cncf-dragonflydb-reviewer
skpm publish cncf-dragonflydb-reviewer
```
