...
7. Run `skcr validate` and `skcr asps coverage cncf-zte-member-reviewer`.
8. Run `skcr compile cncf-zte-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zte-member-reviewer`.
...
```bash
skpm validate cncf-zte-member-reviewer
skpm package cncf-zte-member-reviewer
skpm publish cncf-zte-member-reviewer
```
