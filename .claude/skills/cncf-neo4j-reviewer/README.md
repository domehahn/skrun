...
7. Run `skcr validate` and `skcr asps coverage cncf-neo4j-reviewer`.
8. Run `skcr compile cncf-neo4j-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-neo4j-reviewer`.
...
```bash
skpm validate cncf-neo4j-reviewer
skpm package cncf-neo4j-reviewer
skpm publish cncf-neo4j-reviewer
```
