...
7. Run `skcr validate` and `skcr asps coverage cncf-sysdig-reviewer`.
8. Run `skcr compile cncf-sysdig-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sysdig-reviewer`.
...
```bash
skpm validate cncf-sysdig-reviewer
skpm package cncf-sysdig-reviewer
skpm publish cncf-sysdig-reviewer
```
