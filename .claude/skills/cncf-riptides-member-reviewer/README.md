...
7. Run `skcr validate` and `skcr asps coverage cncf-riptides-member-reviewer`.
8. Run `skcr compile cncf-riptides-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-riptides-member-reviewer`.
...
```bash
skpm validate cncf-riptides-member-reviewer
skpm package cncf-riptides-member-reviewer
skpm publish cncf-riptides-member-reviewer
```
