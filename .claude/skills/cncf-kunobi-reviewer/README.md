...
7. Run `skcr validate` and `skcr asps coverage cncf-kunobi-reviewer`.
8. Run `skcr compile cncf-kunobi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kunobi-reviewer`.
...
```bash
skpm validate cncf-kunobi-reviewer
skpm package cncf-kunobi-reviewer
skpm publish cncf-kunobi-reviewer
```
