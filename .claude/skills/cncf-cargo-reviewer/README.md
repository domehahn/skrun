...
7. Run `skcr validate` and `skcr asps coverage cncf-cargo-reviewer`.
8. Run `skcr compile cncf-cargo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cargo-reviewer`.
...
```bash
skpm validate cncf-cargo-reviewer
skpm package cncf-cargo-reviewer
skpm publish cncf-cargo-reviewer
```
