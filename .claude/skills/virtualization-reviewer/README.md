...
7. Run `skcr validate` and `skcr asps coverage virtualization-reviewer`.
8. Run `skcr compile virtualization-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend virtualization-reviewer`.
...
```bash
skpm validate virtualization-reviewer
skpm package virtualization-reviewer
skpm publish virtualization-reviewer
```
