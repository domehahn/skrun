...
7. Run `skcr validate` and `skcr asps coverage cncf-kube-rs-reviewer`.
8. Run `skcr compile cncf-kube-rs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kube-rs-reviewer`.
...
```bash
skpm validate cncf-kube-rs-reviewer
skpm package cncf-kube-rs-reviewer
skpm publish cncf-kube-rs-reviewer
```
