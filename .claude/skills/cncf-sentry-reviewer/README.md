...
7. Run `skcr validate` and `skcr asps coverage cncf-sentry-reviewer`.
8. Run `skcr compile cncf-sentry-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sentry-reviewer`.
...
```bash
skpm validate cncf-sentry-reviewer
skpm package cncf-sentry-reviewer
skpm publish cncf-sentry-reviewer
```
