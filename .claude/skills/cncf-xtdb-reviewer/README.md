...
7. Run `skcr validate` and `skcr asps coverage cncf-xtdb-reviewer`.
8. Run `skcr compile cncf-xtdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-xtdb-reviewer`.
...
```bash
skpm validate cncf-xtdb-reviewer
skpm package cncf-xtdb-reviewer
skpm publish cncf-xtdb-reviewer
```
