...
7. Run `skcr validate` and `skcr asps coverage cncf-verda-member-reviewer`.
8. Run `skcr compile cncf-verda-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-verda-member-reviewer`.
...
```bash
skpm validate cncf-verda-member-reviewer
skpm package cncf-verda-member-reviewer
skpm publish cncf-verda-member-reviewer
```
