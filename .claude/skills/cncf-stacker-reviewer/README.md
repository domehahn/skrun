...
7. Run `skcr validate` and `skcr asps coverage cncf-stacker-reviewer`.
8. Run `skcr compile cncf-stacker-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-stacker-reviewer`.
...
```bash
skpm validate cncf-stacker-reviewer
skpm package cncf-stacker-reviewer
skpm publish cncf-stacker-reviewer
```
