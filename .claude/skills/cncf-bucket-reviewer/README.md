...
7. Run `skcr validate` and `skcr asps coverage cncf-bucket-reviewer`.
8. Run `skcr compile cncf-bucket-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bucket-reviewer`.
...
```bash
skpm validate cncf-bucket-reviewer
skpm package cncf-bucket-reviewer
skpm publish cncf-bucket-reviewer
```
