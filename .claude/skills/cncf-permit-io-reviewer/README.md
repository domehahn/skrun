...
7. Run `skcr validate` and `skcr asps coverage cncf-permit-io-reviewer`.
8. Run `skcr compile cncf-permit-io-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-permit-io-reviewer`.
...
```bash
skpm validate cncf-permit-io-reviewer
skpm package cncf-permit-io-reviewer
skpm publish cncf-permit-io-reviewer
```
