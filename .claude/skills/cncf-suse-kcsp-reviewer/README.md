...
7. Run `skcr validate` and `skcr asps coverage cncf-suse-kcsp-reviewer`.
8. Run `skcr compile cncf-suse-kcsp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-suse-kcsp-reviewer`.
...
```bash
skpm validate cncf-suse-kcsp-reviewer
skpm package cncf-suse-kcsp-reviewer
skpm publish cncf-suse-kcsp-reviewer
```
