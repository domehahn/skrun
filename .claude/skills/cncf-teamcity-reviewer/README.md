...
7. Run `skcr validate` and `skcr asps coverage cncf-teamcity-reviewer`.
8. Run `skcr compile cncf-teamcity-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-teamcity-reviewer`.
...
```bash
skpm validate cncf-teamcity-reviewer
skpm package cncf-teamcity-reviewer
skpm publish cncf-teamcity-reviewer
```
