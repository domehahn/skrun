...
7. Run `skcr validate` and `skcr asps coverage cncf-tdengine-reviewer`.
8. Run `skcr compile cncf-tdengine-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tdengine-reviewer`.
...
```bash
skpm validate cncf-tdengine-reviewer
skpm package cncf-tdengine-reviewer
skpm publish cncf-tdengine-reviewer
```
