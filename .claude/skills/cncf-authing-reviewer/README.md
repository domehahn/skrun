...
7. Run `skcr validate` and `skcr asps coverage cncf-authing-reviewer`.
8. Run `skcr compile cncf-authing-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-authing-reviewer`.
...
```bash
skpm validate cncf-authing-reviewer
skpm package cncf-authing-reviewer
skpm publish cncf-authing-reviewer
```
