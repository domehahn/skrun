...
7. Run `skcr validate` and `skcr asps coverage cncf-kubegateway-reviewer`.
8. Run `skcr compile cncf-kubegateway-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubegateway-reviewer`.
...
```bash
skpm validate cncf-kubegateway-reviewer
skpm package cncf-kubegateway-reviewer
skpm publish cncf-kubegateway-reviewer
```
