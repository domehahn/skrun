...
7. Run `skcr validate` and `skcr asps coverage cncf-cohdi-reviewer`.
8. Run `skcr compile cncf-cohdi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cohdi-reviewer`.
...
```bash
skpm validate cncf-cohdi-reviewer
skpm package cncf-cohdi-reviewer
skpm publish cncf-cohdi-reviewer
```
