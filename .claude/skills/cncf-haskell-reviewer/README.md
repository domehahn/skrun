...
7. Run `skcr validate` and `skcr asps coverage cncf-haskell-reviewer`.
8. Run `skcr compile cncf-haskell-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-haskell-reviewer`.
...
```bash
skpm validate cncf-haskell-reviewer
skpm package cncf-haskell-reviewer
skpm publish cncf-haskell-reviewer
```
