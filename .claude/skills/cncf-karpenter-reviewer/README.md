...
7. Run `skcr validate` and `skcr asps coverage cncf-karpenter-reviewer`.
8. Run `skcr compile cncf-karpenter-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-karpenter-reviewer`.
...
```bash
skpm validate cncf-karpenter-reviewer
skpm package cncf-karpenter-reviewer
skpm publish cncf-karpenter-reviewer
```
