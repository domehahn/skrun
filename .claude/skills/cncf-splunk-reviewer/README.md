...
7. Run `skcr validate` and `skcr asps coverage cncf-splunk-reviewer`.
8. Run `skcr compile cncf-splunk-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-splunk-reviewer`.
...
```bash
skpm validate cncf-splunk-reviewer
skpm package cncf-splunk-reviewer
skpm publish cncf-splunk-reviewer
```
