...
7. Run `skcr validate` and `skcr asps coverage cncf-dart-reviewer`.
8. Run `skcr compile cncf-dart-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dart-reviewer`.
...
```bash
skpm validate cncf-dart-reviewer
skpm package cncf-dart-reviewer
skpm publish cncf-dart-reviewer
```
