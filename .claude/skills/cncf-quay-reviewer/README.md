...
7. Run `skcr validate` and `skcr asps coverage cncf-quay-reviewer`.
8. Run `skcr compile cncf-quay-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-quay-reviewer`.
...
```bash
skpm validate cncf-quay-reviewer
skpm package cncf-quay-reviewer
skpm publish cncf-quay-reviewer
```
