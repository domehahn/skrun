...
7. Run `skcr validate` and `skcr asps coverage cncf-parseable-reviewer`.
8. Run `skcr compile cncf-parseable-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-parseable-reviewer`.
...
```bash
skpm validate cncf-parseable-reviewer
skpm package cncf-parseable-reviewer
skpm publish cncf-parseable-reviewer
```
