...
7. Run `skcr validate` and `skcr asps coverage cncf-kubekey-reviewer`.
8. Run `skcr compile cncf-kubekey-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubekey-reviewer`.
...
```bash
skpm validate cncf-kubekey-reviewer
skpm package cncf-kubekey-reviewer
skpm publish cncf-kubekey-reviewer
```
