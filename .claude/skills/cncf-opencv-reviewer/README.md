...
7. Run `skcr validate` and `skcr asps coverage cncf-opencv-reviewer`.
8. Run `skcr compile cncf-opencv-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opencv-reviewer`.
...
```bash
skpm validate cncf-opencv-reviewer
skpm package cncf-opencv-reviewer
skpm publish cncf-opencv-reviewer
```
