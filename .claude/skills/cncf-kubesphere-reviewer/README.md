...
7. Run `skcr validate` and `skcr asps coverage cncf-kubesphere-reviewer`.
8. Run `skcr compile cncf-kubesphere-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubesphere-reviewer`.
...
```bash
skpm validate cncf-kubesphere-reviewer
skpm package cncf-kubesphere-reviewer
skpm publish cncf-kubesphere-reviewer
```
