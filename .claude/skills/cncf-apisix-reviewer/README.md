...
7. Run `skcr validate` and `skcr asps coverage cncf-apisix-reviewer`.
8. Run `skcr compile cncf-apisix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-apisix-reviewer`.
...
```bash
skpm validate cncf-apisix-reviewer
skpm package cncf-apisix-reviewer
skpm publish cncf-apisix-reviewer
```
