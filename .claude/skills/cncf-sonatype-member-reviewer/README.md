...
7. Run `skcr validate` and `skcr asps coverage cncf-sonatype-member-reviewer`.
8. Run `skcr compile cncf-sonatype-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sonatype-member-reviewer`.
...
```bash
skpm validate cncf-sonatype-member-reviewer
skpm package cncf-sonatype-member-reviewer
skpm publish cncf-sonatype-member-reviewer
```
