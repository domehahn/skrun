...
7. Run `skcr validate` and `skcr asps coverage cncf-cni-genie-reviewer`.
8. Run `skcr compile cncf-cni-genie-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cni-genie-reviewer`.
...
```bash
skpm validate cncf-cni-genie-reviewer
skpm package cncf-cni-genie-reviewer
skpm publish cncf-cni-genie-reviewer
```
