...
7. Run `skcr validate` and `skcr asps coverage cncf-itq-member-reviewer`.
8. Run `skcr compile cncf-itq-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-itq-member-reviewer`.
...
```bash
skpm validate cncf-itq-member-reviewer
skpm package cncf-itq-member-reviewer
skpm publish cncf-itq-member-reviewer
```
