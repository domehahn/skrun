...
7. Run `skcr validate` and `skcr asps coverage javascript-reviewer`.
8. Run `skcr compile javascript-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend javascript-reviewer`.
...
```bash
skpm validate javascript-reviewer
skpm package javascript-reviewer
skpm publish javascript-reviewer
```
