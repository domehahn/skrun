...
7. Run `skcr validate` and `skcr asps coverage cncf-meshery-reviewer`.
8. Run `skcr compile cncf-meshery-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-meshery-reviewer`.
...
```bash
skpm validate cncf-meshery-reviewer
skpm package cncf-meshery-reviewer
skpm publish cncf-meshery-reviewer
```
