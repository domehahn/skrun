...
7. Run `skcr validate` and `skcr asps coverage cncf-spiffe-reviewer`.
8. Run `skcr compile cncf-spiffe-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spiffe-reviewer`.
...
```bash
skpm validate cncf-spiffe-reviewer
skpm package cncf-spiffe-reviewer
skpm publish cncf-spiffe-reviewer
```
