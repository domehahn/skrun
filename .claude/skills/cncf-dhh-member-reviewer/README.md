...
7. Run `skcr validate` and `skcr asps coverage cncf-dhh-member-reviewer`.
8. Run `skcr compile cncf-dhh-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dhh-member-reviewer`.
...
```bash
skpm validate cncf-dhh-member-reviewer
skpm package cncf-dhh-member-reviewer
skpm publish cncf-dhh-member-reviewer
```
