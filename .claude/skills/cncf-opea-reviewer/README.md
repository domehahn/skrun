...
7. Run `skcr validate` and `skcr asps coverage cncf-opea-reviewer`.
8. Run `skcr compile cncf-opea-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opea-reviewer`.
...
```bash
skpm validate cncf-opea-reviewer
skpm package cncf-opea-reviewer
skpm publish cncf-opea-reviewer
```
