...
7. Run `skcr validate` and `skcr asps coverage cncf-accordion-reviewer`.
8. Run `skcr compile cncf-accordion-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-accordion-reviewer`.
...
```bash
skpm validate cncf-accordion-reviewer
skpm package cncf-accordion-reviewer
skpm publish cncf-accordion-reviewer
```
