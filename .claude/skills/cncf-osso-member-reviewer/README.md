...
7. Run `skcr validate` and `skcr asps coverage cncf-osso-member-reviewer`.
8. Run `skcr compile cncf-osso-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-osso-member-reviewer`.
...
```bash
skpm validate cncf-osso-member-reviewer
skpm package cncf-osso-member-reviewer
skpm publish cncf-osso-member-reviewer
```
