...
7. Run `skcr validate` and `skcr asps coverage cncf-env-zero-member-reviewer`.
8. Run `skcr compile cncf-env-zero-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-env-zero-member-reviewer`.
...
```bash
skpm validate cncf-env-zero-member-reviewer
skpm package cncf-env-zero-member-reviewer
skpm publish cncf-env-zero-member-reviewer
```
