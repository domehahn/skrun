...
7. Run `skcr validate` and `skcr asps coverage cncf-hyscale-reviewer`.
8. Run `skcr compile cncf-hyscale-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hyscale-reviewer`.
...
```bash
skpm validate cncf-hyscale-reviewer
skpm package cncf-hyscale-reviewer
skpm publish cncf-hyscale-reviewer
```
