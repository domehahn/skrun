...
7. Run `skcr validate` and `skcr asps coverage cncf-mirrord-reviewer`.
8. Run `skcr compile cncf-mirrord-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mirrord-reviewer`.
...
```bash
skpm validate cncf-mirrord-reviewer
skpm package cncf-mirrord-reviewer
skpm publish cncf-mirrord-reviewer
```
