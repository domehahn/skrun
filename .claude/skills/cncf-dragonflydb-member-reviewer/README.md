...
7. Run `skcr validate` and `skcr asps coverage cncf-dragonflydb-member-reviewer`.
8. Run `skcr compile cncf-dragonflydb-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dragonflydb-member-reviewer`.
...
```bash
skpm validate cncf-dragonflydb-member-reviewer
skpm package cncf-dragonflydb-member-reviewer
skpm publish cncf-dragonflydb-member-reviewer
```
