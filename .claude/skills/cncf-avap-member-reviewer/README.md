...
7. Run `skcr validate` and `skcr asps coverage cncf-avap-member-reviewer`.
8. Run `skcr compile cncf-avap-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-avap-member-reviewer`.
...
```bash
skpm validate cncf-avap-member-reviewer
skpm package cncf-avap-member-reviewer
skpm publish cncf-avap-member-reviewer
```
