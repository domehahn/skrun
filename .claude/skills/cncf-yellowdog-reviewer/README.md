...
7. Run `skcr validate` and `skcr asps coverage cncf-yellowdog-reviewer`.
8. Run `skcr compile cncf-yellowdog-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yellowdog-reviewer`.
...
```bash
skpm validate cncf-yellowdog-reviewer
skpm package cncf-yellowdog-reviewer
skpm publish cncf-yellowdog-reviewer
```
