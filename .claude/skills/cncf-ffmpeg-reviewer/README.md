...
7. Run `skcr validate` and `skcr asps coverage cncf-ffmpeg-reviewer`.
8. Run `skcr compile cncf-ffmpeg-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ffmpeg-reviewer`.
...
```bash
skpm validate cncf-ffmpeg-reviewer
skpm package cncf-ffmpeg-reviewer
skpm publish cncf-ffmpeg-reviewer
```
