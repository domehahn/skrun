...
7. Run `skcr validate` and `skcr asps coverage cncf-monocle-reviewer`.
8. Run `skcr compile cncf-monocle-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-monocle-reviewer`.
...
```bash
skpm validate cncf-monocle-reviewer
skpm package cncf-monocle-reviewer
skpm publish cncf-monocle-reviewer
```
