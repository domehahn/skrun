...
7. Run `skcr validate` and `skcr asps coverage cncf-snyk-member-reviewer`.
8. Run `skcr compile cncf-snyk-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-snyk-member-reviewer`.
...
```bash
skpm validate cncf-snyk-member-reviewer
skpm package cncf-snyk-member-reviewer
skpm publish cncf-snyk-member-reviewer
```
