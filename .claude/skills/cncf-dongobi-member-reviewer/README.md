...
7. Run `skcr validate` and `skcr asps coverage cncf-dongobi-member-reviewer`.
8. Run `skcr compile cncf-dongobi-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dongobi-member-reviewer`.
...
```bash
skpm validate cncf-dongobi-member-reviewer
skpm package cncf-dongobi-member-reviewer
skpm publish cncf-dongobi-member-reviewer
```
