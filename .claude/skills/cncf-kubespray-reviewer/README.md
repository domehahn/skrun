...
7. Run `skcr validate` and `skcr asps coverage cncf-kubespray-reviewer`.
8. Run `skcr compile cncf-kubespray-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubespray-reviewer`.
...
```bash
skpm validate cncf-kubespray-reviewer
skpm package cncf-kubespray-reviewer
skpm publish cncf-kubespray-reviewer
```
