...
7. Run `skcr validate` and `skcr asps coverage cncf-doit-member-reviewer`.
8. Run `skcr compile cncf-doit-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-doit-member-reviewer`.
...
```bash
skpm validate cncf-doit-member-reviewer
skpm package cncf-doit-member-reviewer
skpm publish cncf-doit-member-reviewer
```
