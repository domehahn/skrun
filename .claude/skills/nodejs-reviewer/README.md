...
7. Run `skcr validate` and `skcr asps coverage nodejs-reviewer`.
8. Run `skcr compile nodejs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend nodejs-reviewer`.
...
```bash
skpm validate nodejs-reviewer
skpm package nodejs-reviewer
skpm publish nodejs-reviewer
```
