...
7. Run `skcr validate` and `skcr asps coverage cncf-fossid-reviewer`.
8. Run `skcr compile cncf-fossid-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fossid-reviewer`.
...
```bash
skpm validate cncf-fossid-reviewer
skpm package cncf-fossid-reviewer
skpm publish cncf-fossid-reviewer
```
