...
7. Run `skcr validate` and `skcr asps coverage cncf-loxilb-reviewer`.
8. Run `skcr compile cncf-loxilb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-loxilb-reviewer`.
...
```bash
skpm validate cncf-loxilb-reviewer
skpm package cncf-loxilb-reviewer
skpm publish cncf-loxilb-reviewer
```
