...
7. Run `skcr validate` and `skcr asps coverage cncf-kuberay-reviewer`.
8. Run `skcr compile cncf-kuberay-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kuberay-reviewer`.
...
```bash
skpm validate cncf-kuberay-reviewer
skpm package cncf-kuberay-reviewer
skpm publish cncf-kuberay-reviewer
```
