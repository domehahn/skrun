...
7. Run `skcr validate` and `skcr asps coverage cncf-isscloud-reviewer`.
8. Run `skcr compile cncf-isscloud-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-isscloud-reviewer`.
...
```bash
skpm validate cncf-isscloud-reviewer
skpm package cncf-isscloud-reviewer
skpm publish cncf-isscloud-reviewer
```
