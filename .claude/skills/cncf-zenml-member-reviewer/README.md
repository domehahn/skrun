...
7. Run `skcr validate` and `skcr asps coverage cncf-zenml-member-reviewer`.
8. Run `skcr compile cncf-zenml-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zenml-member-reviewer`.
...
```bash
skpm validate cncf-zenml-member-reviewer
skpm package cncf-zenml-member-reviewer
skpm publish cncf-zenml-member-reviewer
```
