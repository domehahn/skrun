...
7. Run `skcr validate` and `skcr asps coverage cncf-dosec-reviewer`.
8. Run `skcr compile cncf-dosec-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dosec-reviewer`.
...
```bash
skpm validate cncf-dosec-reviewer
skpm package cncf-dosec-reviewer
skpm publish cncf-dosec-reviewer
```
