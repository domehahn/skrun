...
7. Run `skcr validate` and `skcr asps coverage cncf-expontech-reviewer`.
8. Run `skcr compile cncf-expontech-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-expontech-reviewer`.
...
```bash
skpm validate cncf-expontech-reviewer
skpm package cncf-expontech-reviewer
skpm publish cncf-expontech-reviewer
```
