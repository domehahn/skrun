...
7. Run `skcr validate` and `skcr asps coverage cncf-edb-reviewer`.
8. Run `skcr compile cncf-edb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-edb-reviewer`.
...
```bash
skpm validate cncf-edb-reviewer
skpm package cncf-edb-reviewer
skpm publish cncf-edb-reviewer
```
