...
7. Run `skcr validate` and `skcr asps coverage cncf-fluentd-reviewer`.
8. Run `skcr compile cncf-fluentd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fluentd-reviewer`.
...
```bash
skpm validate cncf-fluentd-reviewer
skpm package cncf-fluentd-reviewer
skpm publish cncf-fluentd-reviewer
```
