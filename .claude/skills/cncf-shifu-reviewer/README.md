...
7. Run `skcr validate` and `skcr asps coverage cncf-shifu-reviewer`.
8. Run `skcr compile cncf-shifu-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-shifu-reviewer`.
...
```bash
skpm validate cncf-shifu-reviewer
skpm package cncf-shifu-reviewer
skpm publish cncf-shifu-reviewer
```
