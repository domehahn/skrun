...
7. Run `skcr validate` and `skcr asps coverage cncf-arks-reviewer`.
8. Run `skcr compile cncf-arks-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-arks-reviewer`.
...
```bash
skpm validate cncf-arks-reviewer
skpm package cncf-arks-reviewer
skpm publish cncf-arks-reviewer
```
