...
7. Run `skcr validate` and `skcr asps coverage cncf-fortio-reviewer`.
8. Run `skcr compile cncf-fortio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fortio-reviewer`.
...
```bash
skpm validate cncf-fortio-reviewer
skpm package cncf-fortio-reviewer
skpm publish cncf-fortio-reviewer
```
