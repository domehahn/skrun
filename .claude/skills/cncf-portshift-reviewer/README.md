...
7. Run `skcr validate` and `skcr asps coverage cncf-portshift-reviewer`.
8. Run `skcr compile cncf-portshift-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-portshift-reviewer`.
...
```bash
skpm validate cncf-portshift-reviewer
skpm package cncf-portshift-reviewer
skpm publish cncf-portshift-reviewer
```
