...
7. Run `skcr validate` and `skcr asps coverage cncf-nops-reviewer`.
8. Run `skcr compile cncf-nops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nops-reviewer`.
...
```bash
skpm validate cncf-nops-reviewer
skpm package cncf-nops-reviewer
skpm publish cncf-nops-reviewer
```
