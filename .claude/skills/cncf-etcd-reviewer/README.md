...
7. Run `skcr validate` and `skcr asps coverage cncf-etcd-reviewer`.
8. Run `skcr compile cncf-etcd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-etcd-reviewer`.
...
```bash
skpm validate cncf-etcd-reviewer
skpm package cncf-etcd-reviewer
skpm publish cncf-etcd-reviewer
```
