...
7. Run `skcr validate` and `skcr asps coverage cncf-icinga-reviewer`.
8. Run `skcr compile cncf-icinga-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-icinga-reviewer`.
...
```bash
skpm validate cncf-icinga-reviewer
skpm package cncf-icinga-reviewer
skpm publish cncf-icinga-reviewer
```
