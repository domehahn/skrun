...
7. Run `skcr validate` and `skcr asps coverage cncf-aiven-member-reviewer`.
8. Run `skcr compile cncf-aiven-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-aiven-member-reviewer`.
...
```bash
skpm validate cncf-aiven-member-reviewer
skpm package cncf-aiven-member-reviewer
skpm publish cncf-aiven-member-reviewer
```
