...
7. Run `skcr validate` and `skcr asps coverage cncf-woodpecker-ci-reviewer`.
8. Run `skcr compile cncf-woodpecker-ci-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-woodpecker-ci-reviewer`.
...
```bash
skpm validate cncf-woodpecker-ci-reviewer
skpm package cncf-woodpecker-ci-reviewer
skpm publish cncf-woodpecker-ci-reviewer
```
