...
7. Run `skcr validate` and `skcr asps coverage cncf-vshn-ag-member-reviewer`.
8. Run `skcr compile cncf-vshn-ag-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vshn-ag-member-reviewer`.
...
```bash
skpm validate cncf-vshn-ag-member-reviewer
skpm package cncf-vshn-ag-member-reviewer
skpm publish cncf-vshn-ag-member-reviewer
```
