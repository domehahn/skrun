...
7. Run `skcr validate` and `skcr asps coverage cncf-vllm-reviewer`.
8. Run `skcr compile cncf-vllm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vllm-reviewer`.
...
```bash
skpm validate cncf-vllm-reviewer
skpm package cncf-vllm-reviewer
skpm publish cncf-vllm-reviewer
```
