...
7. Run `skcr validate` and `skcr asps coverage cncf-flox-member-reviewer`.
8. Run `skcr compile cncf-flox-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flox-member-reviewer`.
...
```bash
skpm validate cncf-flox-member-reviewer
skpm package cncf-flox-member-reviewer
skpm publish cncf-flox-member-reviewer
```
