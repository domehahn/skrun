...
7. Run `skcr validate` and `skcr asps coverage cncf-postgresql-reviewer`.
8. Run `skcr compile cncf-postgresql-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-postgresql-reviewer`.
...
```bash
skpm validate cncf-postgresql-reviewer
skpm package cncf-postgresql-reviewer
skpm publish cncf-postgresql-reviewer
```
