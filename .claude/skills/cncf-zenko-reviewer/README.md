...
7. Run `skcr validate` and `skcr asps coverage cncf-zenko-reviewer`.
8. Run `skcr compile cncf-zenko-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zenko-reviewer`.
...
```bash
skpm validate cncf-zenko-reviewer
skpm package cncf-zenko-reviewer
skpm publish cncf-zenko-reviewer
```
