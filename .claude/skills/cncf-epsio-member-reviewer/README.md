...
7. Run `skcr validate` and `skcr asps coverage cncf-epsio-member-reviewer`.
8. Run `skcr compile cncf-epsio-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-epsio-member-reviewer`.
...
```bash
skpm validate cncf-epsio-member-reviewer
skpm package cncf-epsio-member-reviewer
skpm publish cncf-epsio-member-reviewer
```
