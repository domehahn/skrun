...
7. Run `skcr validate` and `skcr asps coverage cncf-foreman-reviewer`.
8. Run `skcr compile cncf-foreman-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-foreman-reviewer`.
...
```bash
skpm validate cncf-foreman-reviewer
skpm package cncf-foreman-reviewer
skpm publish cncf-foreman-reviewer
```
