...
7. Run `skcr validate` and `skcr asps coverage cncf-yld-member-reviewer`.
8. Run `skcr compile cncf-yld-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yld-member-reviewer`.
...
```bash
skpm validate cncf-yld-member-reviewer
skpm package cncf-yld-member-reviewer
skpm publish cncf-yld-member-reviewer
```
