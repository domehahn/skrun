...
7. Run `skcr validate` and `skcr asps coverage cncf-nagios-reviewer`.
8. Run `skcr compile cncf-nagios-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nagios-reviewer`.
...
```bash
skpm validate cncf-nagios-reviewer
skpm package cncf-nagios-reviewer
skpm publish cncf-nagios-reviewer
```
