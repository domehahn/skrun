...
7. Run `skcr validate` and `skcr asps coverage cncf-kestra-reviewer`.
8. Run `skcr compile cncf-kestra-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kestra-reviewer`.
...
```bash
skpm validate cncf-kestra-reviewer
skpm package cncf-kestra-reviewer
skpm publish cncf-kestra-reviewer
```
