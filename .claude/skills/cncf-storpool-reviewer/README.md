...
7. Run `skcr validate` and `skcr asps coverage cncf-storpool-reviewer`.
8. Run `skcr compile cncf-storpool-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-storpool-reviewer`.
...
```bash
skpm validate cncf-storpool-reviewer
skpm package cncf-storpool-reviewer
skpm publish cncf-storpool-reviewer
```
