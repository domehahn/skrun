...
7. Run `skcr validate` and `skcr asps coverage cncf-nuodb-reviewer`.
8. Run `skcr compile cncf-nuodb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nuodb-reviewer`.
...
```bash
skpm validate cncf-nuodb-reviewer
skpm package cncf-nuodb-reviewer
skpm publish cncf-nuodb-reviewer
```
