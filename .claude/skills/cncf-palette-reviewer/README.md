...
7. Run `skcr validate` and `skcr asps coverage cncf-palette-reviewer`.
8. Run `skcr compile cncf-palette-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-palette-reviewer`.
...
```bash
skpm validate cncf-palette-reviewer
skpm package cncf-palette-reviewer
skpm publish cncf-palette-reviewer
```
