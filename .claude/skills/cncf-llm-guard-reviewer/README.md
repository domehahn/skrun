...
7. Run `skcr validate` and `skcr asps coverage cncf-llm-guard-reviewer`.
8. Run `skcr compile cncf-llm-guard-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llm-guard-reviewer`.
...
```bash
skpm validate cncf-llm-guard-reviewer
skpm package cncf-llm-guard-reviewer
skpm publish cncf-llm-guard-reviewer
```
