...
7. Run `skcr validate` and `skcr asps coverage cncf-opensergo-reviewer`.
8. Run `skcr compile cncf-opensergo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opensergo-reviewer`.
...
```bash
skpm validate cncf-opensergo-reviewer
skpm package cncf-opensergo-reviewer
skpm publish cncf-opensergo-reviewer
```
