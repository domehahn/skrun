...
7. Run `skcr validate` and `skcr asps coverage cncf-gocd-reviewer`.
8. Run `skcr compile cncf-gocd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gocd-reviewer`.
...
```bash
skpm validate cncf-gocd-reviewer
skpm package cncf-gocd-reviewer
skpm publish cncf-gocd-reviewer
```
