...
7. Run `skcr validate` and `skcr asps coverage cncf-packer-reviewer`.
8. Run `skcr compile cncf-packer-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-packer-reviewer`.
...
```bash
skpm validate cncf-packer-reviewer
skpm package cncf-packer-reviewer
skpm publish cncf-packer-reviewer
```
