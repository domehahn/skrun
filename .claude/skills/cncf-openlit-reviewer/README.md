...
7. Run `skcr validate` and `skcr asps coverage cncf-openlit-reviewer`.
8. Run `skcr compile cncf-openlit-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openlit-reviewer`.
...
```bash
skpm validate cncf-openlit-reviewer
skpm package cncf-openlit-reviewer
skpm publish cncf-openlit-reviewer
```
