...
7. Run `skcr validate` and `skcr asps coverage cncf-seldon-reviewer`.
8. Run `skcr compile cncf-seldon-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-seldon-reviewer`.
...
```bash
skpm validate cncf-seldon-reviewer
skpm package cncf-seldon-reviewer
skpm publish cncf-seldon-reviewer
```
