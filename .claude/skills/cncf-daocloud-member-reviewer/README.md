...
7. Run `skcr validate` and `skcr asps coverage cncf-daocloud-member-reviewer`.
8. Run `skcr compile cncf-daocloud-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-daocloud-member-reviewer`.
...
```bash
skpm validate cncf-daocloud-member-reviewer
skpm package cncf-daocloud-member-reviewer
skpm publish cncf-daocloud-member-reviewer
```
