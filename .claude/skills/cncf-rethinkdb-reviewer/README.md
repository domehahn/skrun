...
7. Run `skcr validate` and `skcr asps coverage cncf-rethinkdb-reviewer`.
8. Run `skcr compile cncf-rethinkdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rethinkdb-reviewer`.
...
```bash
skpm validate cncf-rethinkdb-reviewer
skpm package cncf-rethinkdb-reviewer
skpm publish cncf-rethinkdb-reviewer
```
