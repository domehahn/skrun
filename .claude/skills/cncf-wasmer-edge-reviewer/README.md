...
7. Run `skcr validate` and `skcr asps coverage cncf-wasmer-edge-reviewer`.
8. Run `skcr compile cncf-wasmer-edge-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasmer-edge-reviewer`.
...
```bash
skpm validate cncf-wasmer-edge-reviewer
skpm package cncf-wasmer-edge-reviewer
skpm publish cncf-wasmer-edge-reviewer
```
