...
7. Run `skcr validate` and `skcr asps coverage cncf-scalardb-reviewer`.
8. Run `skcr compile cncf-scalardb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scalardb-reviewer`.
...
```bash
skpm validate cncf-scalardb-reviewer
skpm package cncf-scalardb-reviewer
skpm publish cncf-scalardb-reviewer
```
