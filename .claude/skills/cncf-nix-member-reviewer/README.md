...
7. Run `skcr validate` and `skcr asps coverage cncf-nix-member-reviewer`.
8. Run `skcr compile cncf-nix-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nix-member-reviewer`.
...
```bash
skpm validate cncf-nix-member-reviewer
skpm package cncf-nix-member-reviewer
skpm publish cncf-nix-member-reviewer
```
