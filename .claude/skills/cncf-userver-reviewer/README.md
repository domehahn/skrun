...
7. Run `skcr validate` and `skcr asps coverage cncf-userver-reviewer`.
8. Run `skcr compile cncf-userver-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-userver-reviewer`.
...
```bash
skpm validate cncf-userver-reviewer
skpm package cncf-userver-reviewer
skpm publish cncf-userver-reviewer
```
