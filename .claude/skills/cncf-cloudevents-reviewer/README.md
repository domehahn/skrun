...
7. Run `skcr validate` and `skcr asps coverage cncf-cloudevents-reviewer`.
8. Run `skcr compile cncf-cloudevents-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cloudevents-reviewer`.
...
```bash
skpm validate cncf-cloudevents-reviewer
skpm package cncf-cloudevents-reviewer
skpm publish cncf-cloudevents-reviewer
```
