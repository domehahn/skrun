...
7. Run `skcr validate` and `skcr asps coverage cncf-clush-member-reviewer`.
8. Run `skcr compile cncf-clush-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-clush-member-reviewer`.
...
```bash
skpm validate cncf-clush-member-reviewer
skpm package cncf-clush-member-reviewer
skpm publish cncf-clush-member-reviewer
```
