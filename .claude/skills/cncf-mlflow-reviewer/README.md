...
7. Run `skcr validate` and `skcr asps coverage cncf-mlflow-reviewer`.
8. Run `skcr compile cncf-mlflow-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mlflow-reviewer`.
...
```bash
skpm validate cncf-mlflow-reviewer
skpm package cncf-mlflow-reviewer
skpm publish cncf-mlflow-reviewer
```
