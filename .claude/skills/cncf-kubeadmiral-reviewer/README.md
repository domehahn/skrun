...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeadmiral-reviewer`.
8. Run `skcr compile cncf-kubeadmiral-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeadmiral-reviewer`.
...
```bash
skpm validate cncf-kubeadmiral-reviewer
skpm package cncf-kubeadmiral-reviewer
skpm publish cncf-kubeadmiral-reviewer
```
