...
7. Run `skcr validate` and `skcr asps coverage cncf-amd-member-reviewer`.
8. Run `skcr compile cncf-amd-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-amd-member-reviewer`.
...
```bash
skpm validate cncf-amd-member-reviewer
skpm package cncf-amd-member-reviewer
skpm publish cncf-amd-member-reviewer
```
