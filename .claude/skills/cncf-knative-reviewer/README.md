...
7. Run `skcr validate` and `skcr asps coverage cncf-knative-reviewer`.
8. Run `skcr compile cncf-knative-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-knative-reviewer`.
...
```bash
skpm validate cncf-knative-reviewer
skpm package cncf-knative-reviewer
skpm publish cncf-knative-reviewer
```
