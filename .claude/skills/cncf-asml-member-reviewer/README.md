...
7. Run `skcr validate` and `skcr asps coverage cncf-asml-member-reviewer`.
8. Run `skcr compile cncf-asml-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-asml-member-reviewer`.
...
```bash
skpm validate cncf-asml-member-reviewer
skpm package cncf-asml-member-reviewer
skpm publish cncf-asml-member-reviewer
```
