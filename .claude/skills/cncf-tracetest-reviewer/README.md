...
7. Run `skcr validate` and `skcr asps coverage cncf-tracetest-reviewer`.
8. Run `skcr compile cncf-tracetest-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tracetest-reviewer`.
...
```bash
skpm validate cncf-tracetest-reviewer
skpm package cncf-tracetest-reviewer
skpm publish cncf-tracetest-reviewer
```
