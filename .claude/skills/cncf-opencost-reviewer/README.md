...
7. Run `skcr validate` and `skcr asps coverage cncf-opencost-reviewer`.
8. Run `skcr compile cncf-opencost-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opencost-reviewer`.
...
```bash
skpm validate cncf-opencost-reviewer
skpm package cncf-opencost-reviewer
skpm publish cncf-opencost-reviewer
```
