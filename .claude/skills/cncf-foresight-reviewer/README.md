...
7. Run `skcr validate` and `skcr asps coverage cncf-foresight-reviewer`.
8. Run `skcr compile cncf-foresight-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-foresight-reviewer`.
...
```bash
skpm validate cncf-foresight-reviewer
skpm package cncf-foresight-reviewer
skpm publish cncf-foresight-reviewer
```
