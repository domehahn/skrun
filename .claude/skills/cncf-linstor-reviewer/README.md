...
7. Run `skcr validate` and `skcr asps coverage cncf-linstor-reviewer`.
8. Run `skcr compile cncf-linstor-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-linstor-reviewer`.
...
```bash
skpm validate cncf-linstor-reviewer
skpm package cncf-linstor-reviewer
skpm publish cncf-linstor-reviewer
```
