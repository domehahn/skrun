...
7. Run `skcr validate` and `skcr asps coverage cncf-apioak-reviewer`.
8. Run `skcr compile cncf-apioak-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-apioak-reviewer`.
...
```bash
skpm validate cncf-apioak-reviewer
skpm package cncf-apioak-reviewer
skpm publish cncf-apioak-reviewer
```
