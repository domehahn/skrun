...
7. Run `skcr validate` and `skcr asps coverage cncf-enum-member-reviewer`.
8. Run `skcr compile cncf-enum-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-enum-member-reviewer`.
...
```bash
skpm validate cncf-enum-member-reviewer
skpm package cncf-enum-member-reviewer
skpm publish cncf-enum-member-reviewer
```
