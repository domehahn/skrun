...
7. Run `skcr validate` and `skcr asps coverage cncf-decimal-member-reviewer`.
8. Run `skcr compile cncf-decimal-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-decimal-member-reviewer`.
...
```bash
skpm validate cncf-decimal-member-reviewer
skpm package cncf-decimal-member-reviewer
skpm publish cncf-decimal-member-reviewer
```
