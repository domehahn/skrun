...
7. Run `skcr validate` and `skcr asps coverage cncf-drasi-reviewer`.
8. Run `skcr compile cncf-drasi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-drasi-reviewer`.
...
```bash
skpm validate cncf-drasi-reviewer
skpm package cncf-drasi-reviewer
skpm publish cncf-drasi-reviewer
```
