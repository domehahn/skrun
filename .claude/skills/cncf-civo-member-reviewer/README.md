...
7. Run `skcr validate` and `skcr asps coverage cncf-civo-member-reviewer`.
8. Run `skcr compile cncf-civo-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-civo-member-reviewer`.
...
```bash
skpm validate cncf-civo-member-reviewer
skpm package cncf-civo-member-reviewer
skpm publish cncf-civo-member-reviewer
```
