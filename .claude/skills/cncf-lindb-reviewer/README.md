...
7. Run `skcr validate` and `skcr asps coverage cncf-lindb-reviewer`.
8. Run `skcr compile cncf-lindb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lindb-reviewer`.
...
```bash
skpm validate cncf-lindb-reviewer
skpm package cncf-lindb-reviewer
skpm publish cncf-lindb-reviewer
```
