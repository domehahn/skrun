...
7. Run `skcr validate` and `skcr asps coverage cncf-m3-reviewer`.
8. Run `skcr compile cncf-m3-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-m3-reviewer`.
...
```bash
skpm validate cncf-m3-reviewer
skpm package cncf-m3-reviewer
skpm publish cncf-m3-reviewer
```
