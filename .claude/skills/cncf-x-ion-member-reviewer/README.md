...
7. Run `skcr validate` and `skcr asps coverage cncf-x-ion-member-reviewer`.
8. Run `skcr compile cncf-x-ion-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-x-ion-member-reviewer`.
...
```bash
skpm validate cncf-x-ion-member-reviewer
skpm package cncf-x-ion-member-reviewer
skpm publish cncf-x-ion-member-reviewer
```
