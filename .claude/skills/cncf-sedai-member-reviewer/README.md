...
7. Run `skcr validate` and `skcr asps coverage cncf-sedai-member-reviewer`.
8. Run `skcr compile cncf-sedai-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sedai-member-reviewer`.
...
```bash
skpm validate cncf-sedai-member-reviewer
skpm package cncf-sedai-member-reviewer
skpm publish cncf-sedai-member-reviewer
```
