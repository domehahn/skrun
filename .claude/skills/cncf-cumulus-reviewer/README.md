...
7. Run `skcr validate` and `skcr asps coverage cncf-cumulus-reviewer`.
8. Run `skcr compile cncf-cumulus-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cumulus-reviewer`.
...
```bash
skpm validate cncf-cumulus-reviewer
skpm package cncf-cumulus-reviewer
skpm publish cncf-cumulus-reviewer
```
