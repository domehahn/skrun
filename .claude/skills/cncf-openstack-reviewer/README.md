...
7. Run `skcr validate` and `skcr asps coverage cncf-openstack-reviewer`.
8. Run `skcr compile cncf-openstack-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openstack-reviewer`.
...
```bash
skpm validate cncf-openstack-reviewer
skpm package cncf-openstack-reviewer
skpm publish cncf-openstack-reviewer
```
