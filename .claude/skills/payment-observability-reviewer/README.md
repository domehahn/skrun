...
7. Run `skcr validate` and `skcr asps coverage payment-observability-reviewer`.
8. Run `skcr compile payment-observability-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend payment-observability-reviewer`.
...
```bash
skpm validate payment-observability-reviewer
skpm package payment-observability-reviewer
skpm publish payment-observability-reviewer
```
