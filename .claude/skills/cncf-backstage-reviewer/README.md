...
7. Run `skcr validate` and `skcr asps coverage cncf-backstage-reviewer`.
8. Run `skcr compile cncf-backstage-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-backstage-reviewer`.
...
```bash
skpm validate cncf-backstage-reviewer
skpm package cncf-backstage-reviewer
skpm publish cncf-backstage-reviewer
```
