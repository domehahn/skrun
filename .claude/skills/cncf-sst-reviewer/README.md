...
7. Run `skcr validate` and `skcr asps coverage cncf-sst-reviewer`.
8. Run `skcr compile cncf-sst-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sst-reviewer`.
...
```bash
skpm validate cncf-sst-reviewer
skpm package cncf-sst-reviewer
skpm publish cncf-sst-reviewer
```
