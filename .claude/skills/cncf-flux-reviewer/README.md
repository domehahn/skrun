...
7. Run `skcr validate` and `skcr asps coverage cncf-flux-reviewer`.
8. Run `skcr compile cncf-flux-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flux-reviewer`.
...
```bash
skpm validate cncf-flux-reviewer
skpm package cncf-flux-reviewer
skpm publish cncf-flux-reviewer
```
