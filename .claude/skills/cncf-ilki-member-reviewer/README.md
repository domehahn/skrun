...
7. Run `skcr validate` and `skcr asps coverage cncf-ilki-member-reviewer`.
8. Run `skcr compile cncf-ilki-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ilki-member-reviewer`.
...
```bash
skpm validate cncf-ilki-member-reviewer
skpm package cncf-ilki-member-reviewer
skpm publish cncf-ilki-member-reviewer
```
