...
7. Run `skcr validate` and `skcr asps coverage cncf-suse-member-reviewer`.
8. Run `skcr compile cncf-suse-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-suse-member-reviewer`.
...
```bash
skpm validate cncf-suse-member-reviewer
skpm package cncf-suse-member-reviewer
skpm publish cncf-suse-member-reviewer
```
