...
7. Run `skcr validate` and `skcr asps coverage cncf-wabt-reviewer`.
8. Run `skcr compile cncf-wabt-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wabt-reviewer`.
...
```bash
skpm validate cncf-wabt-reviewer
skpm package cncf-wabt-reviewer
skpm publish cncf-wabt-reviewer
```
