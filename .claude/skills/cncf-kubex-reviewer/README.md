...
7. Run `skcr validate` and `skcr asps coverage cncf-kubex-reviewer`.
8. Run `skcr compile cncf-kubex-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubex-reviewer`.
...
```bash
skpm validate cncf-kubex-reviewer
skpm package cncf-kubex-reviewer
skpm publish cncf-kubex-reviewer
```
