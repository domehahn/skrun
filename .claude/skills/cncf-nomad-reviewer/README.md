...
7. Run `skcr validate` and `skcr asps coverage cncf-nomad-reviewer`.
8. Run `skcr compile cncf-nomad-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nomad-reviewer`.
...
```bash
skpm validate cncf-nomad-reviewer
skpm package cncf-nomad-reviewer
skpm publish cncf-nomad-reviewer
```
