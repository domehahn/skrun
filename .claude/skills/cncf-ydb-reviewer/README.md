...
7. Run `skcr validate` and `skcr asps coverage cncf-ydb-reviewer`.
8. Run `skcr compile cncf-ydb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ydb-reviewer`.
...
```bash
skpm validate cncf-ydb-reviewer
skpm package cncf-ydb-reviewer
skpm publish cncf-ydb-reviewer
```
