...
7. Run `skcr validate` and `skcr asps coverage cncf-rkt-reviewer`.
8. Run `skcr compile cncf-rkt-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rkt-reviewer`.
...
```bash
skpm validate cncf-rkt-reviewer
skpm package cncf-rkt-reviewer
skpm publish cncf-rkt-reviewer
```
