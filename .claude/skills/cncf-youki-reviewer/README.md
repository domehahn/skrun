...
7. Run `skcr validate` and `skcr asps coverage cncf-youki-reviewer`.
8. Run `skcr compile cncf-youki-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-youki-reviewer`.
...
```bash
skpm validate cncf-youki-reviewer
skpm package cncf-youki-reviewer
skpm publish cncf-youki-reviewer
```
