...
7. Run `skcr validate` and `skcr asps coverage cncf-clickhouse-member-reviewer`.
8. Run `skcr compile cncf-clickhouse-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-clickhouse-member-reviewer`.
...
```bash
skpm validate cncf-clickhouse-member-reviewer
skpm package cncf-clickhouse-member-reviewer
skpm publish cncf-clickhouse-member-reviewer
```
