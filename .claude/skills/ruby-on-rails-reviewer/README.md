...
7. Run `skcr validate` and `skcr asps coverage ruby-on-rails-reviewer`.
8. Run `skcr compile ruby-on-rails-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend ruby-on-rails-reviewer`.
...
```bash
skpm validate ruby-on-rails-reviewer
skpm package ruby-on-rails-reviewer
skpm publish ruby-on-rails-reviewer
```
