...
7. Run `skcr validate` and `skcr asps coverage cncf-pulsar-reviewer`.
8. Run `skcr compile cncf-pulsar-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pulsar-reviewer`.
...
```bash
skpm validate cncf-pulsar-reviewer
skpm package cncf-pulsar-reviewer
skpm publish cncf-pulsar-reviewer
```
