...
7. Run `skcr validate` and `skcr asps coverage cncf-spinnaker-reviewer`.
8. Run `skcr compile cncf-spinnaker-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spinnaker-reviewer`.
...
```bash
skpm validate cncf-spinnaker-reviewer
skpm package cncf-spinnaker-reviewer
skpm publish cncf-spinnaker-reviewer
```
