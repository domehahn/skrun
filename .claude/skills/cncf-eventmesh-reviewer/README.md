...
7. Run `skcr validate` and `skcr asps coverage cncf-eventmesh-reviewer`.
8. Run `skcr compile cncf-eventmesh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-eventmesh-reviewer`.
...
```bash
skpm validate cncf-eventmesh-reviewer
skpm package cncf-eventmesh-reviewer
skpm publish cncf-eventmesh-reviewer
```
