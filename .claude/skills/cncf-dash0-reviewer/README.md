...
7. Run `skcr validate` and `skcr asps coverage cncf-dash0-reviewer`.
8. Run `skcr compile cncf-dash0-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dash0-reviewer`.
...
```bash
skpm validate cncf-dash0-reviewer
skpm package cncf-dash0-reviewer
skpm publish cncf-dash0-reviewer
```
