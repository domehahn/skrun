...
7. Run `skcr validate` and `skcr asps coverage cncf-voltdb-reviewer`.
8. Run `skcr compile cncf-voltdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-voltdb-reviewer`.
...
```bash
skpm validate cncf-voltdb-reviewer
skpm package cncf-voltdb-reviewer
skpm publish cncf-voltdb-reviewer
```
