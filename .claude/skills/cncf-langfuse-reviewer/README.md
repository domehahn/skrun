...
7. Run `skcr validate` and `skcr asps coverage cncf-langfuse-reviewer`.
8. Run `skcr compile cncf-langfuse-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-langfuse-reviewer`.
...
```bash
skpm validate cncf-langfuse-reviewer
skpm package cncf-langfuse-reviewer
skpm publish cncf-langfuse-reviewer
```
