...
7. Run `skcr validate` and `skcr asps coverage cncf-multus-reviewer`.
8. Run `skcr compile cncf-multus-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-multus-reviewer`.
...
```bash
skpm validate cncf-multus-reviewer
skpm package cncf-multus-reviewer
skpm publish cncf-multus-reviewer
```
