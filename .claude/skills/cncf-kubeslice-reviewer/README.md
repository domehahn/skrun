...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeslice-reviewer`.
8. Run `skcr compile cncf-kubeslice-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeslice-reviewer`.
...
```bash
skpm validate cncf-kubeslice-reviewer
skpm package cncf-kubeslice-reviewer
skpm publish cncf-kubeslice-reviewer
```
