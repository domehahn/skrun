...
7. Run `skcr validate` and `skcr asps coverage cncf-scylladb-reviewer`.
8. Run `skcr compile cncf-scylladb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scylladb-reviewer`.
...
```bash
skpm validate cncf-scylladb-reviewer
skpm package cncf-scylladb-reviewer
skpm publish cncf-scylladb-reviewer
```
