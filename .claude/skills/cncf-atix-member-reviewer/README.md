...
7. Run `skcr validate` and `skcr asps coverage cncf-atix-member-reviewer`.
8. Run `skcr compile cncf-atix-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-atix-member-reviewer`.
...
```bash
skpm validate cncf-atix-member-reviewer
skpm package cncf-atix-member-reviewer
skpm publish cncf-atix-member-reviewer
```
