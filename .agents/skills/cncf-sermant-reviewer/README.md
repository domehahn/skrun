...
7. Run `skcr validate` and `skcr asps coverage cncf-sermant-reviewer`.
8. Run `skcr compile cncf-sermant-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sermant-reviewer`.
...
```bash
skpm validate cncf-sermant-reviewer
skpm package cncf-sermant-reviewer
skpm publish cncf-sermant-reviewer
```
