...
7. Run `skcr validate` and `skcr asps coverage cncf-strands-reviewer`.
8. Run `skcr compile cncf-strands-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-strands-reviewer`.
...
```bash
skpm validate cncf-strands-reviewer
skpm package cncf-strands-reviewer
skpm publish cncf-strands-reviewer
```
