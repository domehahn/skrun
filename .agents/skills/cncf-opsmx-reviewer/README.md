...
7. Run `skcr validate` and `skcr asps coverage cncf-opsmx-reviewer`.
8. Run `skcr compile cncf-opsmx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opsmx-reviewer`.
...
```bash
skpm validate cncf-opsmx-reviewer
skpm package cncf-opsmx-reviewer
skpm publish cncf-opsmx-reviewer
```
