...
7. Run `skcr validate` and `skcr asps coverage cncf-elastisys-member-reviewer`.
8. Run `skcr compile cncf-elastisys-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-elastisys-member-reviewer`.
...
```bash
skpm validate cncf-elastisys-member-reviewer
skpm package cncf-elastisys-member-reviewer
skpm publish cncf-elastisys-member-reviewer
```
