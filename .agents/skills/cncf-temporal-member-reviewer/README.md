...
7. Run `skcr validate` and `skcr asps coverage cncf-temporal-member-reviewer`.
8. Run `skcr compile cncf-temporal-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-temporal-member-reviewer`.
...
```bash
skpm validate cncf-temporal-member-reviewer
skpm package cncf-temporal-member-reviewer
skpm publish cncf-temporal-member-reviewer
```
