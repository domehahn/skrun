...
7. Run `skcr validate` and `skcr asps coverage cncf-kuasar-reviewer`.
8. Run `skcr compile cncf-kuasar-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kuasar-reviewer`.
...
```bash
skpm validate cncf-kuasar-reviewer
skpm package cncf-kuasar-reviewer
skpm publish cncf-kuasar-reviewer
```
