...
7. Run `skcr validate` and `skcr asps coverage cncf-gear-reviewer`.
8. Run `skcr compile cncf-gear-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gear-reviewer`.
...
```bash
skpm validate cncf-gear-reviewer
skpm package cncf-gear-reviewer
skpm publish cncf-gear-reviewer
```
