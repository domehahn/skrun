...
7. Run `skcr validate` and `skcr asps coverage cncf-permify-reviewer`.
8. Run `skcr compile cncf-permify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-permify-reviewer`.
...
```bash
skpm validate cncf-permify-reviewer
skpm package cncf-permify-reviewer
skpm publish cncf-permify-reviewer
```
