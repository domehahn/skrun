...
7. Run `skcr validate` and `skcr asps coverage cncf-redpanda-member-reviewer`.
8. Run `skcr compile cncf-redpanda-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-redpanda-member-reviewer`.
...
```bash
skpm validate cncf-redpanda-member-reviewer
skpm package cncf-redpanda-member-reviewer
skpm publish cncf-redpanda-member-reviewer
```
