...
7. Run `skcr validate` and `skcr asps coverage cncf-doit-kcsp-reviewer`.
8. Run `skcr compile cncf-doit-kcsp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-doit-kcsp-reviewer`.
...
```bash
skpm validate cncf-doit-kcsp-reviewer
skpm package cncf-doit-kcsp-reviewer
skpm publish cncf-doit-kcsp-reviewer
```
