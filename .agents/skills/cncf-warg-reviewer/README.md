...
7. Run `skcr validate` and `skcr asps coverage cncf-warg-reviewer`.
8. Run `skcr compile cncf-warg-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-warg-reviewer`.
...
```bash
skpm validate cncf-warg-reviewer
skpm package cncf-warg-reviewer
skpm publish cncf-warg-reviewer
```
