...
7. Run `skcr validate` and `skcr asps coverage cncf-curvine-reviewer`.
8. Run `skcr compile cncf-curvine-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-curvine-reviewer`.
...
```bash
skpm validate cncf-curvine-reviewer
skpm package cncf-curvine-reviewer
skpm publish cncf-curvine-reviewer
```
