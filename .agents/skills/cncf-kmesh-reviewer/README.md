...
7. Run `skcr validate` and `skcr asps coverage cncf-kmesh-reviewer`.
8. Run `skcr compile cncf-kmesh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kmesh-reviewer`.
...
```bash
skpm validate cncf-kmesh-reviewer
skpm package cncf-kmesh-reviewer
skpm publish cncf-kmesh-reviewer
```
