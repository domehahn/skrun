...
7. Run `skcr validate` and `skcr asps coverage cncf-3scale-reviewer`.
8. Run `skcr compile cncf-3scale-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-3scale-reviewer`.
...
```bash
skpm validate cncf-3scale-reviewer
skpm package cncf-3scale-reviewer
skpm publish cncf-3scale-reviewer
```
