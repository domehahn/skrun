...
7. Run `skcr validate` and `skcr asps coverage cncf-kuma-reviewer`.
8. Run `skcr compile cncf-kuma-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kuma-reviewer`.
...
```bash
skpm validate cncf-kuma-reviewer
skpm package cncf-kuma-reviewer
skpm publish cncf-kuma-reviewer
```
