...
7. Run `skcr validate` and `skcr asps coverage cncf-redis-reviewer`.
8. Run `skcr compile cncf-redis-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-redis-reviewer`.
...
```bash
skpm validate cncf-redis-reviewer
skpm package cncf-redis-reviewer
skpm publish cncf-redis-reviewer
```
