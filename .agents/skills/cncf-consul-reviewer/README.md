...
7. Run `skcr validate` and `skcr asps coverage cncf-consul-reviewer`.
8. Run `skcr compile cncf-consul-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-consul-reviewer`.
...
```bash
skpm validate cncf-consul-reviewer
skpm package cncf-consul-reviewer
skpm publish cncf-consul-reviewer
```
