...
7. Run `skcr validate` and `skcr asps coverage cncf-pomerium-reviewer`.
8. Run `skcr compile cncf-pomerium-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pomerium-reviewer`.
...
```bash
skpm validate cncf-pomerium-reviewer
skpm package cncf-pomerium-reviewer
skpm publish cncf-pomerium-reviewer
```
