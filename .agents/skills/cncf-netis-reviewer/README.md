...
7. Run `skcr validate` and `skcr asps coverage cncf-netis-reviewer`.
8. Run `skcr compile cncf-netis-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-netis-reviewer`.
...
```bash
skpm validate cncf-netis-reviewer
skpm package cncf-netis-reviewer
skpm publish cncf-netis-reviewer
```
