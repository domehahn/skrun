...
7. Run `skcr validate` and `skcr asps coverage cncf-opentracing-reviewer`.
8. Run `skcr compile cncf-opentracing-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opentracing-reviewer`.
...
```bash
skpm validate cncf-opentracing-reviewer
skpm package cncf-opentracing-reviewer
skpm publish cncf-opentracing-reviewer
```
