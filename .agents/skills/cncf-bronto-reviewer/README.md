...
7. Run `skcr validate` and `skcr asps coverage cncf-bronto-reviewer`.
8. Run `skcr compile cncf-bronto-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bronto-reviewer`.
...
```bash
skpm validate cncf-bronto-reviewer
skpm package cncf-bronto-reviewer
skpm publish cncf-bronto-reviewer
```
