...
7. Run `skcr validate` and `skcr asps coverage cncf-axolotl-reviewer`.
8. Run `skcr compile cncf-axolotl-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-axolotl-reviewer`.
...
```bash
skpm validate cncf-axolotl-reviewer
skpm package cncf-axolotl-reviewer
skpm publish cncf-axolotl-reviewer
```
