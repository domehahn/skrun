...
7. Run `skcr validate` and `skcr asps coverage cncf-node-lambda-reviewer`.
8. Run `skcr compile cncf-node-lambda-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-node-lambda-reviewer`.
...
```bash
skpm validate cncf-node-lambda-reviewer
skpm package cncf-node-lambda-reviewer
skpm publish cncf-node-lambda-reviewer
```
