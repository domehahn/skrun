...
7. Run `skcr validate` and `skcr asps coverage cncf-grafeas-reviewer`.
8. Run `skcr compile cncf-grafeas-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grafeas-reviewer`.
...
```bash
skpm validate cncf-grafeas-reviewer
skpm package cncf-grafeas-reviewer
skpm publish cncf-grafeas-reviewer
```
