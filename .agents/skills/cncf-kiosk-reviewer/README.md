...
7. Run `skcr validate` and `skcr asps coverage cncf-kiosk-reviewer`.
8. Run `skcr compile cncf-kiosk-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kiosk-reviewer`.
...
```bash
skpm validate cncf-kiosk-reviewer
skpm package cncf-kiosk-reviewer
skpm publish cncf-kiosk-reviewer
```
