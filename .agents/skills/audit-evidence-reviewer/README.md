...
7. Run `skcr validate` and `skcr asps coverage audit-evidence-reviewer`.
8. Run `skcr compile audit-evidence-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend audit-evidence-reviewer`.
...
```bash
skpm validate audit-evidence-reviewer
skpm package audit-evidence-reviewer
skpm publish audit-evidence-reviewer
```
