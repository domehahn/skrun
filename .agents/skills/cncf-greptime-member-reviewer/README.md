...
7. Run `skcr validate` and `skcr asps coverage cncf-greptime-member-reviewer`.
8. Run `skcr compile cncf-greptime-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-greptime-member-reviewer`.
...
```bash
skpm validate cncf-greptime-member-reviewer
skpm package cncf-greptime-member-reviewer
skpm publish cncf-greptime-member-reviewer
```
