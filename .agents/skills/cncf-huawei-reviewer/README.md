...
7. Run `skcr validate` and `skcr asps coverage cncf-huawei-reviewer`.
8. Run `skcr compile cncf-huawei-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-huawei-reviewer`.
...
```bash
skpm validate cncf-huawei-reviewer
skpm package cncf-huawei-reviewer
skpm publish cncf-huawei-reviewer
```
