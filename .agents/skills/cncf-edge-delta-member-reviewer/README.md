...
7. Run `skcr validate` and `skcr asps coverage cncf-edge-delta-member-reviewer`.
8. Run `skcr compile cncf-edge-delta-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-edge-delta-member-reviewer`.
...
```bash
skpm validate cncf-edge-delta-member-reviewer
skpm package cncf-edge-delta-member-reviewer
skpm publish cncf-edge-delta-member-reviewer
```
