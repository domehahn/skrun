...
7. Run `skcr validate` and `skcr asps coverage cncf-xl-deploy-reviewer`.
8. Run `skcr compile cncf-xl-deploy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-xl-deploy-reviewer`.
...
```bash
skpm validate cncf-xl-deploy-reviewer
skpm package cncf-xl-deploy-reviewer
skpm publish cncf-xl-deploy-reviewer
```
