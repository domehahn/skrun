...
7. Run `skcr validate` and `skcr asps coverage cncf-llama-cpp-reviewer`.
8. Run `skcr compile cncf-llama-cpp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llama-cpp-reviewer`.
...
```bash
skpm validate cncf-llama-cpp-reviewer
skpm package cncf-llama-cpp-reviewer
skpm publish cncf-llama-cpp-reviewer
```
