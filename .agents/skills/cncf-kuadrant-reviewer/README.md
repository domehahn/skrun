...
7. Run `skcr validate` and `skcr asps coverage cncf-kuadrant-reviewer`.
8. Run `skcr compile cncf-kuadrant-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kuadrant-reviewer`.
...
```bash
skpm validate cncf-kuadrant-reviewer
skpm package cncf-kuadrant-reviewer
skpm publish cncf-kuadrant-reviewer
```
