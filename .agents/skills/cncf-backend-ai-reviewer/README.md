...
7. Run `skcr validate` and `skcr asps coverage cncf-backend-ai-reviewer`.
8. Run `skcr compile cncf-backend-ai-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-backend-ai-reviewer`.
...
```bash
skpm validate cncf-backend-ai-reviewer
skpm package cncf-backend-ai-reviewer
skpm publish cncf-backend-ai-reviewer
```
