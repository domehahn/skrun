...
7. Run `skcr validate` and `skcr asps coverage cncf-kui-reviewer`.
8. Run `skcr compile cncf-kui-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kui-reviewer`.
...
```bash
skpm validate cncf-kui-reviewer
skpm package cncf-kui-reviewer
skpm publish cncf-kui-reviewer
```
