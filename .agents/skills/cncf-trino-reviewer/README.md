...
7. Run `skcr validate` and `skcr asps coverage cncf-trino-reviewer`.
8. Run `skcr compile cncf-trino-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-trino-reviewer`.
...
```bash
skpm validate cncf-trino-reviewer
skpm package cncf-trino-reviewer
skpm publish cncf-trino-reviewer
```
