...
7. Run `skcr validate` and `skcr asps coverage cncf-vercel-reviewer`.
8. Run `skcr compile cncf-vercel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vercel-reviewer`.
...
```bash
skpm validate cncf-vercel-reviewer
skpm package cncf-vercel-reviewer
skpm publish cncf-vercel-reviewer
```
