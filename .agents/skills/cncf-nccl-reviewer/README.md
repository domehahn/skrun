...
7. Run `skcr validate` and `skcr asps coverage cncf-nccl-reviewer`.
8. Run `skcr compile cncf-nccl-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nccl-reviewer`.
...
```bash
skpm validate cncf-nccl-reviewer
skpm package cncf-nccl-reviewer
skpm publish cncf-nccl-reviewer
```
