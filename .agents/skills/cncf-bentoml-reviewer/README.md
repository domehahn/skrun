...
7. Run `skcr validate` and `skcr asps coverage cncf-bentoml-reviewer`.
8. Run `skcr compile cncf-bentoml-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bentoml-reviewer`.
...
```bash
skpm validate cncf-bentoml-reviewer
skpm package cncf-bentoml-reviewer
skpm publish cncf-bentoml-reviewer
```
