...
7. Run `skcr validate` and `skcr asps coverage cncf-influxdata-reviewer`.
8. Run `skcr compile cncf-influxdata-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-influxdata-reviewer`.
...
```bash
skpm validate cncf-influxdata-reviewer
skpm package cncf-influxdata-reviewer
skpm publish cncf-influxdata-reviewer
```
