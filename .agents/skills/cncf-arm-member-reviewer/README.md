...
7. Run `skcr validate` and `skcr asps coverage cncf-arm-member-reviewer`.
8. Run `skcr compile cncf-arm-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-arm-member-reviewer`.
...
```bash
skpm validate cncf-arm-member-reviewer
skpm package cncf-arm-member-reviewer
skpm publish cncf-arm-member-reviewer
```
