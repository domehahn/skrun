...
7. Run `skcr validate` and `skcr asps coverage cncf-serverless-reviewer`.
8. Run `skcr compile cncf-serverless-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-serverless-reviewer`.
...
```bash
skpm validate cncf-serverless-reviewer
skpm package cncf-serverless-reviewer
skpm publish cncf-serverless-reviewer
```
