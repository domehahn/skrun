...
7. Run `skcr validate` and `skcr asps coverage cncf-rook-reviewer`.
8. Run `skcr compile cncf-rook-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rook-reviewer`.
...
```bash
skpm validate cncf-rook-reviewer
skpm package cncf-rook-reviewer
skpm publish cncf-rook-reviewer
```
