...
7. Run `skcr validate` and `skcr asps coverage cncf-wiz-member-reviewer`.
8. Run `skcr compile cncf-wiz-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wiz-member-reviewer`.
...
```bash
skpm validate cncf-wiz-member-reviewer
skpm package cncf-wiz-member-reviewer
skpm publish cncf-wiz-member-reviewer
```
