...
7. Run `skcr validate` and `skcr asps coverage cncf-grafana-loki-reviewer`.
8. Run `skcr compile cncf-grafana-loki-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-grafana-loki-reviewer`.
...
```bash
skpm validate cncf-grafana-loki-reviewer
skpm package cncf-grafana-loki-reviewer
skpm publish cncf-grafana-loki-reviewer
```
