...
7. Run `skcr validate` and `skcr asps coverage cncf-airlock-reviewer`.
8. Run `skcr compile cncf-airlock-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-airlock-reviewer`.
...
```bash
skpm validate cncf-airlock-reviewer
skpm package cncf-airlock-reviewer
skpm publish cncf-airlock-reviewer
```
