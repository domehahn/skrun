...
7. Run `skcr validate` and `skcr asps coverage cncf-spire-reviewer`.
8. Run `skcr compile cncf-spire-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spire-reviewer`.
...
```bash
skpm validate cncf-spire-reviewer
skpm package cncf-spire-reviewer
skpm publish cncf-spire-reviewer
```
