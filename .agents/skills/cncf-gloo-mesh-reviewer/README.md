...
7. Run `skcr validate` and `skcr asps coverage cncf-gloo-mesh-reviewer`.
8. Run `skcr compile cncf-gloo-mesh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gloo-mesh-reviewer`.
...
```bash
skpm validate cncf-gloo-mesh-reviewer
skpm package cncf-gloo-mesh-reviewer
skpm publish cncf-gloo-mesh-reviewer
```
