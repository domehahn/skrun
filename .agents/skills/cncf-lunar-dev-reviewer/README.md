...
7. Run `skcr validate` and `skcr asps coverage cncf-lunar-dev-reviewer`.
8. Run `skcr compile cncf-lunar-dev-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lunar-dev-reviewer`.
...
```bash
skpm validate cncf-lunar-dev-reviewer
skpm package cncf-lunar-dev-reviewer
skpm publish cncf-lunar-dev-reviewer
```
