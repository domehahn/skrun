...
7. Run `skcr validate` and `skcr asps coverage cncf-rudder-reviewer`.
8. Run `skcr compile cncf-rudder-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rudder-reviewer`.
...
```bash
skpm validate cncf-rudder-reviewer
skpm package cncf-rudder-reviewer
skpm publish cncf-rudder-reviewer
```
