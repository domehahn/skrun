...
7. Run `skcr validate` and `skcr asps coverage cncf-tesseral-reviewer`.
8. Run `skcr compile cncf-tesseral-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tesseral-reviewer`.
...
```bash
skpm validate cncf-tesseral-reviewer
skpm package cncf-tesseral-reviewer
skpm publish cncf-tesseral-reviewer
```
