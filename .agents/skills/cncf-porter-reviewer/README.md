...
7. Run `skcr validate` and `skcr asps coverage cncf-porter-reviewer`.
8. Run `skcr compile cncf-porter-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-porter-reviewer`.
...
```bash
skpm validate cncf-porter-reviewer
skpm package cncf-porter-reviewer
skpm publish cncf-porter-reviewer
```
