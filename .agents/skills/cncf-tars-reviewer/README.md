...
7. Run `skcr validate` and `skcr asps coverage cncf-tars-reviewer`.
8. Run `skcr compile cncf-tars-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tars-reviewer`.
...
```bash
skpm validate cncf-tars-reviewer
skpm package cncf-tars-reviewer
skpm publish cncf-tars-reviewer
```
