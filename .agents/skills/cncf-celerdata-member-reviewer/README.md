...
7. Run `skcr validate` and `skcr asps coverage cncf-celerdata-member-reviewer`.
8. Run `skcr compile cncf-celerdata-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-celerdata-member-reviewer`.
...
```bash
skpm validate cncf-celerdata-member-reviewer
skpm package cncf-celerdata-member-reviewer
skpm publish cncf-celerdata-member-reviewer
```
