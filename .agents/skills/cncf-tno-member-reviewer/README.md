...
7. Run `skcr validate` and `skcr asps coverage cncf-tno-member-reviewer`.
8. Run `skcr compile cncf-tno-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tno-member-reviewer`.
...
```bash
skpm validate cncf-tno-member-reviewer
skpm package cncf-tno-member-reviewer
skpm publish cncf-tno-member-reviewer
```
