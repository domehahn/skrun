...
7. Run `skcr validate` and `skcr asps coverage cncf-fluid-reviewer`.
8. Run `skcr compile cncf-fluid-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-fluid-reviewer`.
...
```bash
skpm validate cncf-fluid-reviewer
skpm package cncf-fluid-reviewer
skpm publish cncf-fluid-reviewer
```
