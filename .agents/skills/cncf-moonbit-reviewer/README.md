...
7. Run `skcr validate` and `skcr asps coverage cncf-moonbit-reviewer`.
8. Run `skcr compile cncf-moonbit-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-moonbit-reviewer`.
...
```bash
skpm validate cncf-moonbit-reviewer
skpm package cncf-moonbit-reviewer
skpm publish cncf-moonbit-reviewer
```
