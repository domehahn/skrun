...
7. Run `skcr validate` and `skcr asps coverage cncf-presto-reviewer`.
8. Run `skcr compile cncf-presto-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-presto-reviewer`.
...
```bash
skpm validate cncf-presto-reviewer
skpm package cncf-presto-reviewer
skpm publish cncf-presto-reviewer
```
