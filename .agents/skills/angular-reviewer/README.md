...
7. Run `skcr validate` and `skcr asps coverage angular-reviewer`.
8. Run `skcr compile angular-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend angular-reviewer`.
...
```bash
skpm validate angular-reviewer
skpm package angular-reviewer
skpm publish angular-reviewer
```
