...
7. Run `skcr validate` and `skcr asps coverage cncf-cisco-member-reviewer`.
8. Run `skcr compile cncf-cisco-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cisco-member-reviewer`.
...
```bash
skpm validate cncf-cisco-member-reviewer
skpm package cncf-cisco-member-reviewer
skpm publish cncf-cisco-member-reviewer
```
