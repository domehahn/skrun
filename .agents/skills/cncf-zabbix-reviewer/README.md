...
7. Run `skcr validate` and `skcr asps coverage cncf-zabbix-reviewer`.
8. Run `skcr compile cncf-zabbix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zabbix-reviewer`.
...
```bash
skpm validate cncf-zabbix-reviewer
skpm package cncf-zabbix-reviewer
skpm publish cncf-zabbix-reviewer
```
