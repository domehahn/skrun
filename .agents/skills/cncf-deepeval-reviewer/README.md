...
7. Run `skcr validate` and `skcr asps coverage cncf-deepeval-reviewer`.
8. Run `skcr compile cncf-deepeval-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-deepeval-reviewer`.
...
```bash
skpm validate cncf-deepeval-reviewer
skpm package cncf-deepeval-reviewer
skpm publish cncf-deepeval-reviewer
```
