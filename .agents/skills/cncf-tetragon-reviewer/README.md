...
7. Run `skcr validate` and `skcr asps coverage cncf-tetragon-reviewer`.
8. Run `skcr compile cncf-tetragon-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tetragon-reviewer`.
...
```bash
skpm validate cncf-tetragon-reviewer
skpm package cncf-tetragon-reviewer
skpm publish cncf-tetragon-reviewer
```
