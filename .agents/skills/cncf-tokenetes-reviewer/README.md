...
7. Run `skcr validate` and `skcr asps coverage cncf-tokenetes-reviewer`.
8. Run `skcr compile cncf-tokenetes-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tokenetes-reviewer`.
...
```bash
skpm validate cncf-tokenetes-reviewer
skpm package cncf-tokenetes-reviewer
skpm publish cncf-tokenetes-reviewer
```
