...
7. Run `skcr validate` and `skcr asps coverage cncf-rust-reviewer`.
8. Run `skcr compile cncf-rust-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rust-reviewer`.
...
```bash
skpm validate cncf-rust-reviewer
skpm package cncf-rust-reviewer
skpm publish cncf-rust-reviewer
```
