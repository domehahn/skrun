...
7. Run `skcr validate` and `skcr asps coverage cncf-hubble-reviewer`.
8. Run `skcr compile cncf-hubble-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hubble-reviewer`.
...
```bash
skpm validate cncf-hubble-reviewer
skpm package cncf-hubble-reviewer
skpm publish cncf-hubble-reviewer
```
