...
7. Run `skcr validate` and `skcr asps coverage cncf-optuna-reviewer`.
8. Run `skcr compile cncf-optuna-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-optuna-reviewer`.
...
```bash
skpm validate cncf-optuna-reviewer
skpm package cncf-optuna-reviewer
skpm publish cncf-optuna-reviewer
```
