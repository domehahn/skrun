...
7. Run `skcr validate` and `skcr asps coverage cncf-openfga-reviewer`.
8. Run `skcr compile cncf-openfga-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openfga-reviewer`.
...
```bash
skpm validate cncf-openfga-reviewer
skpm package cncf-openfga-reviewer
skpm publish cncf-openfga-reviewer
```
