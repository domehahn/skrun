...
7. Run `skcr validate` and `skcr asps coverage cncf-rundeck-reviewer`.
8. Run `skcr compile cncf-rundeck-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rundeck-reviewer`.
...
```bash
skpm validate cncf-rundeck-reviewer
skpm package cncf-rundeck-reviewer
skpm publish cncf-rundeck-reviewer
```
