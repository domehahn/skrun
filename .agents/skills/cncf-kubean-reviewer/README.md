...
7. Run `skcr validate` and `skcr asps coverage cncf-kubean-reviewer`.
8. Run `skcr compile cncf-kubean-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubean-reviewer`.
...
```bash
skpm validate cncf-kubean-reviewer
skpm package cncf-kubean-reviewer
skpm publish cncf-kubean-reviewer
```
