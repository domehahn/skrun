...
7. Run `skcr validate` and `skcr asps coverage cncf-pgedge-member-reviewer`.
8. Run `skcr compile cncf-pgedge-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pgedge-member-reviewer`.
...
```bash
skpm validate cncf-pgedge-member-reviewer
skpm package cncf-pgedge-member-reviewer
skpm publish cncf-pgedge-member-reviewer
```
