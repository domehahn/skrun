...
7. Run `skcr validate` and `skcr asps coverage cncf-devfile-reviewer`.
8. Run `skcr compile cncf-devfile-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-devfile-reviewer`.
...
```bash
skpm validate cncf-devfile-reviewer
skpm package cncf-devfile-reviewer
skpm publish cncf-devfile-reviewer
```
