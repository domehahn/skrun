...
7. Run `skcr validate` and `skcr asps coverage cncf-openrun-reviewer`.
8. Run `skcr compile cncf-openrun-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openrun-reviewer`.
...
```bash
skpm validate cncf-openrun-reviewer
skpm package cncf-openrun-reviewer
skpm publish cncf-openrun-reviewer
```
