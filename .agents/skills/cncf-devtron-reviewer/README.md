...
7. Run `skcr validate` and `skcr asps coverage cncf-devtron-reviewer`.
8. Run `skcr compile cncf-devtron-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-devtron-reviewer`.
...
```bash
skpm validate cncf-devtron-reviewer
skpm package cncf-devtron-reviewer
skpm publish cncf-devtron-reviewer
```
