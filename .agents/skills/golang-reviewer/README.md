...
7. Run `skcr validate` and `skcr asps coverage golang-reviewer`.
8. Run `skcr compile golang-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend golang-reviewer`.
...
```bash
skpm validate golang-reviewer
skpm package golang-reviewer
skpm publish golang-reviewer
```
