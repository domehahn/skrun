...
7. Run `skcr validate` and `skcr asps coverage cncf-seata-reviewer`.
8. Run `skcr compile cncf-seata-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-seata-reviewer`.
...
```bash
skpm validate cncf-seata-reviewer
skpm package cncf-seata-reviewer
skpm publish cncf-seata-reviewer
```
