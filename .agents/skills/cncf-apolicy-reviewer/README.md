...
7. Run `skcr validate` and `skcr asps coverage cncf-apolicy-reviewer`.
8. Run `skcr compile cncf-apolicy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-apolicy-reviewer`.
...
```bash
skpm validate cncf-apolicy-reviewer
skpm package cncf-apolicy-reviewer
skpm publish cncf-apolicy-reviewer
```
