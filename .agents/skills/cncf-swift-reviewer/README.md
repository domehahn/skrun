...
7. Run `skcr validate` and `skcr asps coverage cncf-swift-reviewer`.
8. Run `skcr compile cncf-swift-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-swift-reviewer`.
...
```bash
skpm validate cncf-swift-reviewer
skpm package cncf-swift-reviewer
skpm publish cncf-swift-reviewer
```
