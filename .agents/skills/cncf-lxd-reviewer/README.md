...
7. Run `skcr validate` and `skcr asps coverage cncf-lxd-reviewer`.
8. Run `skcr compile cncf-lxd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lxd-reviewer`.
...
```bash
skpm validate cncf-lxd-reviewer
skpm package cncf-lxd-reviewer
skpm publish cncf-lxd-reviewer
```
