...
7. Run `skcr validate` and `skcr asps coverage cncf-nvidia-member-reviewer`.
8. Run `skcr compile cncf-nvidia-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nvidia-member-reviewer`.
...
```bash
skpm validate cncf-nvidia-member-reviewer
skpm package cncf-nvidia-member-reviewer
skpm publish cncf-nvidia-member-reviewer
```
