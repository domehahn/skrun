...
7. Run `skcr validate` and `skcr asps coverage cncf-naic-member-reviewer`.
8. Run `skcr compile cncf-naic-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-naic-member-reviewer`.
...
```bash
skpm validate cncf-naic-member-reviewer
skpm package cncf-naic-member-reviewer
skpm publish cncf-naic-member-reviewer
```
