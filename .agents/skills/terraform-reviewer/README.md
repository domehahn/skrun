...
7. Run `skcr validate` and `skcr asps coverage terraform-reviewer`.
8. Run `skcr compile terraform-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend terraform-reviewer`.
...
```bash
skpm validate terraform-reviewer
skpm package terraform-reviewer
skpm publish terraform-reviewer
```
