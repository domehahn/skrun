...
7. Run `skcr validate` and `skcr asps coverage cncf-gonzo-reviewer`.
8. Run `skcr compile cncf-gonzo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gonzo-reviewer`.
...
```bash
skpm validate cncf-gonzo-reviewer
skpm package cncf-gonzo-reviewer
skpm publish cncf-gonzo-reviewer
```
