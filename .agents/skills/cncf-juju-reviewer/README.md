...
7. Run `skcr validate` and `skcr asps coverage cncf-juju-reviewer`.
8. Run `skcr compile cncf-juju-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-juju-reviewer`.
...
```bash
skpm validate cncf-juju-reviewer
skpm package cncf-juju-reviewer
skpm publish cncf-juju-reviewer
```
