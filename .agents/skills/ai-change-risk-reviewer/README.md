...
7. Run `skcr validate` and `skcr asps coverage ai-change-risk-reviewer`.
8. Run `skcr compile ai-change-risk-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend ai-change-risk-reviewer`.
...
```bash
skpm validate ai-change-risk-reviewer
skpm package ai-change-risk-reviewer
skpm publish ai-change-risk-reviewer
```
