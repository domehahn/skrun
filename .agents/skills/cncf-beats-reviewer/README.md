...
7. Run `skcr validate` and `skcr asps coverage cncf-beats-reviewer`.
8. Run `skcr compile cncf-beats-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-beats-reviewer`.
...
```bash
skpm validate cncf-beats-reviewer
skpm package cncf-beats-reviewer
skpm publish cncf-beats-reviewer
```
