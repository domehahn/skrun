...
7. Run `skcr validate` and `skcr asps coverage cncf-hyperlight-reviewer`.
8. Run `skcr compile cncf-hyperlight-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hyperlight-reviewer`.
...
```bash
skpm validate cncf-hyperlight-reviewer
skpm package cncf-hyperlight-reviewer
skpm publish cncf-hyperlight-reviewer
```
