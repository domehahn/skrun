...
7. Run `skcr validate` and `skcr asps coverage cncf-ardc-member-reviewer`.
8. Run `skcr compile cncf-ardc-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ardc-member-reviewer`.
...
```bash
skpm validate cncf-ardc-member-reviewer
skpm package cncf-ardc-member-reviewer
skpm publish cncf-ardc-member-reviewer
```
