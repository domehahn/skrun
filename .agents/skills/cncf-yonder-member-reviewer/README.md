...
7. Run `skcr validate` and `skcr asps coverage cncf-yonder-member-reviewer`.
8. Run `skcr compile cncf-yonder-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yonder-member-reviewer`.
...
```bash
skpm validate cncf-yonder-member-reviewer
skpm package cncf-yonder-member-reviewer
skpm publish cncf-yonder-member-reviewer
```
