...
7. Run `skcr validate` and `skcr asps coverage cncf-kubescape-reviewer`.
8. Run `skcr compile cncf-kubescape-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubescape-reviewer`.
...
```bash
skpm validate cncf-kubescape-reviewer
skpm package cncf-kubescape-reviewer
skpm publish cncf-kubescape-reviewer
```
