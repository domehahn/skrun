...
7. Run `skcr validate` and `skcr asps coverage cncf-checkly-member-reviewer`.
8. Run `skcr compile cncf-checkly-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-checkly-member-reviewer`.
...
```bash
skpm validate cncf-checkly-member-reviewer
skpm package cncf-checkly-member-reviewer
skpm publish cncf-checkly-member-reviewer
```
