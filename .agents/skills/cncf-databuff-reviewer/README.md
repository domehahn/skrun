...
7. Run `skcr validate` and `skcr asps coverage cncf-databuff-reviewer`.
8. Run `skcr compile cncf-databuff-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-databuff-reviewer`.
...
```bash
skpm validate cncf-databuff-reviewer
skpm package cncf-databuff-reviewer
skpm publish cncf-databuff-reviewer
```
