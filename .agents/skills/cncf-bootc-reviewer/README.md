...
7. Run `skcr validate` and `skcr asps coverage cncf-bootc-reviewer`.
8. Run `skcr compile cncf-bootc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bootc-reviewer`.
...
```bash
skpm validate cncf-bootc-reviewer
skpm package cncf-bootc-reviewer
skpm publish cncf-bootc-reviewer
```
