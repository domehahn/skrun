...
7. Run `skcr validate` and `skcr asps coverage cncf-mackerel-reviewer`.
8. Run `skcr compile cncf-mackerel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mackerel-reviewer`.
...
```bash
skpm validate cncf-mackerel-reviewer
skpm package cncf-mackerel-reviewer
skpm publish cncf-mackerel-reviewer
```
