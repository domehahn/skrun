...
7. Run `skcr validate` and `skcr asps coverage cncf-pipelineai-reviewer`.
8. Run `skcr compile cncf-pipelineai-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pipelineai-reviewer`.
...
```bash
skpm validate cncf-pipelineai-reviewer
skpm package cncf-pipelineai-reviewer
skpm publish cncf-pipelineai-reviewer
```
