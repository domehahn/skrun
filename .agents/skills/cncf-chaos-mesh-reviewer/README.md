...
7. Run `skcr validate` and `skcr asps coverage cncf-chaos-mesh-reviewer`.
8. Run `skcr compile cncf-chaos-mesh-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-chaos-mesh-reviewer`.
...
```bash
skpm validate cncf-chaos-mesh-reviewer
skpm package cncf-chaos-mesh-reviewer
skpm publish cncf-chaos-mesh-reviewer
```
