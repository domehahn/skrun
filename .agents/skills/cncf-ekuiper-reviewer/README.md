...
7. Run `skcr validate` and `skcr asps coverage cncf-ekuiper-reviewer`.
8. Run `skcr compile cncf-ekuiper-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ekuiper-reviewer`.
...
```bash
skpm validate cncf-ekuiper-reviewer
skpm package cncf-ekuiper-reviewer
skpm publish cncf-ekuiper-reviewer
```
