...
7. Run `skcr validate` and `skcr asps coverage cncf-pipy-reviewer`.
8. Run `skcr compile cncf-pipy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pipy-reviewer`.
...
```bash
skpm validate cncf-pipy-reviewer
skpm package cncf-pipy-reviewer
skpm publish cncf-pipy-reviewer
```
