...
7. Run `skcr validate` and `skcr asps coverage cncf-mondoo-reviewer`.
8. Run `skcr compile cncf-mondoo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mondoo-reviewer`.
...
```bash
skpm validate cncf-mondoo-reviewer
skpm package cncf-mondoo-reviewer
skpm publish cncf-mondoo-reviewer
```
