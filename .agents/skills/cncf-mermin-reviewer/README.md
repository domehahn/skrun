...
7. Run `skcr validate` and `skcr asps coverage cncf-mermin-reviewer`.
8. Run `skcr compile cncf-mermin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mermin-reviewer`.
...
```bash
skpm validate cncf-mermin-reviewer
skpm package cncf-mermin-reviewer
skpm publish cncf-mermin-reviewer
```
