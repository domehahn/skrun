...
7. Run `skcr validate` and `skcr asps coverage cncf-quobyte-reviewer`.
8. Run `skcr compile cncf-quobyte-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-quobyte-reviewer`.
...
```bash
skpm validate cncf-quobyte-reviewer
skpm package cncf-quobyte-reviewer
skpm publish cncf-quobyte-reviewer
```
