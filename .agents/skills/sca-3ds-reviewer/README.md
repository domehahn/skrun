...
7. Run `skcr validate` and `skcr asps coverage sca-3ds-reviewer`.
8. Run `skcr compile sca-3ds-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend sca-3ds-reviewer`.
...
```bash
skpm validate sca-3ds-reviewer
skpm package sca-3ds-reviewer
skpm publish sca-3ds-reviewer
```
