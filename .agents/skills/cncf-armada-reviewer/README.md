...
7. Run `skcr validate` and `skcr asps coverage cncf-armada-reviewer`.
8. Run `skcr compile cncf-armada-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-armada-reviewer`.
...
```bash
skpm validate cncf-armada-reviewer
skpm package cncf-armada-reviewer
skpm publish cncf-armada-reviewer
```
