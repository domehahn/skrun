...
7. Run `skcr validate` and `skcr asps coverage cncf-mezmo-reviewer`.
8. Run `skcr compile cncf-mezmo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mezmo-reviewer`.
...
```bash
skpm validate cncf-mezmo-reviewer
skpm package cncf-mezmo-reviewer
skpm publish cncf-mezmo-reviewer
```
