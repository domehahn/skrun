...
7. Run `skcr validate` and `skcr asps coverage cncf-appveyor-reviewer`.
8. Run `skcr compile cncf-appveyor-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-appveyor-reviewer`.
...
```bash
skpm validate cncf-appveyor-reviewer
skpm package cncf-appveyor-reviewer
skpm publish cncf-appveyor-reviewer
```
