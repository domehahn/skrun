...
7. Run `skcr validate` and `skcr asps coverage cncf-skyloud-member-reviewer`.
8. Run `skcr compile cncf-skyloud-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-skyloud-member-reviewer`.
...
```bash
skpm validate cncf-skyloud-member-reviewer
skpm package cncf-skyloud-member-reviewer
skpm publish cncf-skyloud-member-reviewer
```
