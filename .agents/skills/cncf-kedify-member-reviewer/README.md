...
7. Run `skcr validate` and `skcr asps coverage cncf-kedify-member-reviewer`.
8. Run `skcr compile cncf-kedify-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kedify-member-reviewer`.
...
```bash
skpm validate cncf-kedify-member-reviewer
skpm package cncf-kedify-member-reviewer
skpm publish cncf-kedify-member-reviewer
```
