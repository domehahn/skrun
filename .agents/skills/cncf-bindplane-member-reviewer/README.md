...
7. Run `skcr validate` and `skcr asps coverage cncf-bindplane-member-reviewer`.
8. Run `skcr compile cncf-bindplane-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bindplane-member-reviewer`.
...
```bash
skpm validate cncf-bindplane-member-reviewer
skpm package cncf-bindplane-member-reviewer
skpm publish cncf-bindplane-member-reviewer
```
