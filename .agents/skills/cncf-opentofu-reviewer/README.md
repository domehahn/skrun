...
7. Run `skcr validate` and `skcr asps coverage cncf-opentofu-reviewer`.
8. Run `skcr compile cncf-opentofu-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opentofu-reviewer`.
...
```bash
skpm validate cncf-opentofu-reviewer
skpm package cncf-opentofu-reviewer
skpm publish cncf-opentofu-reviewer
```
