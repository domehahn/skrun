...
7. Run `skcr validate` and `skcr asps coverage cncf-humio-reviewer`.
8. Run `skcr compile cncf-humio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-humio-reviewer`.
...
```bash
skpm validate cncf-humio-reviewer
skpm package cncf-humio-reviewer
skpm publish cncf-humio-reviewer
```
