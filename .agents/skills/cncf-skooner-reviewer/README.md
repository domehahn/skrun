...
7. Run `skcr validate` and `skcr asps coverage cncf-skooner-reviewer`.
8. Run `skcr compile cncf-skooner-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-skooner-reviewer`.
...
```bash
skpm validate cncf-skooner-reviewer
skpm package cncf-skooner-reviewer
skpm publish cncf-skooner-reviewer
```
