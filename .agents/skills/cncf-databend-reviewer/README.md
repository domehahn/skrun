...
7. Run `skcr validate` and `skcr asps coverage cncf-databend-reviewer`.
8. Run `skcr compile cncf-databend-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-databend-reviewer`.
...
```bash
skpm validate cncf-databend-reviewer
skpm package cncf-databend-reviewer
skpm publish cncf-databend-reviewer
```
