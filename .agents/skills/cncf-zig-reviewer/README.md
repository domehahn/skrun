...
7. Run `skcr validate` and `skcr asps coverage cncf-zig-reviewer`.
8. Run `skcr compile cncf-zig-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zig-reviewer`.
...
```bash
skpm validate cncf-zig-reviewer
skpm package cncf-zig-reviewer
skpm publish cncf-zig-reviewer
```
