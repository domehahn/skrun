...
7. Run `skcr validate` and `skcr asps coverage cncf-sofarpc-reviewer`.
8. Run `skcr compile cncf-sofarpc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sofarpc-reviewer`.
...
```bash
skpm validate cncf-sofarpc-reviewer
skpm package cncf-sofarpc-reviewer
skpm publish cncf-sofarpc-reviewer
```
