...
7. Run `skcr validate` and `skcr asps coverage cncf-oras-reviewer`.
8. Run `skcr compile cncf-oras-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-oras-reviewer`.
...
```bash
skpm validate cncf-oras-reviewer
skpm package cncf-oras-reviewer
skpm publish cncf-oras-reviewer
```
