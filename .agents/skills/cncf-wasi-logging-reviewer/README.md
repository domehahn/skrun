...
7. Run `skcr validate` and `skcr asps coverage cncf-wasi-logging-reviewer`.
8. Run `skcr compile cncf-wasi-logging-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasi-logging-reviewer`.
...
```bash
skpm validate cncf-wasi-logging-reviewer
skpm package cncf-wasi-logging-reviewer
skpm publish cncf-wasi-logging-reviewer
```
