...
7. Run `skcr validate` and `skcr asps coverage cncf-kubefwd-reviewer`.
8. Run `skcr compile cncf-kubefwd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubefwd-reviewer`.
...
```bash
skpm validate cncf-kubefwd-reviewer
skpm package cncf-kubefwd-reviewer
skpm publish cncf-kubefwd-reviewer
```
