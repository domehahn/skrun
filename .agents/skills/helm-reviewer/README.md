...
7. Run `skcr validate` and `skcr asps coverage helm-reviewer`.
8. Run `skcr compile helm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend helm-reviewer`.
...
```bash
skpm validate helm-reviewer
skpm package helm-reviewer
skpm publish helm-reviewer
```
