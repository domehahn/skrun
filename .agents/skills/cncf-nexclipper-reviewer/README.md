...
7. Run `skcr validate` and `skcr asps coverage cncf-nexclipper-reviewer`.
8. Run `skcr compile cncf-nexclipper-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nexclipper-reviewer`.
...
```bash
skpm validate cncf-nexclipper-reviewer
skpm package cncf-nexclipper-reviewer
skpm publish cncf-nexclipper-reviewer
```
