...
7. Run `skcr validate` and `skcr asps coverage cncf-maas-reviewer`.
8. Run `skcr compile cncf-maas-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-maas-reviewer`.
...
```bash
skpm validate cncf-maas-reviewer
skpm package cncf-maas-reviewer
skpm publish cncf-maas-reviewer
```
