...
7. Run `skcr validate` and `skcr asps coverage cncf-metallb-reviewer`.
8. Run `skcr compile cncf-metallb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-metallb-reviewer`.
...
```bash
skpm validate cncf-metallb-reviewer
skpm package cncf-metallb-reviewer
skpm publish cncf-metallb-reviewer
```
