...
7. Run `skcr validate` and `skcr asps coverage cncf-mysql-reviewer`.
8. Run `skcr compile cncf-mysql-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mysql-reviewer`.
...
```bash
skpm validate cncf-mysql-reviewer
skpm package cncf-mysql-reviewer
skpm publish cncf-mysql-reviewer
```
