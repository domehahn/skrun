...
7. Run `skcr validate` and `skcr asps coverage azure-cloud-reviewer`.
8. Run `skcr compile azure-cloud-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend azure-cloud-reviewer`.
...
```bash
skpm validate azure-cloud-reviewer
skpm package azure-cloud-reviewer
skpm publish azure-cloud-reviewer
```
