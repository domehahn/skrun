...
7. Run `skcr validate` and `skcr asps coverage cncf-iguazio-reviewer`.
8. Run `skcr compile cncf-iguazio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-iguazio-reviewer`.
...
```bash
skpm validate cncf-iguazio-reviewer
skpm package cncf-iguazio-reviewer
skpm publish cncf-iguazio-reviewer
```
