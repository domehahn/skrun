...
7. Run `skcr validate` and `skcr asps coverage cncf-mezmo-member-reviewer`.
8. Run `skcr compile cncf-mezmo-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mezmo-member-reviewer`.
...
```bash
skpm validate cncf-mezmo-member-reviewer
skpm package cncf-mezmo-member-reviewer
skpm publish cncf-mezmo-member-reviewer
```
