...
7. Run `skcr validate` and `skcr asps coverage cncf-llamaindex-reviewer`.
8. Run `skcr compile cncf-llamaindex-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llamaindex-reviewer`.
...
```bash
skpm validate cncf-llamaindex-reviewer
skpm package cncf-llamaindex-reviewer
skpm publish cncf-llamaindex-reviewer
```
