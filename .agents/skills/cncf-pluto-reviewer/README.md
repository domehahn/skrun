...
7. Run `skcr validate` and `skcr asps coverage cncf-pluto-reviewer`.
8. Run `skcr compile cncf-pluto-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pluto-reviewer`.
...
```bash
skpm validate cncf-pluto-reviewer
skpm package cncf-pluto-reviewer
skpm publish cncf-pluto-reviewer
```
