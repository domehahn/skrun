...
7. Run `skcr validate` and `skcr asps coverage cncf-draftt-member-reviewer`.
8. Run `skcr compile cncf-draftt-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-draftt-member-reviewer`.
...
```bash
skpm validate cncf-draftt-member-reviewer
skpm package cncf-draftt-member-reviewer
skpm publish cncf-draftt-member-reviewer
```
