...
7. Run `skcr validate` and `skcr asps coverage cncf-wasm3-reviewer`.
8. Run `skcr compile cncf-wasm3-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasm3-reviewer`.
...
```bash
skpm validate cncf-wasm3-reviewer
skpm package cncf-wasm3-reviewer
skpm publish cncf-wasm3-reviewer
```
