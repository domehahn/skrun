...
7. Run `skcr validate` and `skcr asps coverage cncf-spin-reviewer`.
8. Run `skcr compile cncf-spin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-spin-reviewer`.
...
```bash
skpm validate cncf-spin-reviewer
skpm package cncf-spin-reviewer
skpm publish cncf-spin-reviewer
```
