...
7. Run `skcr validate` and `skcr asps coverage cncf-vector-reviewer`.
8. Run `skcr compile cncf-vector-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vector-reviewer`.
...
```bash
skpm validate cncf-vector-reviewer
skpm package cncf-vector-reviewer
skpm publish cncf-vector-reviewer
```
