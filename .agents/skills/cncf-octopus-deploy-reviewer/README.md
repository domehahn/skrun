...
7. Run `skcr validate` and `skcr asps coverage cncf-octopus-deploy-reviewer`.
8. Run `skcr compile cncf-octopus-deploy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-octopus-deploy-reviewer`.
...
```bash
skpm validate cncf-octopus-deploy-reviewer
skpm package cncf-octopus-deploy-reviewer
skpm publish cncf-octopus-deploy-reviewer
```
