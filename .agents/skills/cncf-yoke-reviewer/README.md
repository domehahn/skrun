...
7. Run `skcr validate` and `skcr asps coverage cncf-yoke-reviewer`.
8. Run `skcr compile cncf-yoke-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yoke-reviewer`.
...
```bash
skpm validate cncf-yoke-reviewer
skpm package cncf-yoke-reviewer
skpm publish cncf-yoke-reviewer
```
