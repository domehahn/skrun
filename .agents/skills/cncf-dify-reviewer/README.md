...
7. Run `skcr validate` and `skcr asps coverage cncf-dify-reviewer`.
8. Run `skcr compile cncf-dify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dify-reviewer`.
...
```bash
skpm validate cncf-dify-reviewer
skpm package cncf-dify-reviewer
skpm publish cncf-dify-reviewer
```
