...
7. Run `skcr validate` and `skcr asps coverage cncf-homestar-reviewer`.
8. Run `skcr compile cncf-homestar-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-homestar-reviewer`.
...
```bash
skpm validate cncf-homestar-reviewer
skpm package cncf-homestar-reviewer
skpm publish cncf-homestar-reviewer
```
