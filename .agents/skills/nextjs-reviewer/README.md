...
7. Run `skcr validate` and `skcr asps coverage nextjs-reviewer`.
8. Run `skcr compile nextjs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend nextjs-reviewer`.
...
```bash
skpm validate nextjs-reviewer
skpm package nextjs-reviewer
skpm publish nextjs-reviewer
```
