...
7. Run `skcr validate` and `skcr asps coverage cncf-sops-reviewer`.
8. Run `skcr compile cncf-sops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sops-reviewer`.
...
```bash
skpm validate cncf-sops-reviewer
skpm package cncf-sops-reviewer
skpm publish cncf-sops-reviewer
```
