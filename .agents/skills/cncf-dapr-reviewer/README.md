...
7. Run `skcr validate` and `skcr asps coverage cncf-dapr-reviewer`.
8. Run `skcr compile cncf-dapr-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dapr-reviewer`.
...
```bash
skpm validate cncf-dapr-reviewer
skpm package cncf-dapr-reviewer
skpm publish cncf-dapr-reviewer
```
