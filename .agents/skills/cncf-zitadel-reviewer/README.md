...
7. Run `skcr validate` and `skcr asps coverage cncf-zitadel-reviewer`.
8. Run `skcr compile cncf-zitadel-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zitadel-reviewer`.
...
```bash
skpm validate cncf-zitadel-reviewer
skpm package cncf-zitadel-reviewer
skpm publish cncf-zitadel-reviewer
```
