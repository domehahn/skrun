...
7. Run `skcr validate` and `skcr asps coverage cncf-wasmtime-reviewer`.
8. Run `skcr compile cncf-wasmtime-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasmtime-reviewer`.
...
```bash
skpm validate cncf-wasmtime-reviewer
skpm package cncf-wasmtime-reviewer
skpm publish cncf-wasmtime-reviewer
```
