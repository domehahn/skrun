...
7. Run `skcr validate` and `skcr asps coverage cncf-bfe-reviewer`.
8. Run `skcr compile cncf-bfe-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-bfe-reviewer`.
...
```bash
skpm validate cncf-bfe-reviewer
skpm package cncf-bfe-reviewer
skpm publish cncf-bfe-reviewer
```
