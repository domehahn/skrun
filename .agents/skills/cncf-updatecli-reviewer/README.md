...
7. Run `skcr validate` and `skcr asps coverage cncf-updatecli-reviewer`.
8. Run `skcr compile cncf-updatecli-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-updatecli-reviewer`.
...
```bash
skpm validate cncf-updatecli-reviewer
skpm package cncf-updatecli-reviewer
skpm publish cncf-updatecli-reviewer
```
