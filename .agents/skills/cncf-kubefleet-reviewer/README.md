...
7. Run `skcr validate` and `skcr asps coverage cncf-kubefleet-reviewer`.
8. Run `skcr compile cncf-kubefleet-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubefleet-reviewer`.
...
```bash
skpm validate cncf-kubefleet-reviewer
skpm package cncf-kubefleet-reviewer
skpm publish cncf-kubefleet-reviewer
```
