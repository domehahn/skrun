...
7. Run `skcr validate` and `skcr asps coverage cncf-filecoin-reviewer`.
8. Run `skcr compile cncf-filecoin-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-filecoin-reviewer`.
...
```bash
skpm validate cncf-filecoin-reviewer
skpm package cncf-filecoin-reviewer
skpm publish cncf-filecoin-reviewer
```
