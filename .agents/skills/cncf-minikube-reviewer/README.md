...
7. Run `skcr validate` and `skcr asps coverage cncf-minikube-reviewer`.
8. Run `skcr compile cncf-minikube-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-minikube-reviewer`.
...
```bash
skpm validate cncf-minikube-reviewer
skpm package cncf-minikube-reviewer
skpm publish cncf-minikube-reviewer
```
