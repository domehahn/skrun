...
7. Run `skcr validate` and `skcr asps coverage cncf-contour-reviewer`.
8. Run `skcr compile cncf-contour-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-contour-reviewer`.
...
```bash
skpm validate cncf-contour-reviewer
skpm package cncf-contour-reviewer
skpm publish cncf-contour-reviewer
```
