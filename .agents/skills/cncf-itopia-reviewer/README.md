...
7. Run `skcr validate` and `skcr asps coverage cncf-itopia-reviewer`.
8. Run `skcr compile cncf-itopia-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-itopia-reviewer`.
...
```bash
skpm validate cncf-itopia-reviewer
skpm package cncf-itopia-reviewer
skpm publish cncf-itopia-reviewer
```
