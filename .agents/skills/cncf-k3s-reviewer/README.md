...
7. Run `skcr validate` and `skcr asps coverage cncf-k3s-reviewer`.
8. Run `skcr compile cncf-k3s-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-k3s-reviewer`.
...
```bash
skpm validate cncf-k3s-reviewer
skpm package cncf-k3s-reviewer
skpm publish cncf-k3s-reviewer
```
