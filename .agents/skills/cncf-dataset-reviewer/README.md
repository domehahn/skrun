...
7. Run `skcr validate` and `skcr asps coverage cncf-dataset-reviewer`.
8. Run `skcr compile cncf-dataset-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dataset-reviewer`.
...
```bash
skpm validate cncf-dataset-reviewer
skpm package cncf-dataset-reviewer
skpm publish cncf-dataset-reviewer
```
