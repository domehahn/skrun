...
7. Run `skcr validate` and `skcr asps coverage cncf-openelb-reviewer`.
8. Run `skcr compile cncf-openelb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openelb-reviewer`.
...
```bash
skpm validate cncf-openelb-reviewer
skpm package cncf-openelb-reviewer
skpm publish cncf-openelb-reviewer
```
