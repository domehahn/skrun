...
7. Run `skcr validate` and `skcr asps coverage cncf-hammerspace-member-reviewer`.
8. Run `skcr compile cncf-hammerspace-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hammerspace-member-reviewer`.
...
```bash
skpm validate cncf-hammerspace-member-reviewer
skpm package cncf-hammerspace-member-reviewer
skpm publish cncf-hammerspace-member-reviewer
```
