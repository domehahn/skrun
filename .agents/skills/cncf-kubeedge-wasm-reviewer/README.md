...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeedge-wasm-reviewer`.
8. Run `skcr compile cncf-kubeedge-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeedge-wasm-reviewer`.
...
```bash
skpm validate cncf-kubeedge-wasm-reviewer
skpm package cncf-kubeedge-wasm-reviewer
skpm publish cncf-kubeedge-wasm-reviewer
```
