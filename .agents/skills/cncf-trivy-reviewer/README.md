...
7. Run `skcr validate` and `skcr asps coverage cncf-trivy-reviewer`.
8. Run `skcr compile cncf-trivy-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-trivy-reviewer`.
...
```bash
skpm validate cncf-trivy-reviewer
skpm package cncf-trivy-reviewer
skpm publish cncf-trivy-reviewer
```
