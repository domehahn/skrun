...
7. Run `skcr validate` and `skcr asps coverage cncf-helm-reviewer`.
8. Run `skcr compile cncf-helm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-helm-reviewer`.
...
```bash
skpm validate cncf-helm-reviewer
skpm package cncf-helm-reviewer
skpm publish cncf-helm-reviewer
```
