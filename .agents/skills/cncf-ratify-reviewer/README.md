...
7. Run `skcr validate` and `skcr asps coverage cncf-ratify-reviewer`.
8. Run `skcr compile cncf-ratify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ratify-reviewer`.
...
```bash
skpm validate cncf-ratify-reviewer
skpm package cncf-ratify-reviewer
skpm publish cncf-ratify-reviewer
```
