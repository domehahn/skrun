...
7. Run `skcr validate` and `skcr asps coverage cncf-dell-emc-reviewer`.
8. Run `skcr compile cncf-dell-emc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dell-emc-reviewer`.
...
```bash
skpm validate cncf-dell-emc-reviewer
skpm package cncf-dell-emc-reviewer
skpm publish cncf-dell-emc-reviewer
```
