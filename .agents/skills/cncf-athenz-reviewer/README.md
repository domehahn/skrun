...
7. Run `skcr validate` and `skcr asps coverage cncf-athenz-reviewer`.
8. Run `skcr compile cncf-athenz-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-athenz-reviewer`.
...
```bash
skpm validate cncf-athenz-reviewer
skpm package cncf-athenz-reviewer
skpm publish cncf-athenz-reviewer
```
