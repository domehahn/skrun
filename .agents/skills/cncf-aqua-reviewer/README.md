...
7. Run `skcr validate` and `skcr asps coverage cncf-aqua-reviewer`.
8. Run `skcr compile cncf-aqua-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-aqua-reviewer`.
...
```bash
skpm validate cncf-aqua-reviewer
skpm package cncf-aqua-reviewer
skpm publish cncf-aqua-reviewer
```
