...
7. Run `skcr validate` and `skcr asps coverage cncf-infinispan-reviewer`.
8. Run `skcr compile cncf-infinispan-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-infinispan-reviewer`.
...
```bash
skpm validate cncf-infinispan-reviewer
skpm package cncf-infinispan-reviewer
skpm publish cncf-infinispan-reviewer
```
