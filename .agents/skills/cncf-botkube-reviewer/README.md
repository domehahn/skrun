...
7. Run `skcr validate` and `skcr asps coverage cncf-botkube-reviewer`.
8. Run `skcr compile cncf-botkube-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-botkube-reviewer`.
...
```bash
skpm validate cncf-botkube-reviewer
skpm package cncf-botkube-reviewer
skpm publish cncf-botkube-reviewer
```
