...
7. Run `skcr validate` and `skcr asps coverage cncf-openebs-reviewer`.
8. Run `skcr compile cncf-openebs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openebs-reviewer`.
...
```bash
skpm validate cncf-openebs-reviewer
skpm package cncf-openebs-reviewer
skpm publish cncf-openebs-reviewer
```
