...
7. Run `skcr validate` and `skcr asps coverage cncf-scality-ring-reviewer`.
8. Run `skcr compile cncf-scality-ring-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-scality-ring-reviewer`.
...
```bash
skpm validate cncf-scality-ring-reviewer
skpm package cncf-scality-ring-reviewer
skpm publish cncf-scality-ring-reviewer
```
