...
7. Run `skcr validate` and `skcr asps coverage cncf-mongodb-reviewer`.
8. Run `skcr compile cncf-mongodb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mongodb-reviewer`.
...
```bash
skpm validate cncf-mongodb-reviewer
skpm package cncf-mongodb-reviewer
skpm publish cncf-mongodb-reviewer
```
