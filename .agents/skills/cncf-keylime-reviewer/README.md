...
7. Run `skcr validate` and `skcr asps coverage cncf-keylime-reviewer`.
8. Run `skcr compile cncf-keylime-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-keylime-reviewer`.
...
```bash
skpm validate cncf-keylime-reviewer
skpm package cncf-keylime-reviewer
skpm publish cncf-keylime-reviewer
```
