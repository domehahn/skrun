...
7. Run `skcr validate` and `skcr asps coverage cncf-datadog-member-reviewer`.
8. Run `skcr compile cncf-datadog-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-datadog-member-reviewer`.
...
```bash
skpm validate cncf-datadog-member-reviewer
skpm package cncf-datadog-member-reviewer
skpm publish cncf-datadog-member-reviewer
```
