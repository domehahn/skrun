...
7. Run `skcr validate` and `skcr asps coverage cncf-pytorch-reviewer`.
8. Run `skcr compile cncf-pytorch-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pytorch-reviewer`.
...
```bash
skpm validate cncf-pytorch-reviewer
skpm package cncf-pytorch-reviewer
skpm publish cncf-pytorch-reviewer
```
