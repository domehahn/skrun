...
7. Run `skcr validate` and `skcr asps coverage cncf-argonix-reviewer`.
8. Run `skcr compile cncf-argonix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-argonix-reviewer`.
...
```bash
skpm validate cncf-argonix-reviewer
skpm package cncf-argonix-reviewer
skpm publish cncf-argonix-reviewer
```
