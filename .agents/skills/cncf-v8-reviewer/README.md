...
7. Run `skcr validate` and `skcr asps coverage cncf-v8-reviewer`.
8. Run `skcr compile cncf-v8-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-v8-reviewer`.
...
```bash
skpm validate cncf-v8-reviewer
skpm package cncf-v8-reviewer
skpm publish cncf-v8-reviewer
```
