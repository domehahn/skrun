...
7. Run `skcr validate` and `skcr asps coverage cncf-selenium-grid-reviewer`.
8. Run `skcr compile cncf-selenium-grid-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-selenium-grid-reviewer`.
...
```bash
skpm validate cncf-selenium-grid-reviewer
skpm package cncf-selenium-grid-reviewer
skpm publish cncf-selenium-grid-reviewer
```
