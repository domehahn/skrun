...
7. Run `skcr validate` and `skcr asps coverage gcp-cloud-reviewer`.
8. Run `skcr compile gcp-cloud-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend gcp-cloud-reviewer`.
...
```bash
skpm validate gcp-cloud-reviewer
skpm package gcp-cloud-reviewer
skpm publish gcp-cloud-reviewer
```
