...
7. Run `skcr validate` and `skcr asps coverage cncf-mergify-reviewer`.
8. Run `skcr compile cncf-mergify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-mergify-reviewer`.
...
```bash
skpm validate cncf-mergify-reviewer
skpm package cncf-mergify-reviewer
skpm publish cncf-mergify-reviewer
```
