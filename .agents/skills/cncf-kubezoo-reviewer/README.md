...
7. Run `skcr validate` and `skcr asps coverage cncf-kubezoo-reviewer`.
8. Run `skcr compile cncf-kubezoo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubezoo-reviewer`.
...
```bash
skpm validate cncf-kubezoo-reviewer
skpm package cncf-kubezoo-reviewer
skpm publish cncf-kubezoo-reviewer
```
