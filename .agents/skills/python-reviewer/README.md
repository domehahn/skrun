...
7. Run `skcr validate` and `skcr asps coverage python-reviewer`.
8. Run `skcr compile python-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend python-reviewer`.
...
```bash
skpm validate python-reviewer
skpm package python-reviewer
skpm publish python-reviewer
```
