...
7. Run `skcr validate` and `skcr asps coverage cncf-yomo-reviewer`.
8. Run `skcr compile cncf-yomo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yomo-reviewer`.
...
```bash
skpm validate cncf-yomo-reviewer
skpm package cncf-yomo-reviewer
skpm publish cncf-yomo-reviewer
```
