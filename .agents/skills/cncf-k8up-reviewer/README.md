...
7. Run `skcr validate` and `skcr asps coverage cncf-k8up-reviewer`.
8. Run `skcr compile cncf-k8up-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-k8up-reviewer`.
...
```bash
skpm validate cncf-k8up-reviewer
skpm package cncf-k8up-reviewer
skpm publish cncf-k8up-reviewer
```
