...
7. Run `skcr validate` and `skcr asps coverage rust-reviewer`.
8. Run `skcr compile rust-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend rust-reviewer`.
...
```bash
skpm validate rust-reviewer
skpm package rust-reviewer
skpm publish rust-reviewer
```
