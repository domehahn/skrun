...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeops-reviewer`.
8. Run `skcr compile cncf-kubeops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeops-reviewer`.
...
```bash
skpm validate cncf-kubeops-reviewer
skpm package cncf-kubeops-reviewer
skpm publish cncf-kubeops-reviewer
```
