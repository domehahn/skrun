...
7. Run `skcr validate` and `skcr asps coverage cncf-longhorn-reviewer`.
8. Run `skcr compile cncf-longhorn-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-longhorn-reviewer`.
...
```bash
skpm validate cncf-longhorn-reviewer
skpm package cncf-longhorn-reviewer
skpm publish cncf-longhorn-reviewer
```
