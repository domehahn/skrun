...
7. Run `skcr validate` and `skcr asps coverage cncf-istio-wasm-reviewer`.
8. Run `skcr compile cncf-istio-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-istio-wasm-reviewer`.
...
```bash
skpm validate cncf-istio-wasm-reviewer
skpm package cncf-istio-wasm-reviewer
skpm publish cncf-istio-wasm-reviewer
```
