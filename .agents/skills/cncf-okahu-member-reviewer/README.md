...
7. Run `skcr validate` and `skcr asps coverage cncf-okahu-member-reviewer`.
8. Run `skcr compile cncf-okahu-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-okahu-member-reviewer`.
...
```bash
skpm validate cncf-okahu-member-reviewer
skpm package cncf-okahu-member-reviewer
skpm publish cncf-okahu-member-reviewer
```
