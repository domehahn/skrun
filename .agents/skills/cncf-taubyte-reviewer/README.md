...
7. Run `skcr validate` and `skcr asps coverage cncf-taubyte-reviewer`.
8. Run `skcr compile cncf-taubyte-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-taubyte-reviewer`.
...
```bash
skpm validate cncf-taubyte-reviewer
skpm package cncf-taubyte-reviewer
skpm publish cncf-taubyte-reviewer
```
