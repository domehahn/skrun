...
7. Run `skcr validate` and `skcr asps coverage cncf-argo-reviewer`.
8. Run `skcr compile cncf-argo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-argo-reviewer`.
...
```bash
skpm validate cncf-argo-reviewer
skpm package cncf-argo-reviewer
skpm publish cncf-argo-reviewer
```
