...
7. Run `skcr validate` and `skcr asps coverage cncf-agent-sandbox-reviewer`.
8. Run `skcr compile cncf-agent-sandbox-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-agent-sandbox-reviewer`.
...
```bash
skpm validate cncf-agent-sandbox-reviewer
skpm package cncf-agent-sandbox-reviewer
skpm publish cncf-agent-sandbox-reviewer
```
