...
7. Run `skcr validate` and `skcr asps coverage cncf-chainloop-member-reviewer`.
8. Run `skcr compile cncf-chainloop-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-chainloop-member-reviewer`.
...
```bash
skpm validate cncf-chainloop-member-reviewer
skpm package cncf-chainloop-member-reviewer
skpm publish cncf-chainloop-member-reviewer
```
