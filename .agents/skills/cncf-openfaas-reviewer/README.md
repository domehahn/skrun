...
7. Run `skcr validate` and `skcr asps coverage cncf-openfaas-reviewer`.
8. Run `skcr compile cncf-openfaas-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-openfaas-reviewer`.
...
```bash
skpm validate cncf-openfaas-reviewer
skpm package cncf-openfaas-reviewer
skpm publish cncf-openfaas-reviewer
```
