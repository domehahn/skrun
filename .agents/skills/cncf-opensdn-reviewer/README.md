...
7. Run `skcr validate` and `skcr asps coverage cncf-opensdn-reviewer`.
8. Run `skcr compile cncf-opensdn-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opensdn-reviewer`.
...
```bash
skpm validate cncf-opensdn-reviewer
skpm package cncf-opensdn-reviewer
skpm publish cncf-opensdn-reviewer
```
