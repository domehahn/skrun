...
7. Run `skcr validate` and `skcr asps coverage cncf-nacos-reviewer`.
8. Run `skcr compile cncf-nacos-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nacos-reviewer`.
...
```bash
skpm validate cncf-nacos-reviewer
skpm package cncf-nacos-reviewer
skpm publish cncf-nacos-reviewer
```
