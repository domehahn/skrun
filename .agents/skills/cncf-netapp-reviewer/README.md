...
7. Run `skcr validate` and `skcr asps coverage cncf-netapp-reviewer`.
8. Run `skcr compile cncf-netapp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-netapp-reviewer`.
...
```bash
skpm validate cncf-netapp-reviewer
skpm package cncf-netapp-reviewer
skpm publish cncf-netapp-reviewer
```
