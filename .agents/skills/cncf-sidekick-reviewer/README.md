...
7. Run `skcr validate` and `skcr asps coverage cncf-sidekick-reviewer`.
8. Run `skcr compile cncf-sidekick-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sidekick-reviewer`.
...
```bash
skpm validate cncf-sidekick-reviewer
skpm package cncf-sidekick-reviewer
skpm publish cncf-sidekick-reviewer
```
