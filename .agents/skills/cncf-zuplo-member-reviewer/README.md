...
7. Run `skcr validate` and `skcr asps coverage cncf-zuplo-member-reviewer`.
8. Run `skcr compile cncf-zuplo-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-zuplo-member-reviewer`.
...
```bash
skpm validate cncf-zuplo-member-reviewer
skpm package cncf-zuplo-member-reviewer
skpm publish cncf-zuplo-member-reviewer
```
