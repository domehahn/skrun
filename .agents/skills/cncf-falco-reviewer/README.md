...
7. Run `skcr validate` and `skcr asps coverage cncf-falco-reviewer`.
8. Run `skcr compile cncf-falco-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-falco-reviewer`.
...
```bash
skpm validate cncf-falco-reviewer
skpm package cncf-falco-reviewer
skpm publish cncf-falco-reviewer
```
