...
7. Run `skcr validate` and `skcr asps coverage cncf-hertzbeat-reviewer`.
8. Run `skcr compile cncf-hertzbeat-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-hertzbeat-reviewer`.
...
```bash
skpm validate cncf-hertzbeat-reviewer
skpm package cncf-hertzbeat-reviewer
skpm publish cncf-hertzbeat-reviewer
```
