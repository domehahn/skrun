...
7. Run `skcr validate` and `skcr asps coverage cncf-docker-wasm-reviewer`.
8. Run `skcr compile cncf-docker-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-docker-wasm-reviewer`.
...
```bash
skpm validate cncf-docker-wasm-reviewer
skpm package cncf-docker-wasm-reviewer
skpm publish cncf-docker-wasm-reviewer
```
