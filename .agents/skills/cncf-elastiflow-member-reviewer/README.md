...
7. Run `skcr validate` and `skcr asps coverage cncf-elastiflow-member-reviewer`.
8. Run `skcr compile cncf-elastiflow-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-elastiflow-member-reviewer`.
...
```bash
skpm validate cncf-elastiflow-member-reviewer
skpm package cncf-elastiflow-member-reviewer
skpm publish cncf-elastiflow-member-reviewer
```
