...
7. Run `skcr validate` and `skcr asps coverage cncf-rbg-reviewer`.
8. Run `skcr compile cncf-rbg-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rbg-reviewer`.
...
```bash
skpm validate cncf-rbg-reviewer
skpm package cncf-rbg-reviewer
skpm publish cncf-rbg-reviewer
```
