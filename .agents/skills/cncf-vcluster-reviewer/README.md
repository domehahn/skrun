...
7. Run `skcr validate` and `skcr asps coverage cncf-vcluster-reviewer`.
8. Run `skcr compile cncf-vcluster-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vcluster-reviewer`.
...
```bash
skpm validate cncf-vcluster-reviewer
skpm package cncf-vcluster-reviewer
skpm publish cncf-vcluster-reviewer
```
