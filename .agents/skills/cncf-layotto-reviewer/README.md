...
7. Run `skcr validate` and `skcr asps coverage cncf-layotto-reviewer`.
8. Run `skcr compile cncf-layotto-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-layotto-reviewer`.
...
```bash
skpm validate cncf-layotto-reviewer
skpm package cncf-layotto-reviewer
skpm publish cncf-layotto-reviewer
```
