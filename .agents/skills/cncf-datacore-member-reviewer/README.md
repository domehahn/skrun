...
7. Run `skcr validate` and `skcr asps coverage cncf-datacore-member-reviewer`.
8. Run `skcr compile cncf-datacore-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-datacore-member-reviewer`.
...
```bash
skpm validate cncf-datacore-member-reviewer
skpm package cncf-datacore-member-reviewer
skpm publish cncf-datacore-member-reviewer
```
