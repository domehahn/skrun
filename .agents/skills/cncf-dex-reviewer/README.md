...
7. Run `skcr validate` and `skcr asps coverage cncf-dex-reviewer`.
8. Run `skcr compile cncf-dex-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dex-reviewer`.
...
```bash
skpm validate cncf-dex-reviewer
skpm package cncf-dex-reviewer
skpm publish cncf-dex-reviewer
```
