...
7. Run `skcr validate` and `skcr asps coverage cncf-memphis-reviewer`.
8. Run `skcr compile cncf-memphis-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-memphis-reviewer`.
...
```bash
skpm validate cncf-memphis-reviewer
skpm package cncf-memphis-reviewer
skpm publish cncf-memphis-reviewer
```
