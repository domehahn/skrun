...
7. Run `skcr validate` and `skcr asps coverage cncf-vald-reviewer`.
8. Run `skcr compile cncf-vald-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vald-reviewer`.
...
```bash
skpm validate cncf-vald-reviewer
skpm package cncf-vald-reviewer
skpm publish cncf-vald-reviewer
```
