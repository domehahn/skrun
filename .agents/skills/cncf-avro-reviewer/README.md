...
7. Run `skcr validate` and `skcr asps coverage cncf-avro-reviewer`.
8. Run `skcr compile cncf-avro-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-avro-reviewer`.
...
```bash
skpm validate cncf-avro-reviewer
skpm package cncf-avro-reviewer
skpm publish cncf-avro-reviewer
```
