...
7. Run `skcr validate` and `skcr asps coverage cncf-nginx-reviewer`.
8. Run `skcr compile cncf-nginx-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nginx-reviewer`.
...
```bash
skpm validate cncf-nginx-reviewer
skpm package cncf-nginx-reviewer
skpm publish cncf-nginx-reviewer
```
