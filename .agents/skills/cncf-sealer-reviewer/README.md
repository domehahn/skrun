...
7. Run `skcr validate` and `skcr asps coverage cncf-sealer-reviewer`.
8. Run `skcr compile cncf-sealer-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sealer-reviewer`.
...
```bash
skpm validate cncf-sealer-reviewer
skpm package cncf-sealer-reviewer
skpm publish cncf-sealer-reviewer
```
