...
7. Run `skcr validate` and `skcr asps coverage cncf-causely-reviewer`.
8. Run `skcr compile cncf-causely-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-causely-reviewer`.
...
```bash
skpm validate cncf-causely-reviewer
skpm package cncf-causely-reviewer
skpm publish cncf-causely-reviewer
```
