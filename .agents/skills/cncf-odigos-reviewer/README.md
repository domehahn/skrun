...
7. Run `skcr validate` and `skcr asps coverage cncf-odigos-reviewer`.
8. Run `skcr compile cncf-odigos-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-odigos-reviewer`.
...
```bash
skpm validate cncf-odigos-reviewer
skpm package cncf-odigos-reviewer
skpm publish cncf-odigos-reviewer
```
