...
7. Run `skcr validate` and `skcr asps coverage cncf-open-mpi-reviewer`.
8. Run `skcr compile cncf-open-mpi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-open-mpi-reviewer`.
...
```bash
skpm validate cncf-open-mpi-reviewer
skpm package cncf-open-mpi-reviewer
skpm publish cncf-open-mpi-reviewer
```
