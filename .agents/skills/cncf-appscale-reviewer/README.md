...
7. Run `skcr validate` and `skcr asps coverage cncf-appscale-reviewer`.
8. Run `skcr compile cncf-appscale-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-appscale-reviewer`.
...
```bash
skpm validate cncf-appscale-reviewer
skpm package cncf-appscale-reviewer
skpm publish cncf-appscale-reviewer
```
