...
7. Run `skcr validate` and `skcr asps coverage cncf-open-vswitch-reviewer`.
8. Run `skcr compile cncf-open-vswitch-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-open-vswitch-reviewer`.
...
```bash
skpm validate cncf-open-vswitch-reviewer
skpm package cncf-open-vswitch-reviewer
skpm publish cncf-open-vswitch-reviewer
```
