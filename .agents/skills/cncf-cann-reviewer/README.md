...
7. Run `skcr validate` and `skcr asps coverage cncf-cann-reviewer`.
8. Run `skcr compile cncf-cann-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cann-reviewer`.
...
```bash
skpm validate cncf-cann-reviewer
skpm package cncf-cann-reviewer
skpm publish cncf-cann-reviewer
```
