...
7. Run `skcr validate` and `skcr asps coverage cncf-rbac-lookup-reviewer`.
8. Run `skcr compile cncf-rbac-lookup-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rbac-lookup-reviewer`.
...
```bash
skpm validate cncf-rbac-lookup-reviewer
skpm package cncf-rbac-lookup-reviewer
skpm publish cncf-rbac-lookup-reviewer
```
