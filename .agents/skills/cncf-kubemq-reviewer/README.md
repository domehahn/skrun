...
7. Run `skcr validate` and `skcr asps coverage cncf-kubemq-reviewer`.
8. Run `skcr compile cncf-kubemq-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubemq-reviewer`.
...
```bash
skpm validate cncf-kubemq-reviewer
skpm package cncf-kubemq-reviewer
skpm publish cncf-kubemq-reviewer
```
