...
7. Run `skcr validate` and `skcr asps coverage cncf-dgraph-reviewer`.
8. Run `skcr compile cncf-dgraph-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dgraph-reviewer`.
...
```bash
skpm validate cncf-dgraph-reviewer
skpm package cncf-dgraph-reviewer
skpm publish cncf-dgraph-reviewer
```
