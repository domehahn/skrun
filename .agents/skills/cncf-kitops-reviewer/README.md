...
7. Run `skcr validate` and `skcr asps coverage cncf-kitops-reviewer`.
8. Run `skcr compile cncf-kitops-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kitops-reviewer`.
...
```bash
skpm validate cncf-kitops-reviewer
skpm package cncf-kitops-reviewer
skpm publish cncf-kitops-reviewer
```
