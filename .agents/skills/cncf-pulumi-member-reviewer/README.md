...
7. Run `skcr validate` and `skcr asps coverage cncf-pulumi-member-reviewer`.
8. Run `skcr compile cncf-pulumi-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pulumi-member-reviewer`.
...
```bash
skpm validate cncf-pulumi-member-reviewer
skpm package cncf-pulumi-member-reviewer
skpm publish cncf-pulumi-member-reviewer
```
