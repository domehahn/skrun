...
7. Run `skcr validate` and `skcr asps coverage cncf-wasix-reviewer`.
8. Run `skcr compile cncf-wasix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-wasix-reviewer`.
...
```bash
skpm validate cncf-wasix-reviewer
skpm package cncf-wasix-reviewer
skpm publish cncf-wasix-reviewer
```
