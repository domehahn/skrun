...
7. Run `skcr validate` and `skcr asps coverage identity-access-reviewer`.
8. Run `skcr compile identity-access-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend identity-access-reviewer`.
...
```bash
skpm validate identity-access-reviewer
skpm package identity-access-reviewer
skpm publish identity-access-reviewer
```
