...
7. Run `skcr validate` and `skcr asps coverage cncf-datadog-reviewer`.
8. Run `skcr compile cncf-datadog-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-datadog-reviewer`.
...
```bash
skpm validate cncf-datadog-reviewer
skpm package cncf-datadog-reviewer
skpm publish cncf-datadog-reviewer
```
