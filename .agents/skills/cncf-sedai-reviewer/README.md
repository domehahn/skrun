...
7. Run `skcr validate` and `skcr asps coverage cncf-sedai-reviewer`.
8. Run `skcr compile cncf-sedai-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sedai-reviewer`.
...
```bash
skpm validate cncf-sedai-reviewer
skpm package cncf-sedai-reviewer
skpm publish cncf-sedai-reviewer
```
