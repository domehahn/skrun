...
7. Run `skcr validate` and `skcr asps coverage cncf-kafka-reviewer`.
8. Run `skcr compile cncf-kafka-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kafka-reviewer`.
...
```bash
skpm validate cncf-kafka-reviewer
skpm package cncf-kafka-reviewer
skpm publish cncf-kafka-reviewer
```
