...
7. Run `skcr validate` and `skcr asps coverage spring-boot-reviewer`.
8. Run `skcr compile spring-boot-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend spring-boot-reviewer`.
...
```bash
skpm validate spring-boot-reviewer
skpm package spring-boot-reviewer
skpm publish spring-boot-reviewer
```
