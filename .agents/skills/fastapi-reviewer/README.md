...
7. Run `skcr validate` and `skcr asps coverage fastapi-reviewer`.
8. Run `skcr compile fastapi-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend fastapi-reviewer`.
...
```bash
skpm validate fastapi-reviewer
skpm package fastapi-reviewer
skpm publish fastapi-reviewer
```
