...
7. Run `skcr validate` and `skcr asps coverage cncf-tilt-reviewer`.
8. Run `skcr compile cncf-tilt-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tilt-reviewer`.
...
```bash
skpm validate cncf-tilt-reviewer
skpm package cncf-tilt-reviewer
skpm publish cncf-tilt-reviewer
```
