...
7. Run `skcr validate` and `skcr asps coverage cncf-clyso-member-reviewer`.
8. Run `skcr compile cncf-clyso-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-clyso-member-reviewer`.
...
```bash
skpm validate cncf-clyso-member-reviewer
skpm package cncf-clyso-member-reviewer
skpm publish cncf-clyso-member-reviewer
```
