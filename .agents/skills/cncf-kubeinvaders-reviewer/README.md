...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeinvaders-reviewer`.
8. Run `skcr compile cncf-kubeinvaders-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeinvaders-reviewer`.
...
```bash
skpm validate cncf-kubeinvaders-reviewer
skpm package cncf-kubeinvaders-reviewer
skpm publish cncf-kubeinvaders-reviewer
```
