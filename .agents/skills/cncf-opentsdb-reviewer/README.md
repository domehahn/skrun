...
7. Run `skcr validate` and `skcr asps coverage cncf-opentsdb-reviewer`.
8. Run `skcr compile cncf-opentsdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-opentsdb-reviewer`.
...
```bash
skpm validate cncf-opentsdb-reviewer
skpm package cncf-opentsdb-reviewer
skpm publish cncf-opentsdb-reviewer
```
