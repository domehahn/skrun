...
7. Run `skcr validate` and `skcr asps coverage cncf-kilo-reviewer`.
8. Run `skcr compile cncf-kilo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kilo-reviewer`.
...
```bash
skpm validate cncf-kilo-reviewer
skpm package cncf-kilo-reviewer
skpm publish cncf-kilo-reviewer
```
