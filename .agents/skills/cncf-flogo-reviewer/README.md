...
7. Run `skcr validate` and `skcr asps coverage cncf-flogo-reviewer`.
8. Run `skcr compile cncf-flogo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flogo-reviewer`.
...
```bash
skpm validate cncf-flogo-reviewer
skpm package cncf-flogo-reviewer
skpm publish cncf-flogo-reviewer
```
