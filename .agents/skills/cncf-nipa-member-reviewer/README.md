...
7. Run `skcr validate` and `skcr asps coverage cncf-nipa-member-reviewer`.
8. Run `skcr compile cncf-nipa-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nipa-member-reviewer`.
...
```bash
skpm validate cncf-nipa-member-reviewer
skpm package cncf-nipa-member-reviewer
skpm publish cncf-nipa-member-reviewer
```
