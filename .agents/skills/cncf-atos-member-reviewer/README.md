...
7. Run `skcr validate` and `skcr asps coverage cncf-atos-member-reviewer`.
8. Run `skcr compile cncf-atos-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-atos-member-reviewer`.
...
```bash
skpm validate cncf-atos-member-reviewer
skpm package cncf-atos-member-reviewer
skpm publish cncf-atos-member-reviewer
```
