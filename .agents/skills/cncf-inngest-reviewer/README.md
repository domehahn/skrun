...
7. Run `skcr validate` and `skcr asps coverage cncf-inngest-reviewer`.
8. Run `skcr compile cncf-inngest-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-inngest-reviewer`.
...
```bash
skpm validate cncf-inngest-reviewer
skpm package cncf-inngest-reviewer
skpm publish cncf-inngest-reviewer
```
