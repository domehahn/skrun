...
7. Run `skcr validate` and `skcr asps coverage payment-flow-tester`.
8. Run `skcr compile payment-flow-tester --target skil`.
9. Run `skcr sync` and `skcr version recommend payment-flow-tester`.
...
```bash
skpm validate payment-flow-tester
skpm package payment-flow-tester
skpm publish payment-flow-tester
```
