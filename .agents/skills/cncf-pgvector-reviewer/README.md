...
7. Run `skcr validate` and `skcr asps coverage cncf-pgvector-reviewer`.
8. Run `skcr compile cncf-pgvector-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pgvector-reviewer`.
...
```bash
skpm validate cncf-pgvector-reviewer
skpm package cncf-pgvector-reviewer
skpm publish cncf-pgvector-reviewer
```
