...
7. Run `skcr validate` and `skcr asps coverage cncf-devspace-reviewer`.
8. Run `skcr compile cncf-devspace-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-devspace-reviewer`.
...
```bash
skpm validate cncf-devspace-reviewer
skpm package cncf-devspace-reviewer
skpm publish cncf-devspace-reviewer
```
