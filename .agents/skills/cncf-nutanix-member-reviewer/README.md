...
7. Run `skcr validate` and `skcr asps coverage cncf-nutanix-member-reviewer`.
8. Run `skcr compile cncf-nutanix-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-nutanix-member-reviewer`.
...
```bash
skpm validate cncf-nutanix-member-reviewer
skpm package cncf-nutanix-member-reviewer
skpm publish cncf-nutanix-member-reviewer
```
