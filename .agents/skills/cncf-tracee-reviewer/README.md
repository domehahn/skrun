...
7. Run `skcr validate` and `skcr asps coverage cncf-tracee-reviewer`.
8. Run `skcr compile cncf-tracee-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-tracee-reviewer`.
...
```bash
skpm validate cncf-tracee-reviewer
skpm package cncf-tracee-reviewer
skpm publish cncf-tracee-reviewer
```
