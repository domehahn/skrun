...
7. Run `skcr validate` and `skcr asps coverage cncf-agno-reviewer`.
8. Run `skcr compile cncf-agno-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-agno-reviewer`.
...
```bash
skpm validate cncf-agno-reviewer
skpm package cncf-agno-reviewer
skpm publish cncf-agno-reviewer
```
