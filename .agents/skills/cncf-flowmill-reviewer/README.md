...
7. Run `skcr validate` and `skcr asps coverage cncf-flowmill-reviewer`.
8. Run `skcr compile cncf-flowmill-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flowmill-reviewer`.
...
```bash
skpm validate cncf-flowmill-reviewer
skpm package cncf-flowmill-reviewer
skpm publish cncf-flowmill-reviewer
```
