...
7. Run `skcr validate` and `skcr asps coverage cncf-cubefs-reviewer`.
8. Run `skcr compile cncf-cubefs-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cubefs-reviewer`.
...
```bash
skpm validate cncf-cubefs-reviewer
skpm package cncf-cubefs-reviewer
skpm publish cncf-cubefs-reviewer
```
