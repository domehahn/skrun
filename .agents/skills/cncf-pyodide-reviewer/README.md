...
7. Run `skcr validate` and `skcr asps coverage cncf-pyodide-reviewer`.
8. Run `skcr compile cncf-pyodide-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pyodide-reviewer`.
...
```bash
skpm validate cncf-pyodide-reviewer
skpm package cncf-pyodide-reviewer
skpm publish cncf-pyodide-reviewer
```
