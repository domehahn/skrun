...
7. Run `skcr validate` and `skcr asps coverage cncf-dynatrace-reviewer`.
8. Run `skcr compile cncf-dynatrace-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dynatrace-reviewer`.
...
```bash
skpm validate cncf-dynatrace-reviewer
skpm package cncf-dynatrace-reviewer
skpm publish cncf-dynatrace-reviewer
```
