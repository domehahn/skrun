...
7. Run `skcr validate` and `skcr asps coverage cncf-dfinity-reviewer`.
8. Run `skcr compile cncf-dfinity-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-dfinity-reviewer`.
...
```bash
skpm validate cncf-dfinity-reviewer
skpm package cncf-dfinity-reviewer
skpm publish cncf-dfinity-reviewer
```
