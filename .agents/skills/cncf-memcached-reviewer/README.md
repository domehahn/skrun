...
7. Run `skcr validate` and `skcr asps coverage cncf-memcached-reviewer`.
8. Run `skcr compile cncf-memcached-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-memcached-reviewer`.
...
```bash
skpm validate cncf-memcached-reviewer
skpm package cncf-memcached-reviewer
skpm publish cncf-memcached-reviewer
```
