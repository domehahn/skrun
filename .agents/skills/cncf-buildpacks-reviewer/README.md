...
7. Run `skcr validate` and `skcr asps coverage cncf-buildpacks-reviewer`.
8. Run `skcr compile cncf-buildpacks-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-buildpacks-reviewer`.
...
```bash
skpm validate cncf-buildpacks-reviewer
skpm package cncf-buildpacks-reviewer
skpm publish cncf-buildpacks-reviewer
```
