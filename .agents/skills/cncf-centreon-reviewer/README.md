...
7. Run `skcr validate` and `skcr asps coverage cncf-centreon-reviewer`.
8. Run `skcr compile cncf-centreon-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-centreon-reviewer`.
...
```bash
skpm validate cncf-centreon-reviewer
skpm package cncf-centreon-reviewer
skpm publish cncf-centreon-reviewer
```
