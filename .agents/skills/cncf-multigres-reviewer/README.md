...
7. Run `skcr validate` and `skcr asps coverage cncf-multigres-reviewer`.
8. Run `skcr compile cncf-multigres-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-multigres-reviewer`.
...
```bash
skpm validate cncf-multigres-reviewer
skpm package cncf-multigres-reviewer
skpm publish cncf-multigres-reviewer
```
