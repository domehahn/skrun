...
7. Run `skcr validate` and `skcr asps coverage cncf-jozu-member-reviewer`.
8. Run `skcr compile cncf-jozu-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-jozu-member-reviewer`.
...
```bash
skpm validate cncf-jozu-member-reviewer
skpm package cncf-jozu-member-reviewer
skpm publish cncf-jozu-member-reviewer
```
