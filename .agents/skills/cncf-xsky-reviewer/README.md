...
7. Run `skcr validate` and `skcr asps coverage cncf-xsky-reviewer`.
8. Run `skcr compile cncf-xsky-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-xsky-reviewer`.
...
```bash
skpm validate cncf-xsky-reviewer
skpm package cncf-xsky-reviewer
skpm publish cncf-xsky-reviewer
```
