...
7. Run `skcr validate` and `skcr asps coverage cncf-cloudeq-member-reviewer`.
8. Run `skcr compile cncf-cloudeq-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cloudeq-member-reviewer`.
...
```bash
skpm validate cncf-cloudeq-member-reviewer
skpm package cncf-cloudeq-member-reviewer
skpm publish cncf-cloudeq-member-reviewer
```
