...
7. Run `skcr validate` and `skcr asps coverage cncf-shopify-reviewer`.
8. Run `skcr compile cncf-shopify-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-shopify-reviewer`.
...
```bash
skpm validate cncf-shopify-reviewer
skpm package cncf-shopify-reviewer
skpm publish cncf-shopify-reviewer
```
