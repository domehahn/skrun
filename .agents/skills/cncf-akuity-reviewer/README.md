...
7. Run `skcr validate` and `skcr asps coverage cncf-akuity-reviewer`.
8. Run `skcr compile cncf-akuity-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-akuity-reviewer`.
...
```bash
skpm validate cncf-akuity-reviewer
skpm package cncf-akuity-reviewer
skpm publish cncf-akuity-reviewer
```
