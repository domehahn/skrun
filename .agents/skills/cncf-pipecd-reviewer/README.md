...
7. Run `skcr validate` and `skcr asps coverage cncf-pipecd-reviewer`.
8. Run `skcr compile cncf-pipecd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-pipecd-reviewer`.
...
```bash
skpm validate cncf-pipecd-reviewer
skpm package cncf-pipecd-reviewer
skpm publish cncf-pipecd-reviewer
```
