...
7. Run `skcr validate` and `skcr asps coverage cncf-foundationdb-reviewer`.
8. Run `skcr compile cncf-foundationdb-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-foundationdb-reviewer`.
...
```bash
skpm validate cncf-foundationdb-reviewer
skpm package cncf-foundationdb-reviewer
skpm publish cncf-foundationdb-reviewer
```
