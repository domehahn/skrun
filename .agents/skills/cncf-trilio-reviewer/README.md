...
7. Run `skcr validate` and `skcr asps coverage cncf-trilio-reviewer`.
8. Run `skcr compile cncf-trilio-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-trilio-reviewer`.
...
```bash
skpm validate cncf-trilio-reviewer
skpm package cncf-trilio-reviewer
skpm publish cncf-trilio-reviewer
```
