...
7. Run `skcr validate` and `skcr asps coverage cncf-lmql-reviewer`.
8. Run `skcr compile cncf-lmql-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lmql-reviewer`.
...
```bash
skpm validate cncf-lmql-reviewer
skpm package cncf-lmql-reviewer
skpm publish cncf-lmql-reviewer
```
