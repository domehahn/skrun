...
7. Run `skcr validate` and `skcr asps coverage cncf-heroku-reviewer`.
8. Run `skcr compile cncf-heroku-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-heroku-reviewer`.
...
```bash
skpm validate cncf-heroku-reviewer
skpm package cncf-heroku-reviewer
skpm publish cncf-heroku-reviewer
```
