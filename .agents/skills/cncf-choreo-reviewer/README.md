...
7. Run `skcr validate` and `skcr asps coverage cncf-choreo-reviewer`.
8. Run `skcr compile cncf-choreo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-choreo-reviewer`.
...
```bash
skpm validate cncf-choreo-reviewer
skpm package cncf-choreo-reviewer
skpm publish cncf-choreo-reviewer
```
