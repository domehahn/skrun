...
7. Run `skcr validate` and `skcr asps coverage cncf-flagger-reviewer`.
8. Run `skcr compile cncf-flagger-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-flagger-reviewer`.
...
```bash
skpm validate cncf-flagger-reviewer
skpm package cncf-flagger-reviewer
skpm publish cncf-flagger-reviewer
```
