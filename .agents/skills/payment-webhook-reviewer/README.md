...
7. Run `skcr validate` and `skcr asps coverage payment-webhook-reviewer`.
8. Run `skcr compile payment-webhook-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend payment-webhook-reviewer`.
...
```bash
skpm validate payment-webhook-reviewer
skpm package payment-webhook-reviewer
skpm publish payment-webhook-reviewer
```
