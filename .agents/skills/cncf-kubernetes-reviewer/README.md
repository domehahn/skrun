...
7. Run `skcr validate` and `skcr asps coverage cncf-kubernetes-reviewer`.
8. Run `skcr compile cncf-kubernetes-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubernetes-reviewer`.
...
```bash
skpm validate cncf-kubernetes-reviewer
skpm package cncf-kubernetes-reviewer
skpm publish cncf-kubernetes-reviewer
```
