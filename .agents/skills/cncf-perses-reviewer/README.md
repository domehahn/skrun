...
7. Run `skcr validate` and `skcr asps coverage cncf-perses-reviewer`.
8. Run `skcr compile cncf-perses-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-perses-reviewer`.
...
```bash
skpm validate cncf-perses-reviewer
skpm package cncf-perses-reviewer
skpm publish cncf-perses-reviewer
```
