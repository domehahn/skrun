...
7. Run `skcr validate` and `skcr asps coverage cncf-kwok-reviewer`.
8. Run `skcr compile cncf-kwok-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kwok-reviewer`.
...
```bash
skpm validate cncf-kwok-reviewer
skpm package cncf-kwok-reviewer
skpm publish cncf-kwok-reviewer
```
