...
7. Run `skcr validate` and `skcr asps coverage cncf-squer-member-reviewer`.
8. Run `skcr compile cncf-squer-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-squer-member-reviewer`.
...
```bash
skpm validate cncf-squer-member-reviewer
skpm package cncf-squer-member-reviewer
skpm publish cncf-squer-member-reviewer
```
