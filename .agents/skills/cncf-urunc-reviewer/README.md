...
7. Run `skcr validate` and `skcr asps coverage cncf-urunc-reviewer`.
8. Run `skcr compile cncf-urunc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-urunc-reviewer`.
...
```bash
skpm validate cncf-urunc-reviewer
skpm package cncf-urunc-reviewer
skpm publish cncf-urunc-reviewer
```
