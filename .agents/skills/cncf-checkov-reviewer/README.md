...
7. Run `skcr validate` and `skcr asps coverage cncf-checkov-reviewer`.
8. Run `skcr compile cncf-checkov-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-checkov-reviewer`.
...
```bash
skpm validate cncf-checkov-reviewer
skpm package cncf-checkov-reviewer
skpm publish cncf-checkov-reviewer
```
