...
7. Run `skcr validate` and `skcr asps coverage cncf-kubeops-member-reviewer`.
8. Run `skcr compile cncf-kubeops-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kubeops-member-reviewer`.
...
```bash
skpm validate cncf-kubeops-member-reviewer
skpm package cncf-kubeops-member-reviewer
skpm publish cncf-kubeops-member-reviewer
```
