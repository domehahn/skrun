...
7. Run `skcr validate` and `skcr asps coverage cncf-ko-reviewer`.
8. Run `skcr compile cncf-ko-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-ko-reviewer`.
...
```bash
skpm validate cncf-ko-reviewer
skpm package cncf-ko-reviewer
skpm publish cncf-ko-reviewer
```
