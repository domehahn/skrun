...
7. Run `skcr validate` and `skcr asps coverage cncf-rabbitmq-reviewer`.
8. Run `skcr compile cncf-rabbitmq-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rabbitmq-reviewer`.
...
```bash
skpm validate cncf-rabbitmq-reviewer
skpm package cncf-rabbitmq-reviewer
skpm publish cncf-rabbitmq-reviewer
```
