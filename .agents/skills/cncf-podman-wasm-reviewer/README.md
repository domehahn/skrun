...
7. Run `skcr validate` and `skcr asps coverage cncf-podman-wasm-reviewer`.
8. Run `skcr compile cncf-podman-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-podman-wasm-reviewer`.
...
```bash
skpm validate cncf-podman-wasm-reviewer
skpm package cncf-podman-wasm-reviewer
skpm publish cncf-podman-wasm-reviewer
```
