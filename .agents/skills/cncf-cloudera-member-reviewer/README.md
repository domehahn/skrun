...
7. Run `skcr validate` and `skcr asps coverage cncf-cloudera-member-reviewer`.
8. Run `skcr compile cncf-cloudera-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cloudera-member-reviewer`.
...
```bash
skpm validate cncf-cloudera-member-reviewer
skpm package cncf-cloudera-member-reviewer
skpm publish cncf-cloudera-member-reviewer
```
