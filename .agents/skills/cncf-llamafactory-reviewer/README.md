...
7. Run `skcr validate` and `skcr asps coverage cncf-llamafactory-reviewer`.
8. Run `skcr compile cncf-llamafactory-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llamafactory-reviewer`.
...
```bash
skpm validate cncf-llamafactory-reviewer
skpm package cncf-llamafactory-reviewer
skpm publish cncf-llamafactory-reviewer
```
