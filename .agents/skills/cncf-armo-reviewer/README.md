...
7. Run `skcr validate` and `skcr asps coverage cncf-armo-reviewer`.
8. Run `skcr compile cncf-armo-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-armo-reviewer`.
...
```bash
skpm validate cncf-armo-reviewer
skpm package cncf-armo-reviewer
skpm publish cncf-armo-reviewer
```
