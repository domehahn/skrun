...
7. Run `skcr validate` and `skcr asps coverage cncf-lightbend-reviewer`.
8. Run `skcr compile cncf-lightbend-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lightbend-reviewer`.
...
```bash
skpm validate cncf-lightbend-reviewer
skpm package cncf-lightbend-reviewer
skpm publish cncf-lightbend-reviewer
```
