...
7. Run `skcr validate` and `skcr asps coverage cncf-kaniko-reviewer`.
8. Run `skcr compile cncf-kaniko-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kaniko-reviewer`.
...
```bash
skpm validate cncf-kaniko-reviewer
skpm package cncf-kaniko-reviewer
skpm publish cncf-kaniko-reviewer
```
