...
7. Run `skcr validate` and `skcr asps coverage cncf-rackner-member-reviewer`.
8. Run `skcr compile cncf-rackner-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-rackner-member-reviewer`.
...
```bash
skpm validate cncf-rackner-member-reviewer
skpm package cncf-rackner-member-reviewer
skpm publish cncf-rackner-member-reviewer
```
