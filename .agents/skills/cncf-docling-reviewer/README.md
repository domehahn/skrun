...
7. Run `skcr validate` and `skcr asps coverage cncf-docling-reviewer`.
8. Run `skcr compile cncf-docling-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-docling-reviewer`.
...
```bash
skpm validate cncf-docling-reviewer
skpm package cncf-docling-reviewer
skpm publish cncf-docling-reviewer
```
