...
7. Run `skcr validate` and `skcr asps coverage cncf-kuasar-wasm-reviewer`.
8. Run `skcr compile cncf-kuasar-wasm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kuasar-wasm-reviewer`.
...
```bash
skpm validate cncf-kuasar-wasm-reviewer
skpm package cncf-kuasar-wasm-reviewer
skpm publish cncf-kuasar-wasm-reviewer
```
