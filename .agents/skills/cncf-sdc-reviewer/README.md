...
7. Run `skcr validate` and `skcr asps coverage cncf-sdc-reviewer`.
8. Run `skcr compile cncf-sdc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sdc-reviewer`.
...
```bash
skpm validate cncf-sdc-reviewer
skpm package cncf-sdc-reviewer
skpm publish cncf-sdc-reviewer
```
