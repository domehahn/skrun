...
7. Run `skcr validate` and `skcr asps coverage cncf-feast-reviewer`.
8. Run `skcr compile cncf-feast-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-feast-reviewer`.
...
```bash
skpm validate cncf-feast-reviewer
skpm package cncf-feast-reviewer
skpm publish cncf-feast-reviewer
```
