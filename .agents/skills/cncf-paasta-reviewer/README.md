...
7. Run `skcr validate` and `skcr asps coverage cncf-paasta-reviewer`.
8. Run `skcr compile cncf-paasta-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-paasta-reviewer`.
...
```bash
skpm validate cncf-paasta-reviewer
skpm package cncf-paasta-reviewer
skpm publish cncf-paasta-reviewer
```
