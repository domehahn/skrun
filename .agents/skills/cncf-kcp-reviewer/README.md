...
7. Run `skcr validate` and `skcr asps coverage cncf-kcp-reviewer`.
8. Run `skcr compile cncf-kcp-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kcp-reviewer`.
...
```bash
skpm validate cncf-kcp-reviewer
skpm package cncf-kcp-reviewer
skpm publish cncf-kcp-reviewer
```
