...
7. Run `skcr validate` and `skcr asps coverage cncf-gitlab-reviewer`.
8. Run `skcr compile cncf-gitlab-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gitlab-reviewer`.
...
```bash
skpm validate cncf-gitlab-reviewer
skpm package cncf-gitlab-reviewer
skpm publish cncf-gitlab-reviewer
```
