...
7. Run `skcr validate` and `skcr asps coverage cncf-krustlet-reviewer`.
8. Run `skcr compile cncf-krustlet-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-krustlet-reviewer`.
...
```bash
skpm validate cncf-krustlet-reviewer
skpm package cncf-krustlet-reviewer
skpm publish cncf-krustlet-reviewer
```
