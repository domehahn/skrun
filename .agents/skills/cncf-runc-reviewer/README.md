...
7. Run `skcr validate` and `skcr asps coverage cncf-runc-reviewer`.
8. Run `skcr compile cncf-runc-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-runc-reviewer`.
...
```bash
skpm validate cncf-runc-reviewer
skpm package cncf-runc-reviewer
skpm publish cncf-runc-reviewer
```
