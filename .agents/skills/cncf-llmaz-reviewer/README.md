...
7. Run `skcr validate` and `skcr asps coverage cncf-llmaz-reviewer`.
8. Run `skcr compile cncf-llmaz-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-llmaz-reviewer`.
...
```bash
skpm validate cncf-llmaz-reviewer
skpm package cncf-llmaz-reviewer
skpm publish cncf-llmaz-reviewer
```
