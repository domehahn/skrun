...
7. Run `skcr validate` and `skcr asps coverage cncf-puppet-reviewer`.
8. Run `skcr compile cncf-puppet-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-puppet-reviewer`.
...
```bash
skpm validate cncf-puppet-reviewer
skpm package cncf-puppet-reviewer
skpm publish cncf-puppet-reviewer
```
