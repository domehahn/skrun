...
7. Run `skcr validate` and `skcr asps coverage cncf-gthulhu-reviewer`.
8. Run `skcr compile cncf-gthulhu-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-gthulhu-reviewer`.
...
```bash
skpm validate cncf-gthulhu-reviewer
skpm package cncf-gthulhu-reviewer
skpm publish cncf-gthulhu-reviewer
```
