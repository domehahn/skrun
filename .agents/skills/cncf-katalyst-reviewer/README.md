...
7. Run `skcr validate` and `skcr asps coverage cncf-katalyst-reviewer`.
8. Run `skcr compile cncf-katalyst-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-katalyst-reviewer`.
...
```bash
skpm validate cncf-katalyst-reviewer
skpm package cncf-katalyst-reviewer
skpm publish cncf-katalyst-reviewer
```
