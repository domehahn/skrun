...
7. Run `skcr validate` and `skcr asps coverage cncf-metarget-reviewer`.
8. Run `skcr compile cncf-metarget-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-metarget-reviewer`.
...
```bash
skpm validate cncf-metarget-reviewer
skpm package cncf-metarget-reviewer
skpm publish cncf-metarget-reviewer
```
