...
7. Run `skcr validate` and `skcr asps coverage cncf-vjsar-member-reviewer`.
8. Run `skcr compile cncf-vjsar-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-vjsar-member-reviewer`.
...
```bash
skpm validate cncf-vjsar-member-reviewer
skpm package cncf-vjsar-member-reviewer
skpm publish cncf-vjsar-member-reviewer
```
