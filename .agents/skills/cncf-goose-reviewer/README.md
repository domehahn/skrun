...
7. Run `skcr validate` and `skcr asps coverage cncf-goose-reviewer`.
8. Run `skcr compile cncf-goose-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-goose-reviewer`.
...
```bash
skpm validate cncf-goose-reviewer
skpm package cncf-goose-reviewer
skpm publish cncf-goose-reviewer
```
