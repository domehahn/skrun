...
7. Run `skcr validate` and `skcr asps coverage cncf-slurm-reviewer`.
8. Run `skcr compile cncf-slurm-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-slurm-reviewer`.
...
```bash
skpm validate cncf-slurm-reviewer
skpm package cncf-slurm-reviewer
skpm publish cncf-slurm-reviewer
```
