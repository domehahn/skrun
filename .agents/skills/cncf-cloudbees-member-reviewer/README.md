...
7. Run `skcr validate` and `skcr asps coverage cncf-cloudbees-member-reviewer`.
8. Run `skcr compile cncf-cloudbees-member-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-cloudbees-member-reviewer`.
...
```bash
skpm validate cncf-cloudbees-member-reviewer
skpm package cncf-cloudbees-member-reviewer
skpm publish cncf-cloudbees-member-reviewer
```
