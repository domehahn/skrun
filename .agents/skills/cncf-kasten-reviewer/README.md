...
7. Run `skcr validate` and `skcr asps coverage cncf-kasten-reviewer`.
8. Run `skcr compile cncf-kasten-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kasten-reviewer`.
...
```bash
skpm validate cncf-kasten-reviewer
skpm package cncf-kasten-reviewer
skpm publish cncf-kasten-reviewer
```
