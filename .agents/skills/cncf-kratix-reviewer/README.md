...
7. Run `skcr validate` and `skcr asps coverage cncf-kratix-reviewer`.
8. Run `skcr compile cncf-kratix-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-kratix-reviewer`.
...
```bash
skpm validate cncf-kratix-reviewer
skpm package cncf-kratix-reviewer
skpm publish cncf-kratix-reviewer
```
