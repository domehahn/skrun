...
7. Run `skcr validate` and `skcr asps coverage java-reviewer`.
8. Run `skcr compile java-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend java-reviewer`.
...
```bash
skpm validate java-reviewer
skpm package java-reviewer
skpm publish java-reviewer
```
