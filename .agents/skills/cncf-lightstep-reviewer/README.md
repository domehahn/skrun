...
7. Run `skcr validate` and `skcr asps coverage cncf-lightstep-reviewer`.
8. Run `skcr compile cncf-lightstep-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-lightstep-reviewer`.
...
```bash
skpm validate cncf-lightstep-reviewer
skpm package cncf-lightstep-reviewer
skpm publish cncf-lightstep-reviewer
```
