...
7. Run `skcr validate` and `skcr asps coverage cncf-yrcloudfile-reviewer`.
8. Run `skcr compile cncf-yrcloudfile-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-yrcloudfile-reviewer`.
...
```bash
skpm validate cncf-yrcloudfile-reviewer
skpm package cncf-yrcloudfile-reviewer
skpm publish cncf-yrcloudfile-reviewer
```
