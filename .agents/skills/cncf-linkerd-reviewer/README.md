...
7. Run `skcr validate` and `skcr asps coverage cncf-linkerd-reviewer`.
8. Run `skcr compile cncf-linkerd-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-linkerd-reviewer`.
...
```bash
skpm validate cncf-linkerd-reviewer
skpm package cncf-linkerd-reviewer
skpm publish cncf-linkerd-reviewer
```
