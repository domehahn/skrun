...
7. Run `skcr validate` and `skcr asps coverage cncf-temporal-reviewer`.
8. Run `skcr compile cncf-temporal-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-temporal-reviewer`.
...
```bash
skpm validate cncf-temporal-reviewer
skpm package cncf-temporal-reviewer
skpm publish cncf-temporal-reviewer
```
