...
7. Run `skcr validate` and `skcr asps coverage cncf-milvus-reviewer`.
8. Run `skcr compile cncf-milvus-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-milvus-reviewer`.
...
```bash
skpm validate cncf-milvus-reviewer
skpm package cncf-milvus-reviewer
skpm publish cncf-milvus-reviewer
```
