...
7. Run `skcr validate` and `skcr asps coverage cncf-sap-hana-reviewer`.
8. Run `skcr compile cncf-sap-hana-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-sap-hana-reviewer`.
...
```bash
skpm validate cncf-sap-hana-reviewer
skpm package cncf-sap-hana-reviewer
skpm publish cncf-sap-hana-reviewer
```
