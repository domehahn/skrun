...
7. Run `skcr validate` and `skcr asps coverage cncf-werf-reviewer`.
8. Run `skcr compile cncf-werf-reviewer --target skil`.
9. Run `skcr sync` and `skcr version recommend cncf-werf-reviewer`.
...
```bash
skpm validate cncf-werf-reviewer
skpm package cncf-werf-reviewer
skpm publish cncf-werf-reviewer
```
